/* mrealloc.c: Change the size of a block allocated by `mmalloc'.
...
mrealloc(PTR md, PTR ptr, size_t size)
{
  ...
  switch (type)
	...
	{
	  /* New size is smaller; return excess mem to the free list: */
	...
	{
	  /* Will NOT fit, so allocate a new region that will.
