/* mmalloc.c: Memory allocator `malloc'.
...
initialize(struct mdesc *mdp)
{
  ...
  mdp->heapinfo = (malloc_info *)align(mdp, (mdp->heapsize * sizeof(malloc_info)));
...
mmalloc(PTR md, size_t size)
{
  ...
  if (size <= BLOCKSIZE / 2)
      ...
      if (next != NULL)
	{
	  ...
	  mdp->heapstats.bytes_used += (size_t)(1UL << log);
	  mdp->heapstats.chunks_free--;
	  mdp->heapstats.bytes_free -= (size_t)(1UL << log);
	}
      else
	{
	  ...
	  block = BLOCK(result);
	  mdp->heapinfo[block].busy.type = (int)log;
	  mdp->heapinfo[block].busy.info.frag.nfree = (i - 1);
	  mdp->heapinfo[block].busy.info.frag.first = (i - 1);

	  mdp->heapstats.chunks_free += ((BLOCKSIZE >> log) - 1UL);
	  mdp->heapstats.bytes_free += (BLOCKSIZE - (1UL << log));
 	  mdp->heapstats.bytes_used -= (BLOCKSIZE - (1UL << log));
	}
    }
  else
    {
	  ...
	  if (block == start)
	    {
		 ...
		 block; if so, then we do NOT need to get as much.  */
...
struct mdesc *__mmalloc_default_mdp = (struct mdesc *)NULL;
