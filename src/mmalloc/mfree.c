/* mfree.c: Free a block of memory allocated by `mmalloc'.
 ...
 * Like `mfree' but do NOT call a mfree_hook if there is one: */
void
__mmalloc_free(struct mdesc *mdp, PTR ptr)
{
  int type;
  size_t block, blocks;
  register size_t i;
  ...
  switch (type)
    ...
    default:
      ...
      mdp->heapstats.chunks_used--;
      mdp->heapstats.bytes_used -= (size_t)(1UL << type);
      mdp->heapstats.chunks_free++;
      mdp->heapstats.bytes_free += (size_t)(1UL << type);
