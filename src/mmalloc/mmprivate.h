...
#define BLOCKSIZE	((size_t)((unsigned int)1U << BLOCKLOG))
...
#define BLOCK(A) ((size_t)((char *)(A) - mdp->heapbase) / BLOCKSIZE + 1UL)
...
/* Macro to convert from a user supplied malloc descriptor to pointer
 * to the internal malloc descriptor.  If the user supplied descriptor
 * is NULL, then use the default internal version, initializing it
