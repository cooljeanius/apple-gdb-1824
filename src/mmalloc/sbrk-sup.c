/* sbrk-sup.c: Support for sbrk() regions.
...
#if defined(NEED_DECLARATION_SBRK) && NEED_DECLARATION_SBRK
extern PTR sbrk PARAMS((int));
#endif /* NEED_DECLARATION_SBRK */
 ...
 * by sbrk, test the initial value and (if necessary) sbrk enough
 * additional memory to start off with alignment to BLOCKSIZE.
 * We actually only need it aligned to an alignment suitable for any
 * object, so this is overkill.
...
mmalloc_sbrk_init(void)
{
  ...
  ret->base = (char *)base;
  ret->breakval = ret->top = (char *)sbrk(0);
  ret->fd = -1;
