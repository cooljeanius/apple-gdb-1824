...
#else
# warning mmap-sup.c expects <unistd.h> to be included.
#endif /* HAVE_UNISTD_H */
#ifdef HAVE_STDLIB_H
# include <stdlib.h>
#else
...
# include <sys/types.h>
#else
# warning mmap-sup.c expects <sys/types.h> to be included.
...
#if defined(HAVE_MACH_PORT_T) && !defined(HAVE_PORT_T)
# define port_t mach_port_t /* Not sure if this will work, but it is worth a shot... */
#endif /* HAVE_MACH_PORT_T && !HAVE_PORT_T */
...
# endif /* MS_SYNC */
  ...
  if (mdp -> base == 0)
  ...
  else
    {
      ...
      kret = mach_vm_region (mach_task_self(), &r_start, &r_size,
