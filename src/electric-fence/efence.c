 ...
 * fault. It is then trivial to uncover the offending code with a debugger.
 *
 ...
 * the exact instruction that causes the error, rather than waiting until
 * the next boundary check.
 *
 * There is one product that debugs malloc buffer overruns
 * better than Electric Fence: "Purify" from Purify Systems, and that is
 * only a small part of what Purify does. I am not affiliated with Purify,
 * I just respect a job well done.
 *
 * This version of malloc() should not be linked into production software,
 * since it tremendously increases the time/memory overhead of malloc().
 * Each malloc buffer will consume a minimum of two virtual memory pages,
 * this is 16kb+ on many systems. On some systems it will be necessary
 * to increase the amount of swap space in order to debug big programs that
 ...
 * system at one time.  We will break that memory down into smaller pieces
 * for malloc buffers.  One megabyte is probably a good value.  */
 ...
 * so that its name matches the name of the environment variable used
 ...
 * memory will be available for reallocation, but will still be
 * inaccessable until it is reallocated.
 * If the value is -1, it will be set from the environment or to 0 at
 * run-time.  */
 ...
 * over-runs. It will NOT detect both at the same time, so you should test
 * your software twice, once with this value clear, and once with it set.
 ...
 * available to represent new malloc buffers. When this number gets too
 * low, we will create new slots. */
...
initialize(void)
{
	...
	if (size > allocationListSize) {
		...
		slot[1].internalSize = slot[1].userSize = (size -
                                                   slot[0].internalSize);
		slot[1].mode = FREE;
	}
	...
	/* Account for the two slot structures that we have used: */
 ...
 * and get an immediate segmentation fault. It is then easy to zero in on the
 ...
 * so that it will NOT waste even more. It is slow, but thrashing because your
...
efence_memalign(size_t alignment, size_t userSize)
{
	 ...
	 * inaccessable, so that errant programs will NOT scrawl on the
	 * allocator's arena.  I will un-protect it here so that I can make
	 * a new allocation.  I will re-protect it before I return.  */
	if ( !noAllocationListProtection )
		Page_AllowAccess(allocationList, allocationListSize);

	/* If I am running out of empty slots, create some more before
	 * I do NOT have enough slots left to make an allocation.  */
	...
	/* If I am allocating memory for the allocator's own data structures,
	...
	/* If the buffer I have found is larger than I need, split it into
	...
	if (fullSlot->internalSize > internalSize) {
		emptySlots[0]->internalSize = (fullSlot->internalSize
                                       - internalSize);
	 ...
	 * being debugged cannot stomp on it.  */
	if (!internalUse) {
...
efence_malloc(size_t size)
{
	...
	return efence_memalign((size_t)EF_ALIGNMENT, size);
}
