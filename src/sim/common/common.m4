...
# At 1 point there were hardcoded AC_DEFINE's if ${host} = i386-*-windows*.
...
dnl# It determines if IO from the program is routed via STDIO (buffered)
...
dnl# The argument is either a bitmask of things to enable [exactly what is
dnl# up to the simulator], or is a comma separated list of names of
