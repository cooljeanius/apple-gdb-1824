...
def extract_syms(
    cpp: str, srcdir: Path, headers: Iterable[str], pattern: str, filter: str = r"^$"
) -> dict:
...
def gen_common(output_dir: Path, newlib: Path, cpp: str):
    ...
    gentvals(
        ...
        "open",
        newlib / "newlib/libc/include",
        ("fcntl.h", "sys/fcntl.h", "sys/_default_fcntl.h"),
        r"O_[A-Z0-9]*",
    )
...
def gen_target_syscall(output_dir: Path, newlib: Path, cpp: str):
    ...
    new_lines_h.extend(
        f"#define TARGET_NEWLIB_{sym} {val}" for sym, val in sorted(syms.items())
    )
...
def get_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "-o", "--output", type=Path, help="write to the specified directory"
    )
    parser.add_argument(
