...
# include "verify.h"
#else
# if defined(HAVE_ASSERT_H) || defined(STDC_HEADERS) || defined(__STDC__)
#  include <assert.h>
# endif /* HAVE_ASSERT_H || STDC_HEADERS || __STDC__ */
#endif /* 0 */
  ...
  if (p != NULL)
    {
      ...
      if (((int *)p)[-1] == MAGIC_NUMBER)
        {
          /* Looks like a mmalloca() result.  To see whether it really is
           * one, perform a lookup in the hash table.  */
          ...
          for (; *chain != NULL;)
            {
              if (*chain == p)
                {
                  /* Found it; remove it from the hash table & free it: */
