...
mpwify_filename(char *unixname, char *macname)
{
  ...
  j = 0;
  /* If you are going to end up with one or more colons in the middle of a
...
# define DIRBIT 	(1<<4)		/* It is a directory */
...
mpw_end_progress (char *str, char *file, int line)
{
  ...
  /* Warn if it has been a while since the last spin.  */
  if (measure_spin)
    warn_if_spin_delay (file, line);
