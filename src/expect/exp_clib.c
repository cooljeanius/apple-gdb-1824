...
int exp_full_buffer = FALSE;		/* do NOT return on full buffer */
...
/*
   ...
   do NOT cause anything else in the library to be dragged in, so it
   should NOT cause any problems (e.g., bloat).

   The functions are relatively small but painful enough that I do NOT care
...
int fd;
{
	int i, low;
	struct f *fp;
	struct f *newfs;	/* temporary, so we do NOT lose old fs */
...
#if 0
		/* cannot get fread to return early! */
		else {
	...
	after_read:
		...
		for (ec=ecases;ec->type != exp_end;ec++) {
			...
			if (ec->type == exp_glob) {
				...
				TclRegError((char *)0);
				if (Expect_TclRegExec(ec->re,exp_buffer,exp_buffer)) {
					matched = 1;
		 ...
		 * Do NOT bother if we are waiting forever or polling.
		 */
		if (exp_timeout > 0) {
			time(&current_time);
...
# define EALREADY 37
	...
	/* presumably, no stderr, so do NOT bother with error message */
