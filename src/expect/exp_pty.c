...
# include <unistd.h>
#else
# warning exp_pty.c expects <unistd.h> to be included.
#endif /* HAVE_UNISTD_H */
...
/* At some point, I will dump one, but not until I am satisfied no other */
/* changes are needed */
...
exp_pty_test_start()
{
	...
	/* Very few systems do NOT define O_NOCTTY. Only one */
	/* I know of is Next. */
	...
	if (-1 == (lfd = creat(locksrc,0777))) {
		static char buf[256];
		exp_pty_error = buf;
		sprintf(exp_pty_error,"cannot create %s, errno = %d\n",locksrc, errno);
		return(-1);
	}
	close(lfd);
	return 0;
...
char *num;	/* string representation of number */
{
	...
	/* QNX ptys do NOT have a lot of the same properties such as
           read 0 at EOF, etc */
