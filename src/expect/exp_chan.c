...
# include <sys/wait.h>
#else
# warning exp_chan.c expects <sys/wait.h> to be included.
#endif /* HAVE_SYS_WAIT_H */

#ifdef HAVE_UNISTD_H
# include <unistd.h>
#else
# warning exp_chan.c expects <unistd.h> to be included.
#endif /* HAVE_UNISTD_H */
...
#if 0
    /*
      Really should check that we created one first. Since we are sharing fds
      with Tcl, perhaps a filehandler was created with a plain tcl file - we
      would NOT want to delete that. Although if user really close Expect's
      user_spawn_id, it probably does NOT matter anyway.
    */
     ...
     * becomes a requirement, we shall have to revisit this code. But for now, if
     * it is just Tcl exiting, the processes will exit on their own soon
     * anyway.
     */
	...
	/*
	 * If we are in the middle of a bg event handler, then the event
 ...
 * via spawn -open, Tcl can hang if we do NOT close the connections first.
 */
void
...
Tcl_Interp *interp;
{
    ...
    /* no need to keep things in sync (i.e., tsdPtr, count) since we could only
       be doing this if we are exiting. Just close everything down. */
