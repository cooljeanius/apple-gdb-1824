...
/*
 * HP-UX 10.0 with streams (optional) have both PTMX and PTYM. I do NOT
...
#if defined(HAVE_GETPTY) && defined(HAVE_SYS_VTY_H)
# include <sys/vty.h>
static char master_name[MAXPTYNAMELEN];
static char slave_name[MAXPTYNAMELEN];
#elif defined(HAVE_GETPTY)
# warning pty_termios.c expects <sys/vty.h> to be included.
#endif
