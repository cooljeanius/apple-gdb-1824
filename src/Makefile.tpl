...
local-clean:
	-rm -f $(AM_V_v) *.a TEMP errs core *.o *~ \#* TAGS *.E *.log

distclean-local: local-distclean
.PHONY: distclean-local
local-distclean:
	-rm -f $(AM_V_v) Makefile Makefile*orig src_am/Makefile config.h stamp-h1
	-rm -f config.status config.cache config.log.orig conftest.err
	-rm -f $(AM_V_v) ./*/config.cache ./*/config.cache.bak
	-rm -f $(AM_V_v) *~ config/*~ *.sum
	...
	-if test -d texinfo/js && test -w texinfo/js && test -r texinfo/js/Makefile && test -s texinfo/js/Makefile; then \
	  if test -r Makefile && test -s Makefile && test -x config.status; then \
	    $(MAKE) -C texinfo/js $(AM_V_MFLAG) $(FLAGS_TO_PASS) distclean; \
	  elif test -f texinfo/js/Makefile && test "x${enable_werror}" != "xyes" && test -w /dev/null && test -w . && test -w texinfo/js/Makefile; then \
	    touch am--refresh && touch texinfo/js/Makefile; \
	    $(MAKE) -C texinfo/js -ki $(AM_V_MFLAG) $(FLAGS_TO_PASS) distclean 2>/dev/null; \
	    rm -f am--refresh && rm -f texinfo/js/Makefile; \
	  else \
	    echo "It looks like we cannot make texinfo/js be distclean"; \
	  fi; \
	elif test -e texinfo/js; then \
	  echo "Skipping making texinfo/js be distclean"; \
	fi
	find . -name 'am--refresh' -delete
...
mail-report.log: $(srcdir)/contrib/test_summary
...
$(srcdir)/.local.vimrc: $(srcdir)/contrib/vimrc
	$(LN_S) -f $(AM_V_v) $(srcdir)/contrib/vimrc $@
...
config_dot_status_target: configure config.status
	...
	if test ! -x config.status && test -w config.status; then \
	  chmod +x config.status || stat config.status; \
	fi
...
MACRO_DEPS = $(M4CONFDIR)/acx.m4 $(M4CONFDIR)/override.m4 \
  $(M4CONFDIR)/proginstall.m4 $(M4CONFDIR)/multi.m4
