...
ieee_write_byte(bfd *abfd, int barg)
{
  ...
  byte = (bfd_byte)barg;
  if (bfd_bwrite((void *)&byte, (bfd_size_type)1UL, abfd) != 1)
    return FALSE;
  return TRUE;
}
...
bfd_get_string(common_header_type *ieee, char *string, size_t length)
{
  ...
  for (i = 0; i < length; i++)
    string[i] = (char)this_byte_and_next(ieee);
}
...
read_id(common_header_type *ieee)
{
  ...
  string = (char *)bfd_alloc(ieee->abfd, (bfd_size_type)length + 1);
  if (!string)
    return NULL;
...
static bfd_boolean
ieee_write_expression(bfd *abfd, bfd_vma value, asymbol *symbol,
                      bfd_boolean pcrel, unsigned int uindex)
{
  unsigned int term_count = 0U;
  ...
  if (pcrel)
    {
	  ...
	  || ! ieee_write_byte(abfd,
                               (bfd_byte)(uindex + IEEE_SECTION_NUMBER_BASE))
  ...
  while (loop && (ieee->h.input_p < ieee->h.last_byte))
	...
	case ieee_variable_I_enum:
	  /* Push the address of variable n.  */
	  {
	    ...
	    next_byte(&(ieee->h));
	    sy.index = (unsigned int)must_parse_int(&(ieee->h));
	    sy.letter = 'I';
...
get_section_entry(bfd *abfd, ieee_data_type *ieee, unsigned int uindex)
{
  ...
  if (ieee->section_table[uindex] == (asection *)NULL)
    {
      const size_t tmplen = 11UL;
      char *tmp = (char *)bfd_alloc(abfd, (bfd_size_type)tmplen);
      asection *section;

      if (!tmp)
	return NULL;
      snprintf(tmp, tmplen, " fsec%4d", uindex);
      section = bfd_make_section(abfd, tmp);
      ieee->section_table[uindex] = section;
      section->flags = SEC_NO_FLAGS;
      section->target_index = (int)uindex;
      ieee->section_table[uindex] = section;
    }
...
ieee_slurp_debug(bfd *abfd)
{
  ...
  sec->size = (bfd_size_type)(debug_end - ieee->w.r.debug_information_part);
...
ieee_archive_p(bfd *abfd)
{
  ...
  while (1)
      ...
      if (ieee->element_count >= alc_elts)
	{
      ...
      must_parse_int(&(ieee->h));
      t->file_offset = (file_ptr)must_parse_int(&(ieee->h));
      t->abfd = (bfd *)NULL;
  ...
  for (i = 2; i < ieee->element_count; i++)
	...
	ieee->elements[i].file_offset = (file_ptr)must_parse_int(&(ieee->h));
    }
...
ieee_object_p(bfd *abfd)
{
  ...
  /* Determine the architecture and machine type of the object file.  */
  {
	...
	if (processor[2] == '3')	    /* 683xx integrated processors.  */
	  ...
	  {
	    strcpy(family, "68");
	    strncat(family, (processor + 4), 7UL);
	    family[9] = '\0';
	  }
	else				 /* "Regular" processors.  */
	  {
	    strncpy(family, processor, 9UL);
	    family[9] = '\0';
	  }
      }
    else if ((strncmp(processor, "cpu32", 5UL) == 0) /* CPU32 and CPU32+ */
	     || (strncmp(processor, "CPU32", 5UL) == 0))
      strcpy(family, "68332");
    else
      {
	strncpy(family, processor, 9UL);
  ...
  amt = ((bfd_size_type)ieee->w.r.me_record + 1UL);
  IEEE_DATA(abfd)->h.first_byte = (unsigned char *)bfd_alloc(ieee->h.abfd,
                                                             amt);
  ...
  bfd_bread((void *)(IEEE_DATA(abfd)->h.first_byte),
            ((bfd_size_type)ieee->w.r.me_record + 1UL), abfd);
...
static void
ieee_print_symbol(bfd *abfd, void *afile, asymbol *symbol,
...
static bfd_boolean
ieee_get_section_contents(bfd *abfd, sec_ptr section, void *location,
...
static long
ieee_canonicalize_reloc(bfd *abfd, sec_ptr section, arelent **relptr,
                        asymbol **symbols)
{
...
ieee_write_section_part(bfd *abfd)
{
  ...
  for (s = abfd->sections; s != (asection *)NULL; s = s->next)
	  ...
	  && ((s->flags & SEC_DEBUGGING) == 0))
	{
	  ...
	  if (abfd->flags & EXEC_P)
	    {
	      /* Relocateable sections do NOT have asl records.  */
	      /* Vma.  */
...
do_with_relocs(bfd *abfd, asection *s)
{
  ...
  if (relocs_to_go == 0)
      ...
      if (stream == NULL)
	{
	  ...
	  stream = (bfd_byte *)bfd_zalloc(abfd, s->size);
	  if (!stream)
	    return FALSE;
	}
      while (current_byte_index < s->size)
...
static bfd_boolean
ieee_set_section_contents(bfd *abfd, sec_ptr section, const void *location,
                          file_ptr offset, bfd_size_type count)
{
  if ((section->flags & SEC_DEBUGGING) != 0)
    {
...
ieee_write_object_contents(bfd *abfd)
{
  ...
  old = bfd_tell(abfd);
  if (bfd_seek(abfd, (file_ptr)(8L * N_W_VARIABLES), SEEK_CUR) != 0)
    return FALSE;
