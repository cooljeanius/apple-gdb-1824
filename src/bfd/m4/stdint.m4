dnl# 
dnl# GCC_STDINT_TYPES:
dnl# Require some integer type checks
dnl#
...
dnl#
dnl# Sometimes the stdint.h or inttypes.h headers conflict with
dnl# sys/types.h, so we test the headers together with sys/types.h and
dnl# always include it into the generated header (to match the tests with
dnl# the generated file). Hopefully this is not a big annoyance.
dnl#
dnl# If your installed header files require the stdint-types you will want
dnl# to create an installable file mylib-int.h that all your other
dnl# installable header may include. So, for a library package named
dnl# "mylib", just use:
...
dnl# file will include it directly. No need for fuzzy HAVE_STDINT_H
dnl# things...
dnl#
