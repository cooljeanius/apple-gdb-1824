...
# The idea behind this macro is that there is no need to repeat all the
# autoconf probes done by the intl directory - it has already done them
# for us. In fact, there is no need even to look at the cache for the
...
[# If we have NOT got the data from the intl directory,
