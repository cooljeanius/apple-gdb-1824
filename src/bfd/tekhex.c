  ...
  if (! inited)
    {
      ...
      val = 0;
      for (i = 0; i < 10; i++)
	sum_block[i + '0'] = (char)val++;

      for (i = 'A'; i <= 'Z'; i++)
	sum_block[i] = (char)val++;

      sum_block['$'] = (char)val++;
      sum_block['%'] = (char)val++;
      sum_block['.'] = (char)val++;
      sum_block['_'] = (char)val++;
      for (i = 'a'; i <= 'z'; i++)
	sum_block[i] = (char)val++;
    }
}
  ...
  if (!d)
    {
      ...
      d = (struct data_struct *)bfd_zalloc(abfd, (bfd_size_type)sizeof(struct data_struct));
...
insert_byte(bfd *abfd, int value, bfd_vma addr)
{
  ...
  d->chunk_data[addr & CHUNK_MASK] = (char)value;
...
first_phase (bfd *abfd, int type, char *src)
{
  ...
  unsigned int len;
  bfd_vma val;
  /* A symbol can only be 16chars long (+1 for trailing '\0'): */
  char sym[17];
  ...
  switch (type)
	...
	while (*src)
	  {
	    insert_byte(abfd, (int)HEX(src), addr);
	    src += 2;
	    addr++;
	  }
      }
      ...
      while (*src)
	{
	  switch (*src)
		...
		tekhex_symbol_type *newsym = (tekhex_symbol_type *)bfd_alloc(abfd, amt);
		char stype = (*src);
                char *newname;
		...
		if (!getsym(sym, &src, &len))
		  return FALSE;
                 ...
                 * new->symbol.name at instead of casting away the
                 * cosntness of new->symbol.name and poking at the great
                 * compiler warning beastie: */
                newname = (char *)bfd_alloc(abfd,
                                            ((bfd_size_type)len + 1));
                newsym->symbol.name = NULL;
		if (!newname)
		  return FALSE;
		memcpy(newname, sym, (size_t)(len + 1UL));
  ...
  while (! eof)
    {
      ...
      /* Find first '%': */
      eof = (bfd_boolean)(bfd_bread(src, (bfd_size_type)1UL, abfd) != 1);
      while (*src != '%' && !eof)
	eof = (bfd_boolean)(bfd_bread(src, (bfd_size_type)1UL, abfd) != 1);
	...
	break;
      ...
      if (bfd_bread(src, (bfd_size_type)5UL, abfd) != 5)
	abort(); /* FIXME.  */
	  ...
	  if (need)
	    {
	      ...
	      writevalue(&dst, ((bfd_vma)addr + d->vma));
...
static void
tekhex_get_symbol_info(bfd *abfd ATTRIBUTE_UNUSED, asymbol *symbol,
                       symbol_info *ret)
{
...
static void
tekhex_print_symbol(bfd *abfd, void * filep, asymbol *symbol,
