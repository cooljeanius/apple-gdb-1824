...
#define ABS32 4
#define PCREL13 5
#ifndef ABS32_MAYBE_RELAXABLE
# define ABS32_MAYBE_RELAXABLE 1
#endif /* !ABS32_MAYBE_RELAXABLE */
#ifndef ABS32_WAS_RELAXABLE
...
b_out_squirt_out_relocs (bfd *abfd, asection *section)
{
  ...
  for (natptr = native; count > 0; --count, ++natptr, ++generic)
	...
	raw[7] = (unsigned char)(callj_mask + pcrel_mask + len_2);
      else if (g->howto == &howto_reloc_pcrel24)
	raw[7] = (unsigned char)(pcrel_mask + len_2);
      else if (g->howto == &howto_reloc_pcrel13)
	raw[7] = (unsigned char)(pcrel_mask + len_1);
      else if (g->howto == &howto_reloc_abs32code)
	raw[7] = (unsigned char)(len_2 + incode_mask);
      else if ((g->howto >= howto_align_table)
	       && (g->howto
                   <= (howto_align_table + ARRAY_SIZE(howto_align_table) - 1)))
	{
	  /* symnum == -2; extern_mask not set, pcrel_mask set.  */
	  r_idx = -2;
	  r_extern = 0;
	  raw[7] = (unsigned char)(pcrel_mask
                                   | ((g->howto - howto_align_table) << 1U));
	}
      else
	raw[7] = (unsigned char)len_2;
...
extern bfd_boolean aout_32_write_syms
  PARAMS((bfd *));
extern int aout_32_make_sections
  PARAMS((bfd *));
...
b_out_write_object_contents(bfd *abfd)
{
  ...
  exec_hdr(abfd)->a_talign = (unsigned char)obj_textsec(abfd)->alignment_power;
  exec_hdr(abfd)->a_dalign = (unsigned char)obj_datasec(abfd)->alignment_power;
  exec_hdr(abfd)->a_balign = (unsigned char)obj_bsssec(abfd)->alignment_power;
	       ...
	       bfd_boolean shrinking)
{
  ...
  if (IS_OTHER(symbol->other))
  ...
  else if (IS_CALLNAME(symbol->other))
    {
	 ...
	 bal.  */
      word = (int)(BAL | (((unsigned long)word
...
extern int aout_32_slurp_symbol_table
  PARAMS((bfd *));

/* Allocate enough room for all reloc entries, w/pointers to them all: */
static bfd_boolean
b_out_slurp_reloc_table(bfd *abfd, sec_ptr asect, asymbol **symbols)
{
       ...
       counter++, rptr++, cache_ptr++)
  {
    ...
    if (bfd_header_big_endian(abfd))
      symnum = (unsigned int)((raw[4] << 16U) | (raw[5] << 8U) | raw[6]);
    else
      symnum = (unsigned int)((raw[6] << 16U) | (raw[5] << 8U) | raw[4]);
...
/* */
static bfd_boolean
b_out_set_section_contents(bfd *abfd, asection *section,
...
/* */
static bfd_boolean
b_out_set_arch_mach(bfd *abfd, enum bfd_architecture arch,
...

static void
perform_slip(bfd *abfd, unsigned int slip, asection *input_section,
             bfd_vma value)
{
  asymbol **s;
	  ...
	  unsigned int shrink)
{
  ...
  old_end = (((dot + (bfd_vma)size) & (bfd_vma)~size) + ((bfd_vma)size + 1UL));
  /* Work out where the new end will be - remember that we're smaller
     than we used to be.  */
  new_end = ((dot - shrink + (bfd_vma)size) & (bfd_vma)~size);

  /* This is the new end: */
  gap = (old_end - ((dot + (bfd_vma)size) & (bfd_vma)~size));
...
/* */
static bfd_boolean
b_out_bfd_relax_section(bfd *abfd, asection *i,
			struct bfd_link_info *link_info,
                        bfd_boolean *again)
{
  ...
  /* We only run this relaxation once.  It might work to run it multiple
   * times, but it has NOT been tested: */
  *again = FALSE;
  ...
  if (reloc_size)
      ...
      if (reloc_count > 0)
	{
	  ...
	  for (parent = reloc_vector; *parent; parent++)
		...
		case ALIGNER:
		  /* An alignment reloc: */
		  shrink = (unsigned int)aligncode(abfd, input_section, r,
    						   shrink);
		  break;
		case ABS32CODE:
		  /* A 32bit reloc in an addressing mode: */
		  shrink = (unsigned int)abs32code(input_bfd, input_section, r,
       						   shrink, link_info);
		  break;
                                         ...
                                         asymbol **symbols)
{
  ...
  /* If producing relocatable output, then do NOT bother to relax: */
  if (relocatable)
    return bfd_generic_get_relocated_section_contents(output_bfd,
                                                      link_info,
                                                      link_order, data,
                                                      relocatable,
                                                      symbols);
  ...
  /* Read in the section: */
  BFD_ASSERT(bfd_get_section_contents(input_bfd, input_section, data,
                                      (bfd_vma)0UL, input_section->size));
  ...
  if (reloc_count > 0)
    {
      ...
      arelent *reloc;
      unsigned int dst_address = 0U;
      unsigned int src_address = 0U;
      unsigned int run;
      unsigned int idx;
      ...
      while (dst_address < link_order->size)
	{
	  ...
	  if (reloc)
		...
		case ALIGNDONE:
		  ...
		  dst_address = ((dst_address + (unsigned int)reloc->howto->size)
				 & (unsigned int)~reloc->howto->size);
		  break;
		case ABS32CODE_SHRUNK:
		...
		case PCREL24:
		  {
		    ...
		    word = (int)((unsigned long)(word & ~BAL_MASK)
				 | (((word & BAL_MASK)
				     + value
				     - output_addr(input_section)
				     + reloc->addend)
		...
		case PCREL13:
		  {
		    ...
		    value = get_value(reloc, link_info, input_section);
		    word = (int)((unsigned long)(word & ~PCREL13_MASK)
				 | (((word & PCREL13_MASK)
				     + value
				     + reloc->addend
				     - output_addr(input_section))
...
#ifdef ABS32_MAYBE_RELAXABLE
# undef ABS32_MAYBE_RELAXABLE
#endif /* ABS32_MAYBE_RELAXABLE */
