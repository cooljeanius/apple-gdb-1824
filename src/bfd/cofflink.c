/* cofflink.c: COFF specific linker code.
...
static bfd_boolean coff_link_add_object_symbols(bfd *,
                                                struct bfd_link_info *);
static bfd_boolean coff_link_check_archive_element(bfd *,
                                                   struct bfd_link_info *,
                                                   bfd_boolean *);
  ...
  while (esym < esym_end)
	  ...
	  || classification == COFF_SYMBOL_COMMON)
	{
	  ...
	  if (!h && info->pei386_auto_import
	      && !strncmp(name,"__imp_", (size_t)6UL))
		  ...
		  if (sym.n_numaux != 0)
		    {
		      ...
		      (*sym_hash)->numaux = (char)sym.n_numaux;
		      alloc =
                        ((union internal_auxent *)
                         bfd_hash_allocate(&info->hash->table,
                                           (unsigned int)(sym.n_numaux
                                                          * sizeof(*alloc))));
		      if (alloc == NULL)
			goto error_return;
	  ...
	  if (classification == COFF_SYMBOL_PE_SECTION
	      && (*sym_hash)->numaux != 0)
	    {
		 ...
		 For example, it won't help objdump.  This needs to be
	      ...
	      if (section->size == 0)
		section->size = (bfd_size_type)(*sym_hash)->aux[0].x_scn.x_scnlen;
  ...
  if (! info->relocatable
      ...
      if (stabstr != NULL)
	{
	  ...
	  for (stab = abfd->sections; stab; stab = stab->next)
	    if ((strncmp(".stab", stab->name, (size_t)5UL) == 0)
		&& (!stab->name[5]
	  ...
	  if (obj_pe(abfd) && o->reloc_count >= 0xffff)
	    {
		 ...
		 elsewhere. */
	      ...
	      bfd_byte *excount = (bfd_byte *)bfd_malloc((bfd_size_type)relsz);
...
dores_com(char *ptr, bfd *output_bfd, int heap)
{
  if (coff_data(output_bfd)->pe)
    {
      int val = (int)strtoul(ptr, &ptr, 0);

      if (heap)
	pe_data(output_bfd)->pe_opthdr.SizeOfHeapReserve = (bfd_vma)val;
      else
	pe_data(output_bfd)->pe_opthdr.SizeOfStackReserve = (bfd_vma)val;

      if (ptr[0] == ',')
	{
	  val = (int)strtoul((ptr + 1), &ptr, 0);
	  if (heap)
	    pe_data(output_bfd)->pe_opthdr.SizeOfHeapCommit = (bfd_vma)val;
	  else
	    pe_data(output_bfd)->pe_opthdr.SizeOfStackCommit = (bfd_vma)val;
	}
    }
  return ptr;
}
			   ...
			   bfd *abfd)
{
  ...
  for (s = (char *)copy; s < e; )
      ...
      if (s[0] != '-')
	{
      ...
      if (strncmp(s, "-attr", (size_t)5UL) == 0)
	{
	  char *name;
	  char *attribs;
	  asection *asec;
      ...
      else if (strncmp(s, "-heap", (size_t)5UL) == 0)
	s = dores_com((s + 5), output_bfd, 1);
      else if (strncmp(s, "-stack", (size_t)6UL) == 0)
...
_bfd_coff_link_input_bfd(struct coff_final_link_info *finfo, bfd *input_bfd)
{
  ...
  while (esym < esym_end)
	...
	case COFF_SYMBOL_UNDEFINED:
	  *secpp = bfd_und_section_ptr;
      ...
      if (((finfo->info->strip != strip_none)
	   || (finfo->info->discard != discard_none))
	  && finfo->info->relocatable)
	dont_skip_symbol = (bfd_boolean)*indexp;
      else
	  ...
	  && isym.n_numaux == 1)
	{
	  ...
	  eslend = ((bfd_byte *)obj_coff_external_syms(input_bfd)
		    + ((bfd_size_type)aux.x_sym.x_fcnary.x_fcn.x_endndx.l
		       * isymesz));
	  while (esl < eslend)
	    {
	      const char *elename;
	      ...
	      *epp = (struct coff_debug_merge_element*)bfd_alloc(input_bfd,
                                                                 amt);
	      if (*epp == NULL)
		return FALSE;
		...
		return FALSE;
	      ...
	      (*epp)->tagndx = 0;
	      if ((islp->n_numaux >= 1) && (islp->n_type != T_NULL)
		  && (islp->n_sclass != C_EOS))
		{
	  ...
	  switch (isym.n_sclass)
		  ...
		  && (isym.n_scnum > 0))
		{
		  /* For PE, .lf and .ef get their value left alone,
		     ...
		     the new section.  */
		  isym.n_scnum = (short)(*secpp)->output_section->target_index;
		  break;
		}
	      /* Fall through.  */
	    default:
		  ...
		  if ((bfd_size_type)finfo->last_file_index >= syment_base)
		  ...
		  else
		    {
			 ...
			 borrow *outsym temporarily.  */
		      ...
		      pos += (finfo->last_file_index * (file_ptr)osymesz);
  ...
  /* Fix up the aux entries.  This must be done in a separate pass,
     because we do NOT know the correct symbol indices until we have
  ...
  while (esym < esym_end)
      ...
      else
	{
	  ...
	  /* Handle the aux entries.  This handling is based on
	     coff_pointerize_aux.  I do NOT know if it always correct: */
	  for (i = 0; (i < isymp->n_numaux) && (esym < esym_end); i++)
	      ...
	      if (isymp->n_sclass == C_FILE)
		{
		  ...
		  if ((auxp->x_file.x_n.x_zeroes == 0)
		      ...
		      if (strings == NULL)
			{
		      ...
		      if (indx == (bfd_size_type)-1L)
			return FALSE;
		      auxp->x_file.x_n.x_offset = (long)(STRING_SIZE_SIZE
                                                         + indx);
		    }
		}
		       ...
		       && (isymp->n_sclass != C_NT_WEAK))
		{
		  ...
		  if (ISFCN(isymp->n_type) || ISTAG(isymp->n_sclass)
		      || (isymp->n_sclass == C_BLOCK)
                      || (isymp->n_sclass == C_FCN))
		    {
		      indx =
			(unsigned long)auxp->x_sym.x_fcnary.x_fcn.x_endndx.l;
		      if ((indx > 0)
			  && (indx < obj_raw_syment_count(input_bfd)))
			{
			  ...
			  auxp->x_sym.x_fcnary.x_fcn.x_endndx.l = (long)indx;
			}
		    }

		  indx = (unsigned long)auxp->x_sym.x_tagndx.l;
			  ...
			  if ((bfd_size_type)finfo->last_bf_index
			      >= syment_base)
			    {
				 ...
				 endndx symbols correctly.  */
			      auxout = (finfo->outsyms
					+ (((bfd_size_type)finfo->last_bf_index
					    - syment_base)
					   * osymesz));
			  ...
			  else
			    {
			      ...
			      bfd_coff_swap_aux_out(output_bfd,
						    ...
						    outsym);
			      pos = obj_sym_filepos(output_bfd);
			      pos += (finfo->last_bf_index * (file_ptr)osymesz);
			      if (bfd_seek(output_bfd, pos, SEEK_SET) != 0
  ...
  if ((finfo->info->strip == strip_none)
      ...
      for (o = input_bfd->sections; o != NULL; o = o->next)
	{
	  ...
	  for (; eline < elineend; eline += linesz)
	    {
	      ...
	      if (iline.l_lnno != 0)
		iline.l_addr.l_paddr += (bfd_signed_vma)offset;
		  ...
		  if (indx < 0)
		    {
			 ...
			 with symbol #0) will fail.  We cannot regain
			  ...
			  && (is.n_numaux >= 1))
			{
			  ...
			  auxptr = (finfo->outsyms
				    + (((bfd_size_type)indx - syment_base + 1UL)
				       * osymesz));
			  bfd_coff_swap_aux_in(output_bfd, auxptr,
                                               is.n_type, is.n_sclass,
                                               0, is.n_numaux, &ia);
			  ia.x_sym.x_fcnary.x_fcn.x_lnnoptr =
			    ((bfd_size_type)o->output_section->line_filepos
	  ...
	  pos += (file_ptr)(o->output_section->lineno_count * linesz);
	  amt = (bfd_size_type)(oeline - finfo->linenos);
	  if ((bfd_seek(output_bfd, pos, SEEK_SET) != 0)
	      || (bfd_bwrite(finfo->linenos, amt, output_bfd) != amt))
	    return FALSE;

	  o->output_section->lineno_count += (unsigned int)(amt / linesz);
	}
    }
  ...
  if (outsym > finfo->outsyms)
    {
      ...
      pos = (obj_sym_filepos(output_bfd) + (file_ptr)(syment_base * osymesz));
  ...
  for (o = input_bfd->sections; o != NULL; o = o->next)
	...
	{
	  file_ptr loc =
	    (file_ptr)(o->output_offset * bfd_octets_per_byte(output_bfd));
...
_bfd_coff_write_global_sym (struct coff_link_hash_entry *h, void *data)
{
  ...
  switch (h->root.type)
	...
	if (bfd_is_abs_section (sec))
	  ...
	  isym.n_scnum = (short)sec->target_index;
    ...
    case bfd_link_hash_indirect:
      /* Just ignore these.  They cannot be handled anyhow: */
      return TRUE;
    }

  if (strlen(h->root.root.string) <= SYMNMLEN)
    strncpy(isym._n._n_name, h->root.root.string, (size_t)SYMNMLEN);
  else
    {
      bfd_boolean hash;
      bfd_size_type indx;
  ...
  isym.n_numaux = (unsigned char)h->numaux;
  ...
  pos = obj_sym_filepos(output_bfd);
  pos += (file_ptr)(obj_raw_syment_count(output_bfd) * symesz);
  if (bfd_seek (output_bfd, pos, SEEK_SET) != 0
  ...
  for (i = 0; i < isym.n_numaux; i++)
	      ...
	      ||( h->root.type == bfd_link_hash_defweak)))
	{
	  ...
	  if (sec != NULL)
	    {
	      auxp->x_scn.x_scnlen = (long)sec->size;
		...
		(*_bfd_error_handler)
	      ...
	      auxp->x_scn.x_nreloc = (unsigned short)sec->reloc_count;
	      auxp->x_scn.x_nlinno = (unsigned short)sec->lineno_count;
			    ...
			    struct bfd_link_order *link_order)
{
  ...
  if (link_order->u.reloc.p->addend != 0)
	...
	case bfd_reloc_overflow:
      ...
      loc = (file_ptr)(link_order->offset * bfd_octets_per_byte(output_bfd));
	  ...
	  if (sym && pe_data (output_bfd)->in_reloc_p (output_bfd, howto))
	    {
		 ...
		 and dlltool reads in a long.  */
	      long addr = (long)(rel->r_vaddr
                                 - input_section->vma
                                 + input_section->output_offset
                                 + input_section->output_section->vma);
	      if (coff_data(output_bfd)->pe)
		addr -= (long)pe_data(output_bfd)->pe_opthdr.ImageBase;
	      if (fwrite(&addr, (size_t)1UL, sizeof(long),
                         (FILE *)info->base_file) != sizeof(long))
		{
