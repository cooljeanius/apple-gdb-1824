/* elf-strtab.c: ELF strtab with GC and suffix merging support.
...
bfd_size_type
_bfd_elf_strtab_add(struct elf_strtab_hash *tab, const char *str,
		    bfd_boolean copy)
{
...
_bfd_elf_strtab_finalize(struct elf_strtab_hash *tab)
{
  ...
  /* GCC 2.91.66 (egcs-1.1.2) on i386 miscompiles this function when i is
     ...
     Besides, indexing with a long long would NOT give anything but extra
     cycles.  */
  size_t i;
  ...
  if (size != 0)
    {
	 ...
	 ie. we do NOT want s1 pointing into the old s2.  */
      e = *--a;
      e->len += 1;
      while (--a >= array)
	{
