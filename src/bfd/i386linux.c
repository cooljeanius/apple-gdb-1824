/* i386linux.c: BFD back-end for linux flavored i386 a.out binaries.
                        ...
                        struct bfd_hash_table *table, const char *string)
{
  ...
  if (ret != NULL)
    {
      ; /* Set local fields; there are none to set, so do nothing.  */
    }
                                    ...
                                    struct bfd_link_info *info)
{
  ...
  if (s != NULL)
    {
      s->size = (linux_hash_table(info)->fixup_count + 1UL);
      s->size *= 8;
...
linux_finish_dynamic_link(bfd *output_bfd, struct bfd_link_info *info)
{
  ...
  printf("Fixup table file offset: %lx  VMA: %"BFD_VMA_FMT"x\n",
	 (unsigned long)(os->filepos + s->output_offset),
	 (bfd_vma)(os->vma + s->output_offset));
