/* elf32-epiphany.c: Adapteva epiphany specific support for 32-bit ELF
			   ...
			   bfd_boolean *again)
{
  ...
  static unsigned long page_start = 0UL;
  static unsigned long page_end = 0UL;
  static unsigned int pass = 0U;
  ...
  *again = FALSE;
  ...
  if ((pass == 1) || (new_pass && !changed))
	...
	{
#if defined(HAVE_EPIPHANY_ELF_RELAX_SECTION_PAGE) && HAVE_EPIPHANY_ELF_RELAX_SECTION_PAGE
	  if (!epiphany_elf_relax_section_page(abfd, sec, &changed, &misc,
                                               page_start, page_end))
	    return FALSE;
#else
	  (void)misc;
#endif /* HAVE_EPIPHANY_ELF_RELAX_SECTION_PAGE */
	}
      *again = TRUE;
    }
...
epiphany_final_link_relocate(reloc_howto_type *howto, bfd *input_bfd,
...
#ifdef bfd_elf32_bfd_reloc_name_lookup
# undef bfd_elf32_bfd_reloc_name_lookup
#endif /* bfd_elf32_bfd_reloc_name_lookup */

/* End of elf32-epiphany.c */
