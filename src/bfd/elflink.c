...
_bfd_elf_create_got_section(bfd *abfd, struct bfd_link_info *info)
{
  ...
  if ((s == NULL)
      || !bfd_set_section_alignment(abfd, s, (unsigned int)ptralign))
    return FALSE;
...
_bfd_elf_create_dynamic_sections(bfd *abfd, struct bfd_link_info *info)
{
  ...
  if (bed->plt_not_loaded)
    ...
    pltflags &= (flagword)~(SEC_CODE | SEC_LOAD | SEC_HAS_CONTENTS);
  else
				    ...
				    struct elf_link_hash_entry *h)
{
  if (h->dynindx == -1)
	...
	case STV_HIDDEN:
	  ...
	  ATTRIBUTE_FALLTHROUGH; /* could just as easily break, but w/e */
	default:
	  break;
	}

      h->dynindx = (long)elf_hash_table(info)->dynsymcount;
					 ...
					 bfd *input_bfd, long input_indx)
{
  ...
  /* Go find the symbol, so that we can find its name: */
  if (!bfd_elf_get_elf_syms(input_bfd, &elf_tdata(input_bfd)->symtab_hdr,
			    1UL, (size_t)input_indx, &entry->isym, esym,
                            &eshndx))
    {
                               ...
                               unsigned long *section_sym_count)
{
  ...
  if (info->shared || elf_hash_table(info)->is_relocatable_executable)
	...
	if (((p->flags & SEC_EXCLUDE) == 0)
	  ...
	  elf_section_data(p)->dynindx = (int)(++dynsymcount);
    }
                      ...
                      bfd_boolean *size_change_ok)
{
  ...
  const struct elf_backend_data *abed;
  ...
  *sym_hash = h;
  ...
  abed = get_elf_backend_data(abfd);
  if (abed->merge_symbol
      && !abed->merge_symbol(info, sym_hash, h, sym, psec, pvalue,
  ...
  if (newdyncommon
      && (h->root.type == bfd_link_hash_common))
    {
      *override = TRUE;
      ...
      newdyncommon = FALSE;
      *pvalue = sym->st_size;
      *psec = sec = abed->common_section(oldsec);
      *size_change_ok = TRUE;
    }
  ...
  if (flip != NULL)
    {
	 ...
	 case, we make the versioned symbol point to the normal one.  */
      const struct elf_backend_data *ebed = get_elf_backend_data(abfd);
      flip->root.type = h->root.type;
      h->root.type = bfd_link_hash_indirect;
      h->root.u.i.link = (struct bfd_link_hash_entry *) flip;
      (*ebed->elf_backend_copy_indirect_symbol)(ebed, flip, h);
			     ...
			     bfd_boolean override)
{
  ...
  shortlen = (size_t)(p - name);
  shortname = (char *)bfd_hash_allocate(&info->hash->table,
  					((unsigned int)shortlen + 1U));
  if (shortname == NULL)
    return FALSE;
  ...
  sec = *psec;
  if (!_bfd_elf_merge_symbol(abfd, info, shortname, sym, &sec, value, NULL,
...
nondefault:
  len = strlen(name);
  shortname = (char *)bfd_hash_allocate(&info->hash->table, (unsigned int)len);
  if (shortname == NULL)
    return FALSE;
  memcpy(shortname, name, shortlen);
					 ...
					 void *data)
{
  ...
  a->vna_other = (unsigned short)(h->verinfo.verdef->vd_exp_refno + 1U);
...
_bfd_elf_link_assign_sym_version(struct elf_link_hash_entry *h, void *data)
{
  ...
  if ((p != NULL) && (h->verinfo.vertree == NULL))
	  ...
	  if (strcmp(t->name, p) == 0)
	    {
	      ...
	      len = (size_t)(p - h->root.root.string);
	      alc = (char *)bfd_malloc(len);
	      if (alc == NULL)
		return FALSE;
	      memcpy(alc, h->root.root.string, (len - 1UL));
	      alc[len - 1] = '\0';
      ...
      if (t == NULL && info->executable)
	{
	  ...
	  amt = sizeof(*t);
	  t = (struct bfd_elf_version_tree *)bfd_zalloc(sinfo->output_bfd, amt);
	  if (t == NULL)
	    {
	      sinfo->failed = TRUE;
	      return FALSE;
	  ...
	  for (pp = &sinfo->verdefs; *pp != NULL; pp = &(*pp)->next)
	    ++version_index;
	  t->vernum = (unsigned int)version_index;
...
static bfd_boolean
elf_link_read_relocs_from_section(bfd *abfd, asection *sec,
			   ...
			   bfd_boolean keep_memory)
{
  ...
  if (internal_relocs == NULL)
	...
	{
	  alloc2 = (Elf_Internal_Rela *)bfd_malloc(size);
...
bfd_boolean
_bfd_elf_link_size_reloc_section(bfd *abfd, Elf_Internal_Shdr *rel_hdr,
				 asection *o)
{
...
_bfd_elf_link_sec_merge_syms (struct elf_link_hash_entry *h, void *data)
{
  ...
  if ((h->root.type == bfd_link_hash_defined
      ...
      && sec->sec_info_type == ELF_INFO_TYPE_MERGE)
    {
      ...
      h->root.u.def.value =
	_bfd_merged_section_offset(output_bfd, &h->root.u.def.section,
...
bfd_boolean
_bfd_elf_add_dynamic_entry(struct bfd_link_info *info, bfd_vma tag, bfd_vma val)
{
  struct elf_link_hash_table *hash_table;
  const struct elf_backend_data *bed;
  asection *s;
...
elf_sort_symbol(const void *arg1, const void *arg2)
{
  ...
  vdiff = (bfd_signed_vma)(h1->root.u.def.value - h2->root.u.def.value);
  if (vdiff != 0)
    return ((vdiff > 0L) ? 1 : -1);
  else
    {
...
elf_link_add_object_symbols (bfd *abfd, struct bfd_link_info *info)
{
  ...
  struct elf_link_hash_entry **sym_hash;
  bfd_boolean dynamic;
  Elf_External_Versym *extversym = (Elf_External_Versym *)NULL;
  Elf_External_Versym *ever;
  struct elf_link_hash_entry *weaks;
  struct elf_link_hash_entry **nondeflt_vers = NULL;
  bfd_size_type nondeflt_vers_cnt = 0UL;
      ...
      for (s = abfd->sections; s != NULL; s = s->next)
	{
	  ...
	  if (strncmp (name, ".gnu.warning.", sizeof ".gnu.warning." - 1) == 0)
	      ...
	      if (dynamic)
		{
	      ...
	      sz = s->size;
	      msg = (char *)bfd_alloc(abfd, (sz + 1UL));
	      if (msg == NULL)
		goto error_return;
  ...
  if (! dynamic)
      ...
      if (s != NULL)
	{
	  ...
	  for (extdyn = dynbuf;
		...
		{
		  unsigned int tagv = (unsigned int)dyn.d_un.d_val;
		  soname =
      		    bfd_elf_string_from_elf_section(abfd, (unsigned int)shlink,
              					    tagv);
		  if (soname == NULL)
		    goto error_free_dyn;
		}
  ...
  if (dynamic)
    {
      ...
      if (!_bfd_elf_slurp_version_tables(abfd, info->default_imported_symver))
	goto error_free_sym;
  ...
  for (isym = isymbuf, isymend = (isymbuf + extsymcount);
      ...
      if (is_elf_hash_table(hash_table))
	{
	  ...
	  if (ever == NULL)
		...
		iver.vs_vers = (unsigned short)elf_tdata(abfd)->cverdefs;
	      else
		iver.vs_vers = 0;
	    }
	  else
	  ...
	  if (((iver.vs_vers & VERSYM_HIDDEN) != 0)
	      ...
	      if (isym->st_shndx != SHN_UNDEF)
		{
		  if (vernum > elf_tdata(abfd)->cverdefs)
		    ...
		    verstr =
		      elf_tdata(abfd)->verdef[vernum - 1U].vd_nodename;
		  else
		    verstr = "";
      ...
      if (is_elf_hash_table (hash_table))
	{
	  ...
	  if ((old_alignment || common)
	      && h->root.type != bfd_link_hash_common)
	    {
	      ...
	      symbol_align =
       	 	((unsigned int)ffsl((long)h->root.u.def.value) - 1U);
	      if ((h->root.u.def.section->owner != NULL)
		  && ((h->root.u.def.section->owner->flags & DYNAMIC) == 0))
		{
		...
		(*_bfd_error_handler)
		  (_("Warning: alignment %u of symbol `%s' in %B"
		   ...
		   (1U << normal_align), name, (1U << common_align));
	    }
	  ...
	  if (dynsym && h->dynindx == -1)
	    {
	      if (! bfd_elf_link_record_dynamic_symbol(info, h))
		goto error_free_vers;
	      if ((h->u.weakdef != NULL) && !new_weakdef
		  && (h->u.weakdef->dynindx == -1))
		{
		  if (!bfd_elf_link_record_dynamic_symbol(info, h->u.weakdef))
		    goto error_free_vers;
  ...
  if (nondeflt_vers != NULL)
      ...
      for (cnt = 0; cnt < nondeflt_vers_cnt; ++cnt)
	{
	  ...
	  p = strchr(h->root.root.string, ELF_VER_CHR);
	  if ((p == NULL) || ((h->root.type != bfd_link_hash_defined)
                              && (h->root.type != bfd_link_hash_defweak)))
	    continue;

	  amt = (bfd_size_type)(p - h->root.root.string);
	  shortname = (char *)bfd_malloc(amt + 1UL);
	  ...
	  hi = ((struct elf_link_hash_entry *)
                bfd_link_hash_lookup(&hash_table->root, shortname,
                                     FALSE, FALSE, FALSE));
	  if ((hi != NULL) && (hi->root.type == h->root.type)
  ...
  if (weaks != NULL)
      ...
      while (weaks != NULL)
	{
	  ...
	  while (i < j)
	    {
	      ...
	      vdiff = (bfd_signed_vma)(vlook - h->root.u.def.value);
	      if (vdiff < 0)
		j = idx;
	      else if (vdiff > 0)
		i = (idx + 1UL);
	  ...
	  for (i = (size_t)ilook; i < sym_count; i++)
	    {
	      h = sorted_sym_hash[i];
...
_bfd_elf_archive_symbol_lookup(bfd *abfd, struct bfd_link_info *info,
                               const char *name)
{
  struct elf_link_hash_entry *h;
  char *p, *copy;
...
compute_bucket_count (struct bfd_link_info *info)
{
  ...
  if (info->optimize)
    {
      unsigned long int nsyms = (unsigned long)(hashcodesp - hashcodes);
      size_t minsize;
      size_t maxsize;
      BFD_HOST_U_64_BIT best_chlen = ~((BFD_HOST_U_64_BIT)0UL);
      ...
      else
	{
	  ...
	  for (t = verdefs; t != NULL; t = t->next)
		...
		return FALSE;
	      ...
	      def.vd_version = VER_DEF_CURRENT;
	      def.vd_flags = 0;
	      if ((t->globals.list == NULL) && (t->locals.list == NULL)
		  && ! t->used)
		def.vd_flags |= VER_FLG_WEAK;
	      def.vd_ndx = (unsigned short)(t->vernum
                                            + (info->create_default_symver
                                               ? 2U : 1U));
	      def.vd_cnt = (unsigned short)(cdeps + 1U);
	      def.vd_hash = bfd_elf_hash(t->name);
	      def.vd_aux = sizeof(Elf_External_Verdef);
	      def.vd_next = 0;
	      if (t->next != NULL)
		def.vd_next = (sizeof(Elf_External_Verdef)
			       + ((cdeps + 1U) * sizeof(Elf_External_Verdaux)));
		...
		defaux.vda_next = sizeof(Elf_External_Verdaux);
	      t->name_indx = (unsigned int)defaux.vda_name;
...
elf_link_sort_relocs(bfd *abfd, struct bfd_link_info *info, asection **psec)
{
  ...
  sort_elt = (sizeof(struct elf_link_sort_rela)
	      + ((size_t)i2e - 1UL) * sizeof(Elf_Internal_Rela));
  ...
  if (finfo->symbuf_count > 0)
    {
      ...
      pos = (hdr->sh_offset + (file_ptr)hdr->sh_size);
...
elf_link_output_extsym (struct elf_link_hash_entry *h, void *data)
{
  ...
  if ((sym.st_shndx == SHN_UNDEF)
	...
	bindtype = STB_WEAK;
      sym.st_info = (unsigned char)ELF_ST_INFO(bindtype,
  ...
  if ((h->dynindx != -1)
      && elf_hash_table(finfo->info)->dynamic_sections_created)
    {
      ...
      hash_entry_size = elf_section_data(finfo->hash_sec)->this_hdr.sh_entsize;
      bucketpos = ((bfd_byte *)finfo->hash_sec->contents
		   + ((bucket + 2UL) * hash_entry_size));
      chain = bfd_get((8UL * hash_entry_size), finfo->output_bfd, bucketpos);
      bfd_put((8UL * hash_entry_size), finfo->output_bfd, (bfd_vma)h->dynindx,
              bucketpos);
      bfd_put((8UL * hash_entry_size), finfo->output_bfd, (bfd_vma)chain,
	      ((bfd_byte *)finfo->hash_sec->contents
               + ((bucketcount + 2UL + (size_t)h->dynindx)
                  * hash_entry_size)));
      ...
      if ((finfo->symver_sec != NULL) && (finfo->symver_sec->contents != NULL))
	{
	  ...
	  if (!h->def_regular)
		...
		iversym.vs_vers =
    		  (unsigned short)(h->verinfo.verdef->vd_exp_refno + 1U);
	    }
	  else
	    {
		...
		iversym.vs_vers =
  		  (unsigned short)(h->verinfo.vertree->vernum + 1U);
		  ...
		  *rel_hash = NULL;
		  ...
		  if (ELF_ST_TYPE (sym.st_info) == STT_SECTION)
		      ...
		      else
			{
			  ...
			  if (!bfd_is_abs_section(osec))
			    {
			      r_symndx = (unsigned long)osec->target_index;
			      BFD_ASSERT(r_symndx != 0);
			    }
			}
		      ...
		      if (finfo->indices[r_symndx] == -1)
			{
		      ...
		      r_symndx = (unsigned long)finfo->indices[r_symndx];
		    }
		      ...
		      struct bfd_link_order *link_order)
{
  ...
  if (bed->s->arch_size == 32)
    irel[0].r_info = (bfd_vma)ELF32_R_INFO(indx, howto->type);
  else
...
elf_fixup_link_order (bfd *abfd, asection *o)
{
  ...
  qsort(sections, (size_t)seen_linkorder, sizeof(struct bfd_link_order *),
        compare_link_order);
...
bfd_elf_final_link (bfd *abfd, struct bfd_link_info *info)
{
  ...
  if (elf_numsections (abfd) > SHN_LORESERVE)
    {
      /* Wild guess at number of output symbols; realloc-ed as needed: */
  ...
  if ((info->strip != strip_all) || emit_relocs)
      ...
      for (i = 1; i < elf_numsections(abfd); i++)
	{
	  ...
	  if (o != NULL)
	    o->target_index = (int)bfd_get_symcount(abfd);
	  elfsym.st_shndx = i;
  ...
  if (elf_hash_table (info)->tls_sec)
	   ...
	   sec = sec->next)
	{
	  ...
	  if ((size == 0) && ((sec->flags & SEC_HAS_CONTENTS) == 0))
	    {
	      struct bfd_link_order *lorder;

	      for (lorder = sec->map_head.link_order; lorder != NULL; lorder = lorder->next)
		if (size < (lorder->offset + lorder->size))
		  size = (lorder->offset + lorder->size);
	    }
	  end = (sec->vma + size);
	}
      ...
      if (info->shared || elf_hash_table(info)->is_relocatable_executable)
	{
	  ...
	  for (s = abfd->sections; s != NULL; s = s->next)
		...
		continue;
	      ...
	      BFD_ASSERT(indx > 0);
	      sym.st_shndx = (unsigned int)indx;
	      sym.st_value = s->vma;
      ...
      if (elf_hash_table(info)->dynlocal)
	{
	  ...
	  for (e = elf_hash_table(info)->dynlocal; e ; e = e->next)
		      ...
		      || (e->isym.st_shndx > SHN_HIRESERVE)))
		{
		  ...
		  sym.st_shndx =
		    (unsigned int)elf_section_data(s->output_section)->this_idx;
...
elf_gc_sweep (struct bfd_link_info *info, gc_sweep_hook_fn gc_sweep_hook)
{
     ...
     static symbol table as well?  */
  {
    ...
    elf_hash_table(info)->dynsymcount = (bfd_size_type)i;
  }
			     ...
			     bfd_vma offset)
{
 ...
 win:
  if (!child->vtable)
    {
      child->vtable = ((struct elf_link_vtable *)
			  ...
			  struct elf_link_hash_entry *h, bfd_vma addend)
{
  ...
  if (!h->vtable)
    {
      h->vtable = (struct elf_link_vtable *)bfd_zalloc(abfd,
                                                       sizeof(*h->vtable));
      if (!h->vtable)
	return FALSE;
    }
                                       ...
                                       struct bfd_link_info *info)
{
  ...
  for (i = info->input_bfds; i; i = i->link_next)
      ...
      for (j = 0; j < locsymcount; ++j)
	{
	  if (local_got[j] > 0)
	    {
	      local_got[j] = (bfd_signed_vma)gotoff;
	      gotoff += got_elt_size;
	    }
	  else
