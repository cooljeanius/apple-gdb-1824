...
_bfd_mn10300_elf_create_got_section(bfd *abfd, struct bfd_link_info *info)
{
  ...
  if (bed->plt_not_loaded)
    pltflags &= (flagword)~(SEC_LOAD | SEC_HAS_CONTENTS);
  if (bed->plt_readonly)
...
static void
mn10300_info_to_howto(bfd *abfd ATTRIBUTE_UNUSED, arelent *cache_ptr,
...
static bfd_boolean
mn10300_elf_check_relocs(bfd *abfd, struct bfd_link_info *info,
  ...
  for (rel = relocs; rel < rel_end; rel++)
	...
	case R_MN10300_PLT16:
	  ...
	  /* This symbol requires a procedure linkage table entry.  We
	     ...
	     do NOT need to generate a procedure linkage table entry
	     after all.  */
	...
	case R_MN10300_32:
	  ...
	  if (info->shared
	      ...
	      if (sreloc == NULL)
		{
		  ...
		  BFD_ASSERT((strncmp(name, ".rela", 5UL) == 0)
                                ...
                                int is_local ATTRIBUTE_UNUSED)
{
  ...
  switch (r_type)
	  ...
	  && (input_section->flags & SEC_ALLOC) != 0)
	{
	  ...
	  if (skip)
	      ...
	      else
		{
		  ...
		  relocate = FALSE;
		  outrel.r_info = (bfd_vma)ELF32_R_INFO(h->dynindx,
    	  						R_MN10300_32);
		  outrel.r_addend = (value + addend);
		}
	    }
	  ...
	  && (h->plt.offset != (bfd_vma)-1L))
	{
	  ...
	  other_splt = bfd_get_section_by_name(dynobj, ".plt");

	  value = ((other_splt->output_section->vma
                    + other_splt->output_offset
                    + h->plt.offset) - value);
	}
	  ...
	  && (h->plt.offset != (bfd_vma)-1L))
	{
	  ...
	  our_splt = bfd_get_section_by_name(dynobj, ".plt");

	  value = ((our_splt->output_section->vma
		   + our_splt->output_offset
		   + h->plt.offset) - value);
	}
	...
	s_got = bfd_get_section_by_name(dynobj, ".got");
	  ...
	  if (h != NULL)
		 ...
		 * When doing a dynamic link, we create a .rela.got
                 * relocation entry to initialize the value.  This is done
                 * in the finish_dynamic_symbol routine: */
		bfd_put_32(output_bfd, value, (s_got->contents + off));

	      value = (s_got->output_offset + off);
	    }
	  else
	    {
	      bfd_vma off;
	      ...
	      if (info->shared)
		{
		  ...
		  outrel.r_offset = (s_got->output_section->vma
				     + s_got->output_offset + off);
		  outrel.r_info = ELF32_R_INFO(0, R_MN10300_RELATIVE);
      ...
      /* end brackets that begin at the start of this case */
...
static bfd_boolean
mn10300_elf_relax_section(bfd *abfd, asection *sec,
	   ...
	   input_bfd = input_bfd->link_next)
	{
	  ...
	  for (section = input_bfd->sections;
	      ...
	      if ((section->flags & SEC_CODE) != 0)
		{
		  ...
		  sec_shndx =
                    (unsigned int)_bfd_elf_section_from_bfd_section(input_bfd,
                                                                    section);
	   ...
	   input_bfd = input_bfd->link_next)
	{
	  ...
	  for (section = input_bfd->sections;
	      ...
	      if (section->reloc_count != 0)
		{
		  /* Get a copy of the native relocations.  */
		  internal_relocs =
                    _bfd_elf_link_read_relocs(input_bfd, section, (PTR)NULL,
                                              (Elf_Internal_Rela *)NULL,
                                              link_info->keep_memory);
		  if (internal_relocs == NULL)
		    goto error_return;
                      ...
                      bfd_vma addr, unsigned char *contents)
{
  ...
  if ((byte1 == 0xf8) && (byte2 == 0xfe))
	...
	temp = bfd_get_8(abfd, (addr + 2));
      ...
      hash->stack_size = (unsigned char)-temp;
    }
...
static bfd_boolean
mn10300_elf_relax_delete_bytes(bfd *abfd, asection *sec, bfd_vma addr,
                               int count)
{
  Elf_Internal_Shdr *symtab_hdr;
  unsigned int sec_shndx;
  ...
  sec_shndx = (unsigned int)_bfd_elf_section_from_bfd_section(abfd, sec);
 ...
 * FALSE.  */
static bfd_boolean
mn10300_elf_symbol_address_p(bfd *abfd, asection *sec,
...
elf32_mn10300_link_hash_table_free(struct bfd_link_hash_table *hash)
{
  ...
  _bfd_generic_link_hash_table_free((struct bfd_link_hash_table *)ret->static_hash_table);
                                        ...
                                        bfd_boolean linker ATTRIBUTE_UNUSED)
{
  ...
  elf_elfheader(abfd)->e_flags &= (unsigned long)~(EF_MN10300_MACH);
...
static bfd_boolean
_bfd_mn10300_elf_create_dynamic_sections(bfd *abfd,
                                         struct bfd_link_info *info)
{
  flagword flags;
  asection *s;
                                       ...
                                       Elf_Internal_Sym *sym)
{
  ...
  if (h->plt.offset != (bfd_vma)-1)
      ...
      else
	{
      ...
      rel.r_offset = (sgot->output_section->vma
		      + sgot->output_offset
		      + got_offset);
      rel.r_info = (bfd_vma)ELF32_R_INFO(h->dynindx, R_MN10300_JMP_SLOT);
      rel.r_addend = 0;
  ...
  if (h->got.offset != (bfd_vma)-1)
      ...
      else
	{
	  ...
	  rel.r_info = (bfd_vma)ELF32_R_INFO(h->dynindx, R_MN10300_GLOB_DAT);
	  rel.r_addend = 0;
	}
  ...
  if (h->needs_copy)
    {
      ...
      rel.r_offset = (h->root.u.def.value
		      + h->root.u.def.section->output_section->vma
		      + h->root.u.def.section->output_offset);
      rel.r_info = (bfd_vma)ELF32_R_INFO(h->dynindx, R_MN10300_COPY);
      rel.r_addend = 0;
      bfd_elf32_swap_reloca_out(output_bfd, &rel,
