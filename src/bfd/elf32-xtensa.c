  ...
  if (internal_relocs && !table_section->reloc_done)
      ...
      for (i = 0; i < table_section->reloc_count; i++)
	{
	  ...
	  if (get_elf_r_symndx_section(abfd, r_symndx) == section)
		...
		blocks[block_count].flags =
		  (flagword)bfd_get_32(abfd, (table_data + rel->r_offset + 8));
	      block_count++;
	    }
	}
    }
      ...
      for (off = 0; off < table_size; off += table_entry_size)
	{
	  ...
	  if ((address >= section_addr)
		...
		blocks[block_count].flags =
		  (flagword)bfd_get_32(abfd, (table_data + off + 8));
	      block_count++;
	    }
	}
    }
                    ...
                    bfd_boolean is_weak_undef, const char **error_message)
{
  ...
  if (xtensa_operand_do_reloc(isa, opcode0, opnd, &newval, (uint32)self_address)
      ...
      else if (opcode0 == get_l32r_opcode())
	{
      ...
      *error_message = vsprint_msg(opname, ": %s", ((int)strlen(msg) + 2),
                                   msg);
      return bfd_reloc_dangerous;
    }
				    ...
				    struct bfd_link_info *info)
{
  ...
  if (srelplt && srelplt->size != 0)
	...
	{
	  loc = (srelgot->contents + (rtld_reloc
                                      * sizeof(Elf32_External_Rela)));
      ...
      plt_entries = ((unsigned int)srelplt->size / sizeof(Elf32_External_Rela));
      plt_chunks =
	((plt_entries + PLT_ENTRIES_PER_CHUNK - 1) / PLT_ENTRIES_PER_CHUNK);
...
elf_xtensa_merge_private_bfd_data(bfd *ibfd, bfd *obfd)
{
  ...
  out_flag = (flagword)elf_elfheader(obfd)->e_flags;
  in_flag = (flagword)elf_elfheader(ibfd)->e_flags;
...
{
  FILE *f = (FILE *)farg;
  flagword e_flags = (flagword)elf_elfheader(abfd)->e_flags;
...
elf_xtensa_grok_prstatus(bfd *abfd, Elf_Internal_Note *note)
{
  ...
  /* pr_pid */
  elf_tdata(abfd)->core_pid = (int)bfd_get_32(abfd, (note->descdata + 24));

  /* pr_reg */
  offset = 72;
  size = (unsigned int)(note->descsz - offset - 4U);
			   ...
			   bfd_vma content_length, const char **error_message)
{
  ...
  opcode = get_expanded_call_opcode(chbuf, (int)(content_length - address), 0);
  ...
  xtensa_insnbuf_to_chars(isa, insnbuf, chbuf, (int)(content_length - address));
...
static source_reloc *
find_source_reloc(source_reloc *src_relocs, int src_count, asection *sec,
		  Elf_Internal_Rela *irel)
{
  int i;
...
source_reloc_compare(const void *ap, const void *bp)
{
  ...
  if (a->r_rel.target_offset != b->r_rel.target_offset)
    return (int)(a->r_rel.target_offset - b->r_rel.target_offset);
...
{
  return (unsigned int)((val >> 2U) + (val >> 10U));
}
...
fix_compare(const void *ap, const void *bp)
{
  ...
  if (a->src_offset != b->src_offset)
    return (int)(a->src_offset - b->src_offset);
...
bfd_boolean
is_resolvable_asm_expansion(bfd *abfd, asection *sec, bfd_byte *contents,
			    Elf_Internal_Rela *irel,
			    struct bfd_link_info *link_info,
...
/* The compute_text_actions function will build a list of potential
   ...
   pass.  */
bfd_boolean
compute_text_actions(bfd *abfd, asection *sec, struct bfd_link_info *link_info)
{
  xtensa_relax_info *relax_info;
  bfd_byte *contents;
  Elf_Internal_Rela *internal_relocs;
  ...
  for (i = 0; i < sec->reloc_count; i++)
	...
	{
	  text_action_add(&relax_info->action_list, ta_convert_longcall, sec,
    			  r_offset, 0);
	  continue;
	}
	 ...
	 unreachable section of size 0, then skip forward.  */
      ptbl_idx = (int)(the_entry - prop_table);
...
compute_ebb_proposed_actions (ebb_constraint *ebb_table)
{
  ...
  for (entry = start_entry; entry <= end_entry; entry++)
	  ...
	  && (entry->flags & XTENSA_PROP_INSN_BRANCH_TARGET) != 0)
	{
	  ...
	  if (check_branch_target_aligned_address(offset, (int)insn_len))
...
compute_fill_extra_space (property_table_entry *entry)
{
  ...
  fill_extra_space = (int)entry->size;
		    ...
		    bfd_vma offset)
{
  ...
  i = (unsigned int)(irel - internal_relocs);
  while (i > 0)
    {
...
/* */
bfd_boolean
is_removable_literal(const source_reloc *rel, int i,
                     const source_reloc *src_relocs, int src_count)
{
...
static bfd_boolean
coalesce_shared_literal(asection *sec, source_reloc *rel,
                        property_table_entry *prop_table,
                        int ptblsize, value_map *val_map)
{
  ...
  text_action_add(&relax_info->action_list, ta_remove_literal, sec,
   	          rel->r_rel.target_offset, 4);
...
static bfd_boolean
move_shared_literal(asection *sec, struct bfd_link_info *link_info,
		    source_reloc *rel, property_table_entry *prop_table,
		    int ptblsize, const r_reloc *target_loc,
...
relax_section(bfd *abfd, asection *sec, struct bfd_link_info *link_info)
{
  ...
  internal_relocs = retrieve_internal_relocs(abfd, sec, link_info->keep_memory);
  contents = retrieve_contents(abfd, sec, link_info->keep_memory);
  if (contents == NULL && sec_size != 0)
    {
      ok = FALSE;
  ...
  if ((relax_info->is_relaxable_literal_section
	   ...
	   action = action->next)
	{
	  ...
	  if ((action->offset > orig_dot) && (contents != NULL))
	    {
	  ...
	  if (action->offset == orig_dot)
		...
		{
		  if ((orig_dot_vo == 0) && (contents != NULL))
		    {
			       ...
			       Elf_Internal_Rela *rel)
{
  ...
  if ((r_type == R_XTENSA_32 || r_type == R_XTENSA_PLT)
      ...
      if (is_plt)
	{
	  ...
	  reloc_index = (int)(srel->size / sizeof(Elf32_External_Rela));
	      ...
	      const literal_value *lit)
{
  ...
  if (r_reloc_is_const(r_rel))
	  ...
	  || ((sec->reloc_count + 1) > relax_info->allocated_relocs_count))
	{
	  ...
	  relax_info->allocated_relocs_count = (unsigned int)new_relocs_count;
			...
			struct bfd_link_info *link_info)
{
  ...
  if (internal_relocs && (!link_info->relocatable
      ...
      for (offset = 0; offset < section_size; offset += entry_size)
	{
	  ...
	  if (is_full_prop_section)
	    flags = (flagword)bfd_get_32(abfd, &contents[actual_offset + 8]);
	  else
	    flags = predef_flags;
		  ...
		  if (last_irel)
			...
			(irel->r_addend
			 + bfd_get_32(abfd, &contents[actual_offset]));
		      if (is_full_prop_section)
			old_flags =
      			  (flagword)bfd_get_32(abfd,
                                               &contents[last_irel->r_offset
                                                         + 8]);
		      else
...
relax_section_symbols(bfd *abfd, asection *sec)
{
  ...
  Elf_Internal_Shdr *symtab_hdr;
  Elf_Internal_Sym *isymbuf;
  unsigned int i, num_syms, num_locals;
  ...
  num_syms = ((unsigned int)symtab_hdr->sh_size / sizeof(Elf32_External_Sym));
  num_locals = (unsigned int)symtab_hdr->sh_info;
  ...
  for (i = 0; i < num_locals; i++)
	...
	{
	  bfd_vma new_address =
            offset_with_removed_text(&relax_info->action_list, isym->st_value);
	  bfd_vma new_size = isym->st_size;

	  if (ELF32_ST_TYPE(isym->st_info) == STT_FUNC)
	    {
	      bfd_vma new_end =
        	offset_with_removed_text(&relax_info->action_list,
          				 (isym->st_value + isym->st_size));
...
/* */
static bfd_boolean
pcrel_reloc_fits(xtensa_opcode opc, int opnd,
		 bfd_vma self_address,
		 bfd_vma dest_address)
{
  xtensa_isa isa = xtensa_default_isa;
  uint32 valp = (uint32)dest_address;
  if (xtensa_operand_do_reloc(isa, opc, opnd, &valp, (uint32)self_address)
...
bfd_boolean
xtensa_callback_required_dependence(bfd *abfd, asection *sec,
				    struct bfd_link_info *link_info,
				    deps_callback_t callback, void *closure)
{
  ...
  if (((sec->flags & SEC_LINKER_CREATED) != 0)
      ...
      else
	{
	  ...
	  snprintf(got_name, sizeof(got_name), ".got.plt.%lu", chunk);
