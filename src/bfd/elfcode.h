...
/* Problems and other issues to resolve.
   ...
   (4)  We need a published spec for 64-bit ELF.  We've got some stuff here
	that we are using for SPARC V9 64-bit chips, but do NOT assume that
	it is cast in stone.
 */
...
/* Renaming structs, typedefs, macros and functions to be size-specific: */
...
#if !defined(ARCH_SIZE)
# if defined(__GNUC__) && !defined(__STRICT_ANSI__)
#  warning "ARCH_SIZE should be defined"
...
/* Translate an ELF symbol in external format into an ELF symbol in
 * internal format: */
void
elf_swap_symbol_in(bfd *abfd, const void *psrc, const void *pshn,
                   Elf_Internal_Sym *dst)
{
  ...
  if (signed_vma)
    dst->st_value = (bfd_vma)H_GET_SIGNED_WORD(abfd, src->st_value);
  else
    dst->st_value = H_GET_WORD(abfd, src->st_value);
  dst->st_size = H_GET_WORD(abfd, src->st_size);
  dst->st_info = H_GET_8(abfd, src->st_info);
  dst->st_other = H_GET_8(abfd, src->st_other);
  dst->st_shndx = (unsigned int)H_GET_16(abfd, src->st_shndx);
  if (dst->st_shndx == SHN_XINDEX)
    {
      if (shndx == NULL)
	abort();
      dst->st_shndx = (unsigned int)H_GET_32(abfd, shndx->est_shndx);
    }
}
...
static void
elf_swap_ehdr_in(bfd *abfd, const Elf_External_Ehdr *src,
                 Elf_Internal_Ehdr *dst)
{
  int signed_vma = get_elf_backend_data(abfd)->sign_extend_vma;
  memcpy(dst->e_ident, src->e_ident, EI_NIDENT);
  dst->e_type = (unsigned short)H_GET_16(abfd, src->e_type);
  dst->e_machine = (unsigned short)H_GET_16(abfd, src->e_machine);
  dst->e_version = (unsigned long)H_GET_32(abfd, src->e_version);
  if (signed_vma)
    dst->e_entry = (bfd_vma)H_GET_SIGNED_WORD(abfd, src->e_entry);
  else
  ...
  dst->e_shentsize = (unsigned int)H_GET_16(abfd, src->e_shentsize);
  dst->e_shnum = (unsigned int)H_GET_16(abfd, src->e_shnum);
  dst->e_shstrndx = (unsigned int)H_GET_16(abfd, src->e_shstrndx);
}
...
static void
elf_swap_shdr_in(bfd *abfd, const Elf_External_Shdr *src,
                 Elf_Internal_Shdr *dst)
{
  int signed_vma = get_elf_backend_data(abfd)->sign_extend_vma;

  dst->sh_name = (unsigned int)H_GET_32(abfd, src->sh_name);
  dst->sh_type = (unsigned int)H_GET_32(abfd, src->sh_type);
  dst->sh_flags = H_GET_WORD(abfd, src->sh_flags);
  if (signed_vma)
    dst->sh_addr = (bfd_vma)H_GET_SIGNED_WORD(abfd, src->sh_addr);
  else
    dst->sh_addr = H_GET_WORD(abfd, src->sh_addr);
  dst->sh_offset = (file_ptr)H_GET_WORD(abfd, src->sh_offset);
...
static void
elf_swap_shdr_out(bfd *abfd, const Elf_Internal_Shdr *src,
		  Elf_External_Shdr *dst)
{
  ...
  H_PUT_WORD(abfd, (bfd_uint64_t)src->sh_offset, dst->sh_offset);
		 ...
		 Elf_Internal_Phdr *dst)
{
  ...
  if (signed_vma)
    {
      dst->p_vaddr = (bfd_vma)H_GET_SIGNED_WORD(abfd, src->p_vaddr);
      dst->p_paddr = (bfd_vma)H_GET_SIGNED_WORD(abfd, src->p_paddr);
    }
  else
    {
...
void
elf_swap_reloc_in(bfd *abfd, const bfd_byte *s, Elf_Internal_Rela *dst)
{
...
/* likewise: */
void
elf_swap_reloca_in(bfd *abfd, const bfd_byte *s, Elf_Internal_Rela *dst)
{
  const Elf_External_Rela *src = (const Elf_External_Rela *)s;
  dst->r_offset = H_GET_WORD(abfd, src->r_offset);
  dst->r_info = H_GET_WORD(abfd, src->r_info);
  dst->r_addend = (bfd_vma)H_GET_SIGNED_WORD(abfd, src->r_addend);
}

/* Translate an ELF reloc from internal format to external format: */
void
elf_swap_reloc_out(bfd *abfd, const Elf_Internal_Rela *src, bfd_byte *d)
{
...
/* likewise: */
void
elf_swap_reloca_out(bfd *abfd, const Elf_Internal_Rela *src, bfd_byte *d)
{
...
elf_object_p(bfd *abfd)
{
  ...
  /* If this is a relocatable file and there is no section header table,
   * then we are hosed: */
  ...
  if (! bfd_default_set_arch_mach(abfd, ebd->arch, 0))
    {
      /* It is OK if this fails for the generic target: */
      if (ebd->elf_machine_code != EM_NONE)
	goto got_no_match;
    }
  ...
  if (i_ehdrp->e_shoff != 0)
    {
      bfd_signed_vma where = (bfd_signed_vma)i_ehdrp->e_shoff;
	...
	{
	  i_ehdrp->e_shnum = (unsigned int)i_shdr.sh_size;
	...
	{
	  i_ehdrp->e_shstrndx = (unsigned int)i_shdr.sh_link;
      ...
      if (i_ehdrp->e_shnum != 1)
	{
	  ...
	  where = (bfd_signed_vma)(i_ehdrp->e_shoff + sizeof(x_shdr));
	  if (bfd_seek(abfd, (file_ptr)where, SEEK_SET) != 0)
	    goto got_no_match;
	}
    }
  ...
  if (i_ehdrp->e_shnum != 0)
      ...
      for (shindex = 1; shindex < i_ehdrp->e_shnum; shindex++)
	{
	  ...
	  if ((i_shdrp[shindex].sh_size != 0)
	      ...
	      && (((i_shdrp[shindex].sh_addr
                    - (bfd_vma)i_shdrp[shindex].sh_offset)
		   % ebd->minpagesize)
		  != 0))
  ...
  for (s = abfd->sections; s != NULL; s = s->next)
	  ...
	  && (elf_section_data(s)->this_hdr.sh_info > 0))
	{
	  ...
	  targ_sec = bfd_section_from_elf_index(abfd,
                                                (unsigned int)targ_index);
	  if ((targ_sec != NULL)
  ...
  for (idx = 0; idx < sec->reloc_count; idx++, dst_rela += extsize)
	  ...
	  && ! _bfd_elf_validate_reloc(abfd, ptr))
	{
	  *failedp = TRUE;
      ...
      src_rela.r_info = (bfd_vma)ELF_R_INFO((unsigned int)n, ptr->howto->type);
...
int
elf_write_out_phdrs(bfd *abfd, const Elf_Internal_Phdr *phdr,
                    unsigned int count)
{
  while (count--)
    {
...
elf_write_shdrs_and_ehdr(bfd *abfd)
{
  ...
  /* at this point we have concocted all the ELF sections...  */
  amt = i_ehdrp->e_shnum;
  amt *= sizeof(*x_shdrp);
...
elf_slurp_symbol_table(bfd *abfd, asymbol **symptrs, bfd_boolean dynamic)
{
  ...
  symcount = (unsigned long)(sym - symbase);

  /* Fill in the user's symbol pointer vector if needed: */
  if (symptrs)
    {
      long l = (long)symcount;
                                   ...
                                   bfd_boolean dynamic)
{
  ...
  entsize = (int)rel_hdr->sh_entsize;
...
bfd_boolean
elf_slurp_reloc_table(bfd *abfd, asection *asect, asymbol **symbols,
                      bfd_boolean dynamic)
{
  ...
  if (!elf_slurp_reloc_table_from_section(abfd, asect, rel_hdr,
                                          reloc_count, relents, symbols,
                                          dynamic))
    return FALSE;

  if (rel_hdr2
      && !elf_slurp_reloc_table_from_section(abfd, asect, rel_hdr2,
                                             reloc_count2,
                                             (relents + reloc_count),
                                             symbols, dynamic))
    return FALSE;
...
elf_debug_section(int num, Elf_Internal_Shdr *hdr)
{
  ...
  fprintf(stderr,
	  ...
	  (long)hdr->sh_name, (long)hdr->sh_type, (long)hdr->sh_flags);
  fprintf(stderr,
	  "sh_addr      = %ld\tsh_offset    = %ld\tsh_size      = %ld\n",
	  (long)hdr->sh_addr, (long)hdr->sh_offset, (long)hdr->sh_size);
  fprintf(stderr,
	  "sh_link      = %ld\tsh_info      = %ld\tsh_addralign = %ld\n",
	  (long)hdr->sh_link, (long)hdr->sh_info, (long)hdr->sh_addralign);
   ...
   int (*target_read_memory)(bfd_vma, bfd_byte *, int))
{
  ...
  x_phdrs = (Elf_External_Phdr *)bfd_malloc(i_ehdr.e_phnum * (sizeof(*x_phdrs) + sizeof(*i_phdrs)));
  if (x_phdrs == NULL)
    {
  ...
  if (last_phdr == NULL)
    {
      /* There were no PT_LOAD segments, so we have nothing to read: */
      free(x_phdrs);
      bfd_set_error(bfd_error_wrong_format);
      return NULL;
    }
  ...
  if (((bfd_vma)contents_size > (last_phdr->p_offset + last_phdr->p_filesz))
    ...
    {
      contents_size = (int)(last_phdr->p_offset + last_phdr->p_filesz);
      if ((bfd_vma)contents_size < (i_ehdr.e_shoff
                                    + (i_ehdr.e_shnum * i_ehdr.e_shentsize)))
	contents_size = (int)(i_ehdr.e_shoff
                              + (i_ehdr.e_shnum * i_ehdr.e_shentsize));
    }
  else
    contents_size = (int)(last_phdr->p_offset + last_phdr->p_filesz);
  ...
  contents = (bfd_byte *)bfd_zmalloc((bfd_size_type)contents_size);
  if (contents == NULL)
    {
      free(x_phdrs);
      bfd_set_error(bfd_error_no_memory);
  ...
  bim->size = (bfd_size_type)contents_size;
  bim->buffer = contents;
  nbfd->iostream = bim;
...
#else
# if defined(__GNUC__) && !defined(__STRICT_ANSI__)
#  warning "elfcode.h already included once."
# endif /* __GNUC__ && !__STRICT_ANSI__ */
#endif /* !ELFCODE_H */
