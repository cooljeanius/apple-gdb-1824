/* elf64-ppc.c: PowerPC64-specific support for 64-bit ELF.
			  ...
			  bfd *output_bfd, const char **error_message)
{
  ...
  if (error_message != NULL)
    {
      static char buf[60];
      sprintf(buf, "generic linker cannot handle %s",
...
{
  if ((sec != NULL) && (ppc64_elf_section_data(sec) != NULL)
...
/* Create synthetic symbols, effectively restoring "dot-symbol" function
   entry syms.  */
static long
ppc64_elf_get_synthetic_symtab(bfd *abfd, long static_count,
                               asymbol **static_syms, long dyn_count,
                               asymbol **dyn_syms, asymbol **ret)
{
  asymbol *s;
  long i;
  long count;
...
ppc_get_stub_entry(const asection *input_section, const asection *sym_sec,
		   struct ppc_link_hash_entry *h,
		   const Elf_Internal_Rela *rel,
		   struct ppc_link_hash_table *htab)
{
...
static struct ppc_stub_hash_entry *
ppc_add_stub(const char *stub_name, asection *section,
	     struct ppc_link_hash_table *htab)
{
  asection *link_sec;
  asection *stub_sec;
...
ppc64_elf_add_symbol_hook(bfd *ibfd,
			  ...
			  Elf_Internal_Sym *isym, const char **name,
...
ppc64_elf_archive_symbol_lookup(bfd *abfd, struct bfd_link_info *info,
                                const char *name)
{
  ...
  len = strlen(name);
  dot_name = (char *)bfd_alloc(abfd, (len + 2UL));
  if (dot_name == NULL)
...
add_symbol_adjust(struct elf_link_hash_entry *h, void *inf)
{
  ...
  data = (struct add_symbol_adjust_data *)inf;
                       ...
                       asection *sec, const Elf_Internal_Rela *relocs)
{
  ...
  if (strcmp(bfd_get_section_name(abfd, sec), ".opd") == 0)
	...
	long entry_adjust;
      ...
      amt = (sec->size * sizeof(union opd_info) / 8UL);
  ...
  for (rel = relocs; rel < rel_end; rel++)
	  ...
	  if (ppc64_elf_section_data(sec)->t_symndx == NULL)
	    {
	      ...
	      bfd_size_type amt = (sec->size * sizeof(unsigned) / 8U + 1U);
	      ppc64_elf_section_data(sec)->t_symndx =
                (unsigned int *)bfd_zalloc(abfd, amt);
	      if (ppc64_elf_section_data(sec)->t_symndx == NULL)
		return FALSE;
	    }
	...
	case R_PPC64_ADDR64:
	  if ((opd_sym_map != NULL) && ((rel + 1) < rel_end)
	...
	dodyn:
	  if ((info->shared
	      ...
	      else
		{
	      ...
	      if ((p == NULL) || (p->sec != sec))
		{
		  p = (struct ppc_dyn_relocs *)bfd_alloc(htab->elf.dynobj,
                                                         sizeof(*p));
		  if (p == NULL)
		    return FALSE;
		  p->next = *head;
...
static bfd_vma
opd_entry_value(asection *opd_sec, bfd_vma offset, asection **code_sec,
                bfd_vma *code_off)
{
  ...
  if (opd_sec->reloc_count == 0)
    {
      ...
      if (!bfd_get_section_contents(opd_bfd, opd_sec, &val1, offset, 8))
	return (bfd_vma)(-1L);
	  ...
	  if ((ELF64_R_TYPE(look->r_info) == R_PPC64_ADDR64)
	      && (ELF64_R_TYPE((look + 1)->r_info) == R_PPC64_TOC))
	    {
	      ...
	      asection *sec;
              ...
              symndx = (unsigned long)ELF64_R_SYM(look->r_info);
		...
		*code_sec = sec;
		...
		val0 += (sec->output_section->vma + sec->output_offset);
	    }
	  break;
	}
    }
  ...
  for (i = parm->lo; i <= parm->hi; i++)
	  ...
	  && !h->def_regular)
	{
	  ...
	  if (htab->sfpr->contents == NULL)
	    {
	      htab->sfpr->contents =
                (unsigned char *)bfd_alloc(htab->elf.dynobj, SFPR_MAX);
	      if (htab->sfpr->contents == NULL)
		return FALSE;
	    }
	}
...
func_desc_adjust(struct elf_link_hash_entry *h, void *inf)
{
  ...
  if ((fdh != NULL) && !fdh->elf.forced_local
      && (info->shared || fdh->elf.def_dynamic || fdh->elf.ref_dynamic
	  || (fdh->elf.root.type == bfd_link_hash_undefweak
  ...
  force_local = (!fh->elf.def_regular || (fdh == NULL)
...
get_tls_mask(char **tls_maskp, unsigned long *toc_symndx,
             Elf_Internal_Sym **locsymsp, const Elf_Internal_Rela *rel,
             bfd *ibfd)
{
  unsigned long r_symndx;
  int next_r;
  ...
  if (((*tls_maskp != NULL) && (**tls_maskp != 0)) || (sec == NULL)
...
adjust_opd_syms(struct elf_link_hash_entry *h, void *inf ATTRIBUTE_UNUSED)
{
  ...
  opd_adjust = (long *)get_opd_info(sym_sec);
  if (opd_adjust != NULL)
    {
...
static bfd_boolean
dec_dynrel_count(bfd_vma r_info, asection *sec, struct bfd_link_info *info,
                 ...
                 struct elf_link_hash_entry *h, asection *sym_sec)
{
  ...
  r_type = (enum elf_ppc64_reloc_type)ELF64_R_TYPE(r_info);
  switch (r_type)
    {
    default:
      return TRUE;
  ...
  if (local_syms != NULL)
    {
      ...
      r_symndx = (unsigned long)ELF64_R_SYM(r_info);
		   ...
		   bfd_boolean non_overlapping)
{
  ...
  for (ibfd = info->input_bfds; ibfd != NULL; ibfd = ibfd->link_next)
    {
      ...
      bfd_size_type cnt_16b = 0UL;

      sec = bfd_get_section_by_name(ibfd, ".opd");
      if ((sec == NULL) || (sec->size == 0))
	continue;

      amt = (sec->size * sizeof(long) / 8UL);
      opd_adjust = (long *)get_opd_info(sec);
      if (opd_adjust == NULL)
	{
	  /* check_relocs has NOT been called.  Must be a ld -r link
	     or --just-symbols object.   */
	  opd_adjust = (long *)bfd_alloc(obfd, amt);
	  if (opd_adjust == NULL)
	    return FALSE;
	  ppc64_elf_section_data(sec)->opd.adjust = opd_adjust;
	}
      memset(opd_adjust, 0, (size_t)amt);
      ...
      for (rel = relstart; rel < relend; )
	{
	  ...
	  /* .opd contains a regular array of 16 or 24 byte entries.  We're
	     ...
	     point.  */
	  if ((rel->r_offset != offset) || ((rel + 1) >= relend)
      ...
      if (need_edit || add_aux_fields)
	{
	  ...
	  if (add_aux_fields)
	    {
	      new_contents = (bfd_byte *)bfd_malloc(sec->size
                                                    + (cnt_16b * 8UL));
	  ...
	  sec->reloc_count = (unsigned)(write_rel - relstart);
	  if (add_aux_fields)
	    {
	      free (sec->contents);
  ...
  if ((need_pad != NULL) && !info->relocatable)
      ...
      if ((need_pad->flags & SEC_IN_MEMORY) == 0)
	{
	  ...
	  p = (bfd_byte *)bfd_malloc(need_pad->size + 8);
	  if (p == NULL)
	    return FALSE;
	...
	{
	  p = (bfd_byte *)bfd_realloc(need_pad->contents,
                                      (need_pad->size + 8UL));
	  if (p == NULL)
	    return FALSE;
      ...
      memset(need_pad->contents + need_pad->size, 0, (size_t)8UL);
      need_pad->size += 8;
    }
		...
		if ((h == NULL) || !h->def_dynamic)
		  {
		    ...
		    ok_tprel = ((value + TP_OFFSET + ((bfd_vma)1UL << 31))
				< ((bfd_vma)1UL << 32));
		  }
  ...
  for (ibfd = info->input_bfds; ibfd != NULL; ibfd = ibfd->link_next)
      ...
      for (sec = ibfd->sections; sec != NULL; sec = sec->next)
	{
	  if ((sec->reloc_count == 0) || !elf_discarded_section(sec)
	      || get_opd_info(sec) || ((sec->flags & SEC_ALLOC) == 0)
	  ...
	  for (rel = relstart; rel < (relstart + sec->reloc_count); ++rel)
		...
		{
		  skip = ((unsigned long *)
                          bfd_zmalloc((sizeof(*skip) * (toc->size + 7UL))
                                      / 8UL));
		  if (skip == NULL)
		    goto error_ret;
		}
	...
	continue;
      ...
      used = ((unsigned char *)
              bfd_zmalloc(sizeof(*used) * (toc->size + 7UL) / 8UL));
      if (used == NULL)
	{
	error_ret:
	  if ((local_syms != NULL)
      ...
      for (sec = ibfd->sections;
	   ...
	   sec = ((sec == toc) ? NULL
		  : (((sec->next == toc) && sec->next->next)
                     ? sec->next->next : ((sec->next == NULL)
                                          ? toc : sec->next))))
	{
	  int repeat;
      ...
      if (some_unused)
	{
	  ...
	  if (toc->reloc_count != 0)
		...
		if (skip[rel->r_offset >> 3] != (unsigned long) -1)
	      ...
	      toc->reloc_count = (unsigned)(wrel - relstart);
  ...
  for (ibfd = info->input_bfds; ibfd != NULL; ibfd = ibfd->link_next)
      ...
      if ((s != NULL) && (s != htab->got))
	{
	  if (s->size == 0)
	    ...
	    {
	      s->contents = (unsigned char *)bfd_zalloc(ibfd, s->size);
	      if (s->contents == NULL)
		return FALSE;
	    }
	}
...
ppc_type_of_stub(asection *input_sec, const Elf_Internal_Rela *rel,
		 struct ppc_link_hash_entry **hash, bfd_vma destination)
{
  ...
  location = (input_sec->output_offset + input_sec->output_section->vma
	      + rel->r_offset);
...
ppc_build_one_stub(struct bfd_hash_entry *gen_entry, void *in_arg)
{
  ...
  switch (stub_entry->stub_type)
      ...
      if (info->emitrelocations)
	{
	  ...
	  if (stub_entry->h != NULL)
	      ...
	      if (hashes == NULL)
		{
		  ...
		  hsize = ((htab->stub_globals + 1UL) * sizeof(*hashes));
		  hashes = ((struct elf_link_hash_entry **)
                            bfd_zalloc(htab->stub_bfd, hsize));
		  if (hashes == NULL)
		    return FALSE;
    ...
    case ppc_stub_plt_branch_r2off:
      ...
      if (br_entry == NULL)
	{
	  (*_bfd_error_handler)(_("cannot find branch stub `%s'"),
                                stub_entry->root.string + 9);
	...
	abort();
      ...
      off &= ~(bfd_vma)1UL;
      off += (htab->plt->output_offset + htab->plt->output_section->vma
      ...
      if (((off + 0x80008000) > 0xffffffff) || ((off & 7) != 0))
	{
      ...
      p = build_plt_stub(htab->stub_bfd, loc, (int)off);
      size = (int)(p - loc);
      break;
...
ppc_size_one_stub(struct bfd_hash_entry *gen_entry, void *in_arg)
{
  ...
  if (stub_entry->stub_type == ppc_stub_plt_call)
      ...
      if ((off + (1 << 25)) >= (bfd_vma)(1UL << 26))
	{
	  ...
	  if (br_entry == NULL)
	    {
	      (*_bfd_error_handler)(_("cannot build branch stub `%s'"),
				    stub_entry->root.string + 9);
	      htab->stub_error = TRUE;
	      return FALSE;
	    }
	  ...
	  if (br_entry->iter != htab->stub_iteration)
	    {
	      ...
	      br_entry->offset = (unsigned int)htab->brlt->size;
	      htab->brlt->size += 8;
			      ...
			      int no_multi_toc)
{
  ...
  amt = (sizeof(asection *) * (top_index + 1UL));
  ...
  for (rel = relstart; rel < (relstart + isec->reloc_count); ++rel)
    {
      ...
      r_type = (enum elf_ppc64_reloc_type)ELF64_R_TYPE(rel->r_info);
      if ((r_type != R_PPC64_REL24) && (r_type != R_PPC64_REL14)
	  && (r_type != R_PPC64_REL14_BRTAKEN)
	   ...
	   input_bfd = input_bfd->link_next, bfd_indx++)
	{
	  ...
	  for (section = input_bfd->sections;
	      ...
	      for (; irela < irelaend; irela++)
		{
		  ...
		  r_type =
                    (enum elf_ppc64_reloc_type)ELF64_R_TYPE(irela->r_info);
		  r_indx = (unsigned int)ELF64_R_SYM(irela->r_info);
...
static bfd_boolean
ppc64_elf_relocate_section(bfd *output_bfd, struct bfd_link_info *info,
  ...
  for (; rel < relend; rel++)
    {
	 ...
	 proper TOC base to use.  */
      if ((rel->r_info == ELF64_R_INFO(0, R_PPC64_TOC)) && (rel != relocs)
	  && (ELF64_R_TYPE(rel[-1].r_info) == R_PPC64_ADDR64) && is_opd)
	r_symndx = (unsigned long)ELF64_R_SYM(rel[-1].r_info);
	...
	case R_PPC64_GOT_TLSLD16_LO:
	  if ((tls_mask != 0) && ((tls_mask & TLS_LD) == 0))
	      ...
	      if ((rel + 1) < relend)
		{
		  ...
		  r_type2 =
                    (enum elf_ppc64_reloc_type)ELF64_R_TYPE(rel[1].r_info);
		  r_symndx2 = (unsigned long)ELF64_R_SYM(rel[1].r_info);
	...
	case R_PPC64_REL24:
	  ...
	  if ((((h != NULL)
		&& ((((fdh = h->oh) != NULL)
	       ...
	       || ((sec != NULL) && (sec->output_section != NULL)
		   && (sec->id <= htab->top_id)
	...
	dogot:
	  {
		...
		*offp = off | 1;
		if ((info->shared || indx != 0)
			...
			|| h->elf.root.type != bfd_link_hash_undefweak))
		  {
		    outrel0.r_offset = (got->output_section->vma
                                        + got->output_offset + off);
		    outrel0.r_addend = rel->r_addend;
		    if (tls_type & (TLS_LD | TLS_GD))
		      {
			outrel0.r_addend = 0;
			outrel0.r_info = ELF64_R_INFO(indx,
                                                      R_PPC64_DTPMOD64);
			if (tls_type == (TLS_TLS | TLS_GD))
			  {
			    loc0 = relgot->contents;
			    loc0 += (relgot->reloc_count++
                                     * sizeof(Elf64_External_Rela));
			...
			/* Write the .got section contents for the sake
			   of prelink.  */
			loc0 = (got->contents + off);
			bfd_put_64(output_bfd,
                                   (outrel0.r_addend + relocation), loc0);
		      }
		    else
		      outrel0.r_info = ELF64_R_INFO(indx,
                                                    R_PPC64_GLOB_DAT);
		    ...
		    if (indx == 0 && tls_type != (TLS_TLS | TLS_LD))
		      {
			outrel0.r_addend += relocation;
			if (tls_type & (TLS_GD | TLS_DTPREL | TLS_TPREL))
			  outrel0.r_addend -= htab->elf.tls_sec->vma;
		      }
		    loc0 = relgot->contents;
		    loc0 += (relgot->reloc_count++
                             * sizeof(Elf64_External_Rela));
		    bfd_elf64_swap_reloca_out(output_bfd, &outrel0, loc0);
		  }
	...
	dodyn2:
	  ...
	  if ((info->shared
	      ...
	      || (ELIMINATE_COPY_RELOCS && !info->shared && (h != NULL)
		  && (h->elf.dynindx != -1) && !h->elf.non_got_ref
		  && h->elf.def_dynamic && !h->elf.def_regular))
	    {
	      Elf_Internal_Rela outrel1;
		...
		r_type ^= R_PPC64_ADDR16 ^ R_PPC64_UADDR16;
	      ...
	      if (skip)
		memset(&outrel1, 0, sizeof(outrel1));
	      else if (!SYMBOL_REFERENCES_LOCAL(info, &h->elf)
		       && !is_opd && (r_type != R_PPC64_TOC))
		outrel1.r_info = ELF64_R_INFO(h->elf.dynindx, r_type);
	      else
		{
		  /* This symbol is local, or marked to become local,
		     or this is an opd section reloc which must point
		     at a local function.  */
		  outrel1.r_addend += relocation;
		  if (r_type == R_PPC64_ADDR64 || r_type == R_PPC64_TOC)
		      ...
		      if (is_opd && h != NULL)
			{
		      ...
		      outrel1.r_info = ELF64_R_INFO(0, R_PPC64_RELATIVE);
		      ...
		      else
			{
			  ...
			  /* We are turning this relocation into one
			     ...
			     section.  */
			  outrel1.r_addend -= osec->vma;
			}

		      outrel1.r_info = ELF64_R_INFO(indx, r_type);
		    }
		}
		...
		abort();
	      ...
	      loc1 = sreloc->contents;
	      loc1 += sreloc->reloc_count++ * sizeof(Elf64_External_Rela);
	      bfd_elf64_swap_reloca_out(output_bfd, &outrel1, loc1);
	...
	case R_PPC64_DTPREL16_LO_DS:
	  insn = ((unsigned long)
                  bfd_get_32(input_bfd, contents + (rel->r_offset & ~3)));
	  mask = 3;
