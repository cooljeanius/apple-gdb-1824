...
MY(reloc_howto)(bfd *abfd, struct reloc_std_external *rel,
		int *r_index, int *r_extern, int *r_pcrel)
{
  unsigned int r_length;
  unsigned int r_pcrel_done;
  unsigned int r_neg;
  int local_index; /* needed to be renamed due to shadowing global decl */
  ...
  *r_pcrel = 0;
  ...
  return (MY(howto_table) + local_index);
}
