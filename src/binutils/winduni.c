/* winduni.c -- unicode support for the windres program.
   Copyright (C) 1997-1998, 2000-2001, 2003-2014 Free Software Foundation
...
#include "bucomm.h"
/* The above must have been included before <windows.h> and <winnls.h>,
...
static rc_uint_type wind_WideCharToMultiByte(rc_uint_type, const unichar *,
                                             char *, rc_uint_type);
static rc_uint_type wind_MultiByteToWideChar(rc_uint_type, const char *,
                                             unichar *, rc_uint_type);
...
static const wind_language_t languages[] =
{
  ...
  { 0x0401, 864, 1256, "Arabic", "Saudi Arabia" },
  { 0x0402, 866, 1251, "Bulgarian", "Bulgaria" },
  { 0x0403, 850, 1252, "Catalan", "Spain" },
  { 0x0404, 950,  950, "Chinese", "Taiwan" },
  ...
  { 0x0414, 850, 1252, "Norwegian (Bokm\345l)", "Norway" },
  ...
  { 0x3009, 437, 1252, "English", "Zimbabwe" },
  { 0x300A, 850, 1252, "Spanish", "Ecuador" },
  { 0x3401, 864, 1256, "Arabic", "Kuwait" },
  { 0x3409, 437, 1252, "English", "Philippines" },
...
/* Convert an ASCII string to a unicode string (for real now).  See the
 * previous comment for level of intelligence embodied in this function: */
void
unicode_from_ascii_old(int *length, unichar **unicode, const char *ascii)
{
  int len;
#ifndef _WIN32
  const char *s;
  ...
  *w = 0;
   ...
   * then this will set *length to -1 for us: */
  len--;
#endif /* !_WIN32 */
...
/* new version of the previous: */
void
unicode_from_ascii(rc_uint_type *length, unichar **unicode,
                   const char *ascii)
{
  unicode_from_codepage(length, unicode, ascii, wind_current_codepage);
}

/* Convert an ASCII string with length A_LENGTH to a unicode string.
 * We just copy it, expanding chars to shorts, rather than doing something
 * intelligent.  This routine also converts '\0' within a string: */
void
...
void
ascii_from_unicode(rc_uint_type *length, const unichar *unicode,
                   char **ascii)
{
...
codepage_from_langid(unsigned short langid)
{
  ...
  c = GetLocaleInfoA(MAKELCID(langid, SORT_DEFAULT),
                     LOCALE_IDEFAULTANSICODEPAGE, cp_string,
                     max_cp_string_len);
  /* If codepage data for an LCID is not installed on users's system,
   * then GetLocaleInfo will return an empty string.  Fall back to system
   * ANSI default: */
  if (c == 0)
    return CP_ACP;
...
wincodepage_from_langid(unsigned short langid)
{
   ...
   * but is unavailable on Win95.  */
  c = GetLocaleInfoA(MAKELCID(langid, SORT_DEFAULT),
                     LOCALE_IDEFAULTCODEPAGE, cp_string,
                     max_cp_string_len);
  /* If codepage data for an LCID is not installed on users's system,
   * then GetLocaleInfo will return an empty string.  Fall back to system
   * ANSI default: */
...
lang_from_langid(unsigned short langid)
{
  ...
  c = GetLocaleInfoA(MAKELCID(langid, SORT_DEFAULT), LOCALE_SENGLANGUAGE,
                     cp_string, 260);
  /* If codepage data for an LCID is not installed on users's system,
   * then GetLocaleInfo will return an empty string.  Fall back to system
   * ANSI default: */
...
country_from_langid(unsigned short langid)
{
  ...
  memset (cp_string, 0, 261);
  c = GetLocaleInfoA(MAKELCID(langid, SORT_DEFAULT), LOCALE_SENGCOUNTRY,
                     cp_string, 260);
  /* If codepage data for an LCID is not installed on users's system,
   * then GetLocaleInfo will return an empty string.  Fall back to system
   * ANSI default: */
...
void
unicode_from_codepage(rc_uint_type *length, unichar **u, const char *src,
                      rc_uint_type cp)
{
  rc_uint_type len;
  ...
  /* Discount the trailing '/0'.  If MultiByteToWideChar failed, then
   * this will set *length to -1 for us:  */
  len -= sizeof(unichar);
...
void
codepage_from_unicode(rc_uint_type *length, const unichar *unicode,
                      char **ascii, rc_uint_type cp)
{
  rc_uint_type len;
...
#endif /* HAVE_ICONV && !_WIN32 && !__CYGWIN__ */
                         ...
                         unichar *u, rc_uint_type u_len)
{
  ...
  ret = (rc_uint_type)MultiByteToWideChar(cp, conv_flags, mb, -1, u,
                                          u_len);
  /* Convert to bytes: */
  ret *= sizeof(unichar);
...
static rc_uint_type
wind_WideCharToMultiByte(rc_uint_type cp, const unichar *u, char *mb,
                         rc_uint_type mb_len)
{
  rc_uint_type ret = 0;
#if defined(_WIN32) || defined(__CYGWIN__)
