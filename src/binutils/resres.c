...
/* Write resource file: */
void
write_res_file(const char *fn, const struct res_directory *resdir)
{
  int language;
...
read_resource_entry(void)
{
  ...
  r = bin_to_res(type, (const unsigned char *)buff, reshdr.data_size, 0);
  r->res_info = resinfo;
...
static void
write_res_directory(const struct res_directory *rd,
                    ...
                    int *language, int level)
{
  ...
  for (re = rd->entries; re != NULL; re = re->next)
	...
	{
	case 1:
	  /* If we are at level 1, the key of this resource is the
	...
	case 2:
	  /* If we are at level 2, the key of this resource is the name
	     we are going to use in the rc printout.  */
	  name = &re->id;
	  break;

	case 3:
	  /* If we are at level 3, then this key represents a language.
...
static void
write_res_resource(const struct res_id *type, const struct res_id *name,
...
static void
write_res_bin(const struct res_resource *res,
              const struct res_id *type, const struct res_id *name,
...
static void
write_res_header(unsigned long datasize, const struct res_id *type,
...
static void
write_res_data(const void *data, size_t size, int count)
{
...
static void
read_res_data(void *data, size_t size, int count)
{
...
/* Write resource info: */
static void
write_res_info(const struct res_res_info *info)
{
...
read_res_id(struct res_id *id)
{
  ...
  if (ord == 0xFFFF)		/* an ordinal id */
	...
	fatal("%s: %s: failed to seek in file", program_name, filename);
...
/* Check if file is a win32 binary resource file; if so, then skip past
 * the null resource. Returns 0 if successful, or -1 on error: */
static void
...
void
res_add_resource(struct res_resource *r, const struct res_id *type,
                 const struct res_id *id, int language, int dupok)
{
  struct res_id a[3];
