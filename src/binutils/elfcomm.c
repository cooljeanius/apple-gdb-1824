...
error(const char *message, ...)
{
  ...
  /* Try to keep error messages in sync with the normal output of the
   * program: */
  fflush(stdout);
...
warn(const char *message, ...)
{
  ...
  /* Try to keep warning messages in sync with the normal output of the
   * program: */
  fflush(stdout);
		     ...
		     int name_len)
{
  ...
  if (IS_ABSOLUTE_PATH(name) || (base_name == file_name))
    {
      member_file_name = (char *)malloc((size_t)name_len + 1UL);
      if (member_file_name == NULL)
        {
          error(_("Out of memory\n"));
          return NULL;
        }
      memcpy(member_file_name, name, (size_t)name_len);
  ...
  else
    {
      ...
      size_t prefix_len = (size_t)(base_name - file_name);
      member_file_name = (char *)malloc(prefix_len + (size_t)name_len + 1);
      if (member_file_name == NULL)
        {
          error(_("Out of memory\n"));
          return NULL;
      ...
      memcpy((member_file_name + prefix_len), name, (size_t)name_len);
      member_file_name[prefix_len + (size_t)name_len] = '\0';
    }
				  ...
				  bfd_boolean read_symbols)
{
  ...
  if (! read_symbols)
    {
      if (fseek(arch->file, (long)size, SEEK_CUR) != 0)
	{
      ...
      if (size < (arch->index_num * sizeof_ar_index))
	{
      ...
      index_buffer = ((unsigned char *)
                      malloc((size_t)arch->index_num * sizeof_ar_index));
      if (index_buffer == NULL)
	{
      ...
      got = fread(index_buffer, sizeof_ar_index, (size_t)arch->index_num,
                  arch->file);
      if (got != arch->index_num)
	{
      ...
      size -= (unsigned long)(arch->index_num * sizeof_ar_index);

      /* Convert the index numbers into the host's numeric format: */
      arch->index_array =
        ((elf_vma *)
         malloc((size_t)arch->index_num * sizeof(* arch->index_array)));
                        ...
                        struct archive_info *nested_arch)
{
  ...
  /* The full ar_name field is used.  Do NOT rely on ar_date starting
     with a zero byte.  */
  {
    char *name = (char *)xmalloc(sizeof(arch->arhdr.ar_name) + 1UL);
		    ...
		    const char *member_name)
{
  ...
  if (arch->is_thin_archive
	...
	len += (strlen(nested_arch->file_name) + 2UL);
      else
