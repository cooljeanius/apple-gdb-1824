	...
	| DATE STRING STRING STRING
	  {
	    ...
	    free($2);
            if ($2 != $3) {
	      free($3); /* FIXME: potential double free? */
            }
            if (($2 != $4) && ($3 != $4)) {
	      free($4); /* FIXME: potential double free? */
            }
...
symbol:
	  STRING
	  {
	    if (symbol_prefix == NULL)
	      ...
	      {
                size_t symlen = (strlen(symbol_prefix) + strlen($1) + 4UL);
		$$ = (char *)xmalloc(symlen);
		snprintf($$, symlen, "%s@%s", symbol_prefix, $1);
		free($1);
	      }
	  }
	;
