...
add_specific_symbols(const char *filename, struct symlist **list)
{
  ...
  buffer = (char *)xmalloc((size_t)size + 2UL);
...
/* Choose which symbol entries to copy; put the result in OSYMS.
   We do NOT copy in place, because that confuses the relocs.
               ...
               asymbol **isyms, long symcount)
{
  ...
  long src_count = 0L, dst_count = 0L;
      ...
      if (add_leading_char || prefix_symbols_string)
        {
          ...
          ptr = n = (char *)xmalloc(1UL + strlen(prefix_symbols_string)
                                    + strlen(name) + 1UL);
          if (add_leading_char)
...
add_redefine_syms_file(const char *filename)
{
 ...
 comment:
      if ((len != 0UL) && ((outsym_off == 0UL) || (outsym_off == len)))
...
copy_unknown_object(bfd *ibfd, bfd *obfd)
{
  ...
  if (bfd_seek(ibfd, (file_ptr)0L, SEEK_SET) != 0)
    {
      bfd_nonfatal(bfd_get_archive_filename(ibfd));
      return FALSE;
    }
...
copy_object(bfd *ibfd, bfd *obfd)
{
  bfd_vma start;
  long symcount;
  asection **osections = (asection **)NULL;
  asection *gnu_debuglink_section = (asection *)NULL;
  bfd_size_type *gaps = (bfd_size_type *)NULL;
  bfd_size_type max_gap = (bfd_size_type)0UL;
  long symsize;
  void *dhandle;
  ...
  if (add_sections != NULL)
      ...
      for (padd = add_sections; padd != NULL; padd = padd->next)
	{
	  ...
	  if (padd->section == NULL)
	    {
	      non_fatal(_("cannot create section `%s': %s"),
	  ...
	  for (i = 0; i < (c - 1); i++)
	      ...
	      if (gap_start < gap_stop)
		{
		  if (! bfd_set_section_size(obfd, osections[i],
		    ...
		    {
		      non_fatal(_("Cannot fill gap after %s: %s"),
	  ...
	  if ((lma + size) < pad_to)
	    {
	      if (! bfd_set_section_size(obfd, osections[c - 1],
					 (pad_to - lma)))
		{
		  non_fatal(_("Cannot add padding to %s: %s"),
  ...
  osympp = isympp = (asymbol **)xmalloc((size_t)symsize);
  ...
  if ((strip_symbols == STRIP_DEBUG)
      ...
      || weaken)
    {
	 ...
	 have NOT been set yet.  mark_symbols_used_in_relocations will
	...
	bfd_map_over_sections(ibfd,
                              ...
                              isympp);
      osympp = ((asymbol **)
                xmalloc((size_t)(symcount + 1L) * sizeof(asymbol *)));
      symcount = (long)filter_symbols(ibfd, obfd, osympp, isympp,
                                      symcount);
    }
  ...
  bfd_set_symtab(obfd, osympp, (unsigned int)symcount);
  ...
  if (gap_fill_set || pad_to_set)
    {
      ...
      if (max_gap > 8192UL)
	max_gap = 8192UL;
      buf = (bfd_byte *)xmalloc((size_t)max_gap);
	  ...
	  if (gaps[i] != 0)
	    {
	      ...
	      left = gaps[i];
	      off = (file_ptr)(bfd_section_size(obfd, osections[i])
                               - left);
...
/* Read each archive element in turn from IBFD, copy the contents
 * to temp file, and keep the temp file handle: */
static void
  ...
  while (!status && (this_element != NULL))
    {
      ...
      struct stat buf;
      int stat_status = 0;
      bfd_boolean need_to_delete = TRUE;

      /* Create an output file for this member: */
      output_name = concat(dir, "/",
                           bfd_get_filename(this_element), (char *)0);

      /* If the file already exists, then make another temp dir: */
	...
	{
	  need_to_delete = ! copy_object(this_element, output_bfd);
	  ...
	  if (! need_to_delete
	      ...
	      if (!bfd_close(output_bfd))
		{
		  ...
		  /* Error in new object file. Do NOT change archive: */
		  status = 1;
		}
	    }
	  else
...
copy_unknown_element:
	  need_to_delete = !copy_unknown_object(this_element, output_bfd);
	  if (!bfd_close_all_done(output_bfd))
	    {
	      bfd_nonfatal(bfd_get_filename(output_bfd));
	      /* Error in new object file. Do NOT change the archive: */
	      status = 1;
	    }
	}
...
setup_section(bfd *ibfd, sec_ptr isection, void *obfdarg)
{
  ...
  if (prefix)
    {
      ...
      n = (char *)xmalloc(strlen(prefix) + strlen(name) + 1UL);
      strcpy(n, prefix);
      strcat(n, name);
      name = n;
    }
  ...
  if (copy_byte >= 0)
    size = ((size + (bfd_size_type)interleave - 1UL)
            / (bfd_size_type)interleave);
...
/* Copy the data of input section ISECTION of IBFD to an output section
 * with the same name in OBFD.  If stripping, then do NOT copy any
 * relocation info: */
static void
copy_section(bfd *ibfd, sec_ptr isection, void *obfdarg)
{
  ...
  /* If we have already failed earlier on, then do not keep on generating
   * complaints now: */
  if (status != 0)
    return;
  ...
  if (bfd_get_format(obfd) == bfd_core)
      ...
      if (relsize < 0L)
	{
	  ...
	  if ((relsize == -1L) && (bfd_get_error() == bfd_error_invalid_operation))
	    relsize = 0L;
	  else
	    RETURN_NONFATAL(bfd_get_filename(ibfd));
	}
  ...
  if (relsize == 0L)
      ...
      if (strip_symbols == STRIP_ALL)
	{
	  ...
	  arelent **temp_relpp;
	  long temp_relcount = 0L;
	  long i;

	  temp_relpp = (arelent **)xmalloc((size_t)relsize);
	  for (i = 0L; i < relcount; i++)
      ...
      bfd_set_reloc(obfd, osection, ((relcount == 0L) ? NULL : relpp),
  ...
  if ((bfd_get_section_flags(ibfd, isection) & SEC_HAS_CONTENTS)
    ...
    {
      void *memhunk = xmalloc((size_t)size);
      ...
      if (copy_byte >= 0)
	{
	  ...
	  size = ((size + (bfd_size_type)interleave - 1UL - (bfd_size_type)copy_byte)
                  / (bfd_size_type)interleave);
	  osection->lma /= (bfd_vma)interleave;
	}
  ...
  else if (p != NULL && p->set_flags && (p->flags & SEC_HAS_CONTENTS) != 0)
    {
      ...
      /* We do NOT permit the user to turn off the SEC_HAS_CONTENTS
...
/* Get all sections.  This is for when --gap-fill or --pad-to is used: */
static void
...
static void
mark_symbols_used_in_relocations(bfd *ibfd, sec_ptr isection,
                                 void *symbolsarg)
{
...
copy_main (int argc, char *argv[])
{
  ...
  while ((c = getopt_long (argc, argv, "b:B:i:I:j:K:N:s:O:d:F:L:G:R:SpgxXHhVvW:w",
	...
	case OPTION_ADD_SECTION:
	  {
	    ...
	    pa->filename = (s + 1);
	    pa->size = (size_t)size;
	    pa->contents = (bfd_byte *)xmalloc((size_t)size);
	...
	case OPTION_REDEFINE_SYM:
	  {
	    ...
	    len = (size_t)(s - optarg);
	    source = (char *)xmalloc(len + 1UL);
	    strncpy(source, optarg, len);
	    source[len] = '\0';

	    nextarg = (s + 1);
	    len = strlen(nextarg);
	    target = (char *)xmalloc(len + 1UL);
	...
	case OPTION_RENAME_SECTION:
	  {
	    ...
	    len = (unsigned int)(eq - optarg);
	    if (len == 0)
	      fatal(_("bad format for %s"), "--rename-section");

	    old_name = (char *)xmalloc(len + 1U);
	...
	case OPTION_SREC_LEN:
	  Chunk = (unsigned int)parse_vma(optarg, "--srec-len");
	  break;
	...
	case 0:
	  /* We have been given a long option: */
	  break;
...
#if defined(HAVE_DOS_BASED_FILE_SYSTEM) || defined(HAVE_EXEEXT)
