...
/*
   ----------------------------------------------------------------------------
   This file contains what used to be called the CFLoader interface. The name
   was changed to fit the newer convention of having CodeFragment as a common
   prefix, and to reduce pervasive confusion between the Code Fragment Manager
   and the Code Fragment Loaders, promulgated by the long history of the
   Segment Loader. This file defines the abstract interface to the physical
   representation of code fragments.
*/


/* !!! This version has minimal comments, the main purpose is to just get
 * things compiled.
 */
...
struct CFContSectionInfo {
  ...
  UInt8               alignment; /* ! The power of 2, aka number of low order zero bits. */
  UInt8               reservedA;
...
/*
   This is here to provide binary compatibility for code outside of CFM that
   uses the original container handler functions as documented in old versions
   of CodeFragmentsPriv.
*/
/* --------------------------------------------------------------------------*/
...
/*
   ----------------------------------------------------------------------------
   The ABI version is a pair of UInt16s used as simple counters. The high order
   part is the current version number, the low order part is the oldest
   compatible definition version number. This pair is to be used by the
   specific container handlers to describe what version of the container
   handler ABI they support.
    0x00010001
    ----------
