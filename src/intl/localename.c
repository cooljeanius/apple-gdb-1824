...
const char *_nl_locale_name(int category, const char *categoryname)
{
   ...
   * On some systems this can be done by the 'setlocale' func itself: */
...
# else
  ...
  if ((retval == NULL) || (retval[0] == '\0')) {
	  ...
	  retval = getenv("LANG");
	  if ((retval == NULL) || (retval[0] == '\0')) {
	    /* We use C as the default domain. POSIX says that this is
...
# endif /* HAVE_SETLOCALE && HAVE_LC_MESSAGES && HAVE_LOCALE_NULL */
  ...
  /* Return an XPG style locale name language[_territory][@modifier].
     Do NOT even bother determining the codeset; it is not useful in this
  ...
  switch (primary) {
    ...
    case LANG_ENGLISH:
      switch (sub) {
	/* SUBLANG_ENGLISH_US == SUBLANG_DEFAULT. As it should be.
         * Murica, F Yeah! */
