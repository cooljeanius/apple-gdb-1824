...
/* define alias resolution action rules mask */
    ...
    kARMSearchRelFirst = 0x00000400, /* search target on relative path first */
...
pascal OSErr NewAliasMinimal(const FSSpec *target,
                             AliasHandle *alias)
    = {0x7008,0xA823};
/* create a minimal new alias from a target fullpath
 * (optional zone and server name) and return alias record handle
 */
...
/* given an alias handle and fromFile, resolve the alias, update the alias
 * record and return aliased filename and wasChanged flag. */
...
/*
 * Low Level Routines
 * given an alias handle and fromFile, match the alias and return aliased
 * filename(s) and needsUpdate flag
 */
...
pascal OSErr MatchAlias(const FSSpec *fromFile,
                        ...
                        void *yourDataPtr)
    = {0x7005,0xA823};
/* given a fromFile-target pair and an alias handle, update the alias record
 * pointed to by alias handle to represent target as the new alias. */
