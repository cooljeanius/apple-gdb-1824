...
/* Another horrible header file full of arbitrary hex values... sigh... */
enum {
 ...
 MMUFlags = 0x0CB0,	/*[GLOBAL VAR] (byte) cleared to 0 (reserved for future use)*/
...
enum {
 EndSRTPtr = 0x0DB4, /*[GLOBAL VAR] (long) Pointer to end of Slot Resource Table (Not the SRT buffer).*/
...
enum {
 ...
 ScrVRes = 0x102, /*[GLOBAL VAR] Pix per inch vertically (word)
...
enum {
 ...
 MonkeyLives = 0x100, /*[GLOBAL VAR]  monkey lives (what?) if >= 0 [word]*/
...
enum {
 ScrDmpEnb = 0x2F8, /*[GLOBAL VAR] 0 if GetNextEvent should NOT process Command-Shift-number combos (byte)
...
enum {
 ...
 DragHook = 0x9F6, /*[GLOBAL VAR] Addr of proc to exec during TrackGoAway, DragWindow, GrowWindow, DragGrayRgn, TrackControl, & DragControl
