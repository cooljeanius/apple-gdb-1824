...
Created: Thursday, September 7, 1989 at 9:11 PM
	...
	RotateCursor(counter)	  - Sequence thru cursor frames for counter mod 32
 ...
 *	the System file.
 *
 *	If NewCursors is not NULL, it is ASSUMED to be a handle to an 'acur'
 *	formatted resource designated by the caller and it will be used instead of
 *	doing a GetResource on 'acur'. Note, if RotateCursor or SpinCursor are
 *	called without calling InitCursorCtl, then RotateCursor and SpinCursor will
 *	do the call for the user the first time it is called. However, the possible
 *	disadvantage of using this technique is that the resource memory allocated
 *	may have undesirable affect (fragmentation?) on the application. Using
 *	InitCursorCtl has the advantage of causing the allocation at a specific
 ...
 *	InitCursorCtl is called, the caller must guarantee NewCursors always points
 *	to a "fresh" copy of an 'acur' resource. This need only be of concern to a
 *	caller who wants to repeatly use multiple 'acur' resources during execution
 *	of their programs.
 */

pascal void RotateCursor(long counter);
/*
 *	RotateCursor is called to rotate the "I am active" "beach ball" cursor, or
 *	to animate whatever sequence of cursors set up by InitCursorCtl. The next
 *	cursor ("frame") is used when Counter % 32 = 0 (Counter is some kind of
 *	incrementing or decrementing index maintained by the caller). A positive
 *	counter sequences forward through the cursors (e.g., it rotates the
 *	"beach ball" cursor clockwise), and a negative cursor sequences through the
 *	cursors backwards (eg, it rotates the "beach ball" cursor counterclockwise).
 *	Note, RotateCursor just does a Mac SetCursor call for the proper cursor
 *	picture. It is assumed the cursor is visible from a prior Show_Cursor call.
 */
 ...
 *	done for the specified cursor prior to doing a ShowCursor. HIDDEN_CURSOR
 *	just causes a ShowCursor call. Note, ARROW_CURSOR will only work correctly
 *	if there is already a grafPort set up pointed to by 0(A5).
 */
#ifdef __cplusplus
}
#endif /* __cplusplus */
