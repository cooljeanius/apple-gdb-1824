...
# makes configure think it is cross compiling. If --target was omitted, then
# we cannot configure, so something is wrong.
AC_DEFUN([CY_AC_C_CROSS],
[# If we cannot run a trivial program, we must be cross compiling.
AC_MSG_CHECKING(whether cross-compiling)
AC_CACHE_VAL([ac_cv_c_cross],[
AC_RUN_IFELSE([AC_LANG_SOURCE([[
  int main(void){return(0);}]])],[ac_cv_c_cross=no],[ac_cv_c_cross=yes],[ac_cv_c_cross=yes])
])
...
],[AC_MSG_RESULT([this cannot happen])
],[AC_MSG_ERROR([cannot be cross compiled])
])
...
# see if one is installed
if test x"${ac_cv_c_tkh}" = x""; then
  AC_CHECK_HEADER([tk.h],[ac_cv_c_tkh=installed])
fi
])
if test x"${ac_cv_c_tkh}" != x ; then
  no_tk=""
...
include or libraries - report this to your system admin) ],[AC_MSG_ERROR(cannot be cross compiled)
])
CPPFLAGS="${orig_includes}"
fi
