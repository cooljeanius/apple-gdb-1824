 ...
 * what tcgetattr returns is inherently not robust, plus it does NOT
 ...
 * which invokes readline completion. We do NOT want this. I do NOT know
 ...
 *
 * Asynchronous output while we are editing an input line should be
...
/* Unfortunately, we cannot safely display echo from the inferior process.
   ...
   we are actually editing. Then we send the line to the inferior, and the
...
int get_slave_pty(char *name) {
   ...
   if ((gptr = getgrnam("tty")) != 0) {
      ...
      /* if the tty group does not exist, do NOT change the
...
main(int argc, char** argv)
{
  ...
  if (child == 0)
      ...
      if (slave > 2)
	{
      ...
      /* Try to restore window size; failure is NOT critical */
  ...
  for (;;)
      ...
      if (FD_ISSET (in_from_tty_fd, &in_set))
	{
	  ...
	  /* If we cannot get tty modes for the master side of the pty, we
	     cannot handle non-canonical-mode programs. Always assume the
	     master is in canonical echo mode if we cannot tell. */
	      ...
	      if (num_keys == 0)
		{
		  ...
		  if (prompt == NULL)
		      ...
		      else
			{
		      ...
		      /* Does NOT quite work when do_emphasize_input is 1. */
