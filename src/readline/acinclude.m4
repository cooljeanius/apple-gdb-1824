...
dnl#
dnl# This is very similar to AC\_C\_LONG\_DOUBLE
...
dnl#
dnl# check for typedef-ed symbols in header files, but allow the caller to
...
if test $bash_cv_type_$1 = no; then
  AC_DEFINE_UNQUOTED([$1],[$3],[Defined depending on the arguments to this macro])
fi
])
...
if test $bash_cv_decl_$1 = yes; then
  AC_DEFINE_UNQUOTED([$bash_tr_func],[1],[Defined to 1 if declared])
else
  AC_DEFINE_UNQUOTED([$bash_tr_func],[0],[Defined to 0 if not declared])
fi
])
...
AC_DEFINE([PRINTF_DECLARED],[1],[Define to 1 if printf is declared])
fi
])
...
AC_DEFINE([SBRK_DECLARED],[1],[Define to 1 if sbrk is declared])
fi
])
...
]])],[bash_cv_decl_under_sys_siglist=yes],[bash_cv_decl_under_sys_siglist=no])])dnl
AC_MSG_RESULT([$bash_cv_decl_under_sys_siglist])
if test $bash_cv_decl_under_sys_siglist = yes; then
AC_DEFINE([UNDER_SYS_SIGLIST_DECLARED],[1],[Define to 1 if something about sys_siglist[]...])
fi
])
...
AC_DEFINE([HAVE_UNDER_SYS_SIGLIST],[1],[Define to 1 for something else about _sys_siglist])
fi
])
...
AC_DEFINE([HAVE_SYS_SIGLIST],[1],[Define to 1 if sys_siglist is available])
fi
])

dnl# Check for the various permutations of sys_siglist and make sure we
dnl# compile in siglist.o if they are not defined
...
AC_DEFINE([HAVE_SYS_ERRLIST],[1],[Define to 1 if we have sys_errlist])
fi
])
...
AC_DEFINE([DUP2_BROKEN],[1],[Define to 1 if dup2 is broken])
fi
])
...
#include <signal.h>]],[[char *s = (char *)strsignal(2);]])],[bash_cv_have_strsignal=yes],[bash_cv_have_strsignal=no])])
AC_MSG_RESULT([$bash_cv_have_strsignal])
if test $bash_cv_have_strsignal = yes; then
AC_DEFINE([HAVE_STRSIGNAL],[1],[Define to 1 if strsignal exists])
fi
])
...
AC_DEFINE([OPENDIR_NOT_ROBUST],[1],[Define to 1 if opendir opens non-directories])
fi
])
...
if test "$bash_cv_type_long_long" = 'long long'; then
  AC_DEFINE([HAVE_LONG_LONG],[1],[Define to 1 if we have the long long type])
fi
])
...
if test "$bash_cv_type_unsigned_long_long" = 'unsigned long long'; then
  AC_DEFINE([HAVE_UNSIGNED_LONG_LONG],[1],[Define to 1 if we have the unsigned long long type])
fi
])
...
AC_DEFINE([RLIMTYPE],[quad_t],[Type of struct rlimit fields])
elif test $bash_cv_type_rlimit = rlim_t; then
AC_DEFINE([RLIMTYPE],[rlim_t],[Type of struct rlimit fields])
fi
])
...
if test $bash_cv_func_lstat = yes; then
  AC_DEFINE([HAVE_LSTAT],[1],[Define to 1 if we have the lstat function])
fi
])
...
if test $bash_cv_func_inet_aton = yes; then
  AC_DEFINE([HAVE_INET_ATON],[1],[Define to 1 if we have the inet_aton function])
else
  AC_LIBOBJ([inet_aton])
fi
])
...
/* The NeXT operating system allows this program to run, but does not allow bash
   to link when it redefines getenv. I am not really interested in figuring out
   why not. */
#if defined (NeXT)
exit(1);
#endif
...
AC_DEFINE([CAN_REDEFINE_GETENV],[1],[Define to 1 if we can redefine getenv])
fi
])
...
AC_DEFINE([HAVE_STD_PUTENV],[1],[Define to 1 if we have a standard-conformant putenv declaration])
fi
])
...
AC_DEFINE([HAVE_STD_UNSETENV],[1],[Define to 1 if we have a standard-conformant unsetenv declaration])
fi
])
...
AC_DEFINE([GETCWD_BROKEN],[1],[Define to 1 if getcwd is broken])
AC_LIBOBJ([getcwd])
fi
])

dnl#
dnl# This needs BASH_CHECK_SOCKLIB, but since that is not called on every
dnl# system, we cannot use AC_PREREQ
dnl#
...
AC_DEFINE([HAVE_LIBC_FNM_EXTMATCH],[1],[Define to 1 if fnmatch does extended pattern matching with FNM_EXTMATCH])
fi
])
...
AC_DEFINE([HAVE_POSIX_SIGSETJMP],[1],[Define to 1 if sigsetjmp/siglongjmp are POSIX-style])
fi
])
...
AC_DEFINE([STRCOLL_BROKEN],[1],[Define to 1 if strcoll is broken])
fi
])
...
AC_DEFINE([HAVE_PRINTF_A_FORMAT],[1],[Define to 1 if printf floating point output in hex notation is available])
fi
])
...
dnl#
dnl# Like AC_STRUCT_ST_BLOCKS, but does NOT muck with LIBOBJS
dnl#
...
AC_DEFINE([HAVE_STRUCT_STAT_ST_BLOCKS],[1],[Define to 1 if we have the struct stat.st_blocks])
fi
])
...
dnl# it is there, since most systems with libsocket require linking
...
if test $bash_cv_have_socklib = yes; then
  ...
  AC_DEFINE([HAVE_LIBSOCKET],[1],[Define to 1 if we have the socket library])
  AC_DEFINE([HAVE_GETPEERNAME],[1],[Define to 1 if we have the getpeername function])
fi
])
...
AC_DEFINE([STRUCT_DIRENT_HAS_D_INO],[1],[if struct dirent has a d_ino member])
fi
])
...
AC_DEFINE([STRUCT_DIRENT_HAS_D_FILENO],[1],[Define to 1 if struct dirent has a d_fileno member])
fi
])
...
if test $bash_cv_struct_timeval = yes; then
  AC_DEFINE([HAVE_TIMEVAL],[1],[Define to 1 if we have the timeval struct])
fi
])
...
AC_DEFINE([PGRP_PIPE],[1],[Define to 1 if pgrps need synchronization])
fi
])
...
AC_DEFINE([NAMED_PIPES_MISSING],[1],[Define to 1 if named pipes are missing])
fi
])
...
AC_DEFINE_UNQUOTED([DEFAULT_MAIL_DIRECTORY],["$bash_cv_mail_dir"],[The default mail directory])
])
...
AC_DEFINE([HAVE_GETPW_DECLS],[1],[Define to 1 if getpw functions are declared in pwd.h])
fi
])
...
if test $bash_cv_dev_fd = "standard"; then
  AC_DEFINE([HAVE_DEV_FD],[1],[Define to 1 if /dev/fd is available])
  AC_DEFINE_UNQUOTED([DEV_FD_PREFIX],["/dev/fd/"],[The /dev/fd prefix])
elif test $bash_cv_dev_fd = "whacky"; then
  AC_DEFINE([HAVE_DEV_FD],[1],[Define to 1 if /dev/fd is available (even if it is "whacky")])
  AC_DEFINE([DEV_FD_PREFIX],["/proc/self/fd/"])
fi
])

AC_DEFUN([BASH_CHECK_DEV_STDIN],
[AC_MSG_CHECKING([whether /dev/stdin and /dev/stdout and /dev/stderr are available])
...
if test $bash_cv_dev_stdin = "present"; then
  AC_DEFINE([HAVE_DEV_STDIN],[1],[Define to 1 if /dev/stdin and /dev/stdout and /dev/stderr are available])
fi
])
...
AC_DEFINE([RLIMIT_NEEDS_KERNEL],[1],[Define to 1 if _KERNEL is needed for RLIMIT defines])
fi
])
...
if test $bash_cv_off_t_64 = yes; then
        AC_DEFINE([HAVE_OFF_T_64],[1],[Define to 1 if we have a 64-bit off_t])
fi])
...
AC_DEFINE([UNUSABLE_RT_SIGNALS],[1],[Define to 1 if real-time signals are unusable due to large values])
fi
])
...
AC_CHECK_FUNC([mbrtowc],[AC_DEFINE([HAVE_MBRTOWC],[1],[Define to 1 if we have the mbrtowc function])])
AC_CHECK_FUNC([mbsrtowcs],[AC_DEFINE([HAVE_MBSRTOWCS],[1],[Define to 1 if we have the mbsrtowcs function])])
AC_CHECK_FUNC([wcwidth],[AC_DEFINE([HAVE_WCWIDTH],[1],[Define to 1 if we have the wcwidth function])])
...
[AC_LINK_IFELSE([AC_LANG_PROGRAM([[#include <wchar.h>]],[[int
main ()
{
  mbstate_t ps;
  return 0;
...
if test $bash_cv_have_mbstate_t = yes; then
	AC_DEFINE([HAVE_MBSTATE_T],[1],[Define to 1 if we have the mbstate_t type])
fi
...
if test $bash_cv_langinfo_codeset = yes; then
  AC_DEFINE([HAVE_LANGINFO_CODESET],[1],[Define to 1 if we have nl_langinfo and CODESET])
fi
