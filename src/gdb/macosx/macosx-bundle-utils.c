/* macosx/macosx-bundle-utils.c: Work with Mac OS X bundles, for GDB.
          ...
          if (plist != NULL)
            {
               ...
               * and NULL the pointer if it is NOT: */
...
/* Return the string value suitable for use with posix file system calls
 * for  KEY found in PLIST.  NULL will be returned if KEY does NOT have a
 * valid value in the the property list, if the value is NOT a string,
 * or if there were errors extracting the value for KEY: */
const char *
macosx_get_plist_posix_value(const void *plist, const char* key)
{
  char *value = NULL;
  CFStringRef cf_key, cf_value;

  if (plist == NULL) {
    return NULL;
  }

  cf_key = CFStringCreateWithCString(kCFAllocatorDefault, key,
  ...
  if ((cf_value != NULL) && (CFGetTypeID(cf_value) == CFStringGetTypeID()))
    {
      ...
      if (max_value_len > 0)
        {
          value = (char *)xmalloc(max_value_len + 1UL);
          if (value)
            {
              if (!CFStringGetFileSystemRepresentation(cf_value, value,
                                                       max_value_len))
                {
                  /* We failed to get a file system representation of the
                   * bundle executable, so instead just free the buffer
                   * that we malloc'ed: */
                  xfree(value);
                  value = NULL;
                }
  ...
  if ((bundle_suffix_pos != NULL) && (bundle_suffix_pos > bundle))
    {
      ...
      /* Allocate enough memory for the bundle directory path with suffix,
       * a directory delimiter, the relative plist bundle path,  and a NULL
       * terminator: */
