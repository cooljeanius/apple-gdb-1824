...
static int look_up_sigtramp_addr_range(void **sigtramp_start,
                                       void **sigtramp_end);
static int slower_better_dladdr(void *addr, Dl_info *dlinfo);
static void fill_in_stack_frame_string_buffer(void *addr, char *buf,
                                              int buflen);
static
int grope_around_incore_macho_file(void *incoreptr_macho_header,
...
/* Hardcoding the allowable range of the stack is definitely a hack but
   I do NOT know a better way to discover what address range may be valid.
...
gdb_self_backtrace(void **buffer, int bufsize)
{
  ...
  return 0;
#endif /* !__ppc__ && !__ppc64__ && !__i386__ */
...
look_up_sigtramp_addr_range(void **sigtramp_start, void **sigtramp_end)
{
  ...
  *sigtramp_start = dlsym(RTLD_DEFAULT, "_sigtramp");
  ...
  p = (char *)incoreptr_linkedit_external_nlists;
...
grope_around_incore_macho_file(void *incoreptr_macho_header,
                               ...
                               int *number_of_external_nlists)
{
  char *incoreptr_linkedit_start = (char *)NULL;
  char *incoreptr_linkedit_nlists = (char *)NULL;

  char *p;
  uint32_t tmp4bytes;
  int wordsize;
  size_t macho_linkedit_fileoff = 0UL;
  uint32_t i;
...
slower_better_dladdr(void *address_to_find, Dl_info *dlinfo)
{
  ...
  if (grope_around_incore_macho_file(dlinfo->dli_fbase, &slide,
                                     &incoreptr_linkedit_strings,
                                     &incoreptr_linkedit_local_nlists,
                                     &number_of_local_nlists,
                                     &incoreptr_linkedit_external_nlists,
  ...
  p = (char *)incoreptr_linkedit_local_nlists;
  ...
  while (p < endpoint)
    {
      ...
      if (((nlist->n_type & N_STAB) == 0)
        ...
        {
          /* Is the address POSSIBLY_BETTER_ADDRESS a closer match than
             the best we have seen so far?  (the best we have seen so far is
             dlinfo->dli_saddr; the target address we are aiming for
