...
#include "macosx-nat-info.h"

extern macosx_inferior_status *macosx_status;

extern void _initialize_macosx_info_commands(void);
...
#endif /* PRINT_FIELD && TASK_EVENTS_INFO[_COUNT] && MACH_CHECK_ERROR */
...
info_mach_ports_command(const char *args, int from_tty)
{
  ...
  for (loop_index = 0U; loop_index < name_count; ++loop_index)
    {
      printf_unfiltered("port name: %#x, type %#x\n",
                        port_names_data[loop_index], port_types_data[loop_index]);
    }
