...
macosx_debug_port_info(task_t task, mach_port_t port)
{
    ...
    (defined(HAVE_MACH_PORT_GET_RECEIVE_STATUS) || defined(mach_port_get_receive_status))
  kern_return_t kret;
...
{
#if (defined(DEBUG) || defined(_DEBUG)) && defined(__APPLE__) && \
    (!defined(__DARWIN_UNIX03) || !__DARWIN_UNIX03) && \
    (!defined(_NO_PORT_T_FROM_MACH) || !_NO_PORT_T_FROM_MACH)
# ifndef port_name_array_t
#  define port_name_array_t mach_port_array_t
# endif /* !port_name_array_t */
# ifndef port_type_array_t
#  define port_type_array_t mach_port_array_t
# endif /* !port_type_array_t */
  kern_return_t ret;
  unsigned int i;
...
{
  const char *dbgmsgname = "macosx_process_message";
  if (msg->msgh_id == MACH_NOTIFY_PORT_DELETED)
    {
      mach_port_deleted_notification_t *dmsg;
      dmsg = (mach_port_deleted_notification_t *)msg;
      if (dmsg->not_port == inferior->task)
        {
          inferior_debug(2, "%s: deletion message for task port 0x%lx\n",
                         dbgmsgname, (unsigned long)dmsg->not_port);
        }
      else
        {
          inferior_debug(2, "%s: deletion message for unknown port 0x%lx; ignoring\n",
                         dbgmsgname, (unsigned long)dmsg->not_port);
        }
    }
  else
    {
      warning("%s: unknown notification type 0x%lx; ignoring",
              dbgmsgname, (unsigned long)msg->msgh_id);
    }
}
