...
macosx_inferior_destroy(macosx_inferior_status *s)
{
  ...
  /* We may have destroyed the signal thread before we reaped the
     child.  Make sure we do that here, or we will pile up children till
     we exit.

     Note, we sometimes get called before we have set up a child in
     which case s->pid will be zero.  Do NOT wait on that...  */
...
macosx_save_exception_ports(task_t task, struct macosx_exception_info *info)
{
  ...
  kret = task_get_exception_ports(task, GDB_MACH_EXC_MASK_ALL,
                               ...
                               struct macosx_exception_info *info)
{
  ...
  for (i = 0; i < info->count; i++)
    {
      kret = task_set_exception_ports(task, info->masks[i], info->ports[i],
                                      info->behaviors[i],
                                      info->flavors[i]);
      if (kret != KERN_SUCCESS)
        return kret;
