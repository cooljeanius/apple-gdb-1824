...
static inline void supply_uint32(int regnum, uint32_t val)
{
...
arm_macosx_fetch_gp_registers_raw(struct gdb_arm_thread_state *gp_regs)
{
  int i;
  for ((i = 0); (i < ARM_MACOSX_NUM_GP_REGS); i++) {
    regcache_raw_supply(current_regcache, (ARM_R0_REGNUM + i),
                        &gp_regs->r[i]);
  }
...
/* Fetch VFP Registers: */
void arm_macosx_fetch_vfpv1_regs(gdb_arm_thread_vfpv1_state_t *fp_regs)
{
  int i;
...
void arm_macosx_fetch_vfpv3_regs(gdb_arm_thread_vfpv3_state_t *fp_regs)
{
  int i;
  for (i = 0; i < ARM_MACOSX_NUM_VFP_REGS; i++)
...
void arm_macosx_fetch_vfpv1_regs_raw(gdb_arm_thread_vfpv1_state_t *fp_regs)
{
  int i;
  for (i = 0; i < ARM_MACOSX_NUM_VFP_REGS; i++)
...
/* Store VFP Registers: */
void arm_macosx_store_vfpv1_regs(gdb_arm_thread_vfpv1_state_t *fp_regs)
{
  int i;
  for (i = 0; i < ARM_MACOSX_NUM_VFP_REGS; i++)
...
void arm_macosx_store_vfpv3_regs(gdb_arm_thread_vfpv3_state_t *fp_regs)
{
  int i;
  for (i = 0; i < ARM_MACOSX_NUM_VFP_REGS; i++)
...
void fetch_inferior_registers(int regno)
{
  ...
  if ((regno == -1) || ARM_MACOSX_IS_GP_RELATED_REGNUM(regno)) {
      ...
      ret = thread_get_state(current_thread, GDB_ARM_THREAD_STATE,
  ...
  if ((regno == -1) || ARM_MACOSX_IS_FP_RELATED_REGNUM(regno)) {
       ...
       * numbering scheme so that we can be able to connect
       * to remote versions of gdbserver that use FSF register numbers: */
