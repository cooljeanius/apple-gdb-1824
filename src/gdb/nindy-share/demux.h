 ...
 *
 *	The TIME_INPUT macro is NOT implemented: it is a no-op.
 *
 *	The damned 'poll' call cannot be used on normal tty's, so DEMUX_WAIT is
 *	also a no-op: DEMUX_READ uses the FIONREAD ioctl if it is available;
...
#	define DEMUX_WAIT(fd)	{					\
			 ...
			 * does NOT mistakenly say there is/was input on them.
			 */
