/* tracepoint.c: Tracing functionality for remote targets in custom GDB protocol
...
extern void x_command(const char *, int);
extern int addressprint;	/* Print machine addresses? */

/* GDB commands implemented in other modules:
 */

extern void output_command(const char *, int);
...
set_traceframe_context(CORE_ADDR trace_pc)
{
  ...
  if (trace_pc == (CORE_ADDR)-1)  /* Cease debugging any trace buffers: */
    {
      traceframe_fun = 0;
...
trace_command(const char *arg, int from_tty)
{
  ...
  struct tracepoint *t;
  const char *addr_start = (const char *)0;
  const char *addr_end = (const char *)0;
  int i;
...
{
  struct tracepoint *t1 = (struct tracepoint *)-1;
  struct tracepoint *t2;
  unsigned int count;
  int all = 0;
...
trace_actions_command(const char *args, int from_tty)
{
  struct tracepoint *t;
  char tmpbuf[128];
  const char *end_msg = "End with a line saying just \"end\".";
...
read_actions(struct tracepoint *t)
{
  ...
  /* FIXME: kettenis/20010823: Something is wrong here.  In this file
     ...
     is NOT declared.  Leave this alone for now.  */
#ifdef STOP_SIGNAL
  if (job_control)
  ...
  while (1)
      ...
      if (linetype == STEPPING)	/* begin "while-stepping" */
	{
	  if (prompt == prompt2)
	    ...
	    prompt = (char *)prompt2; /* change prompt for stepping actions */
	}
      else if (linetype == END)
	{
	  if (prompt == prompt2)
	    {
	      prompt = (char *)prompt1; /* end of single-stepping actions */
	    }
	  else
	    {			/* end of actions */
...
validate_actionline(char **line, struct tracepoint *t)
{
  ...
  if (cmd_cfunc_eq(c, collect_pseudocommand))
	...
	p++;
      ...
      if (*p == '\0' || *steparg == '\0' ||
	  (t->step_count = strtol(p, (char **)&p, 0)) == 0)
	{
...
{
  size_t len; /* 'size_t' should be the same as 'unsigned long' */
  unsigned int reg;
  ...
  switch (SYMBOL_CLASS(sym))
    {
    default:
      printf_filtered("%s: do NOT know symbol class %d\n",
	...
	printf_filtered("LOC_REG[parm] %s: ",
	  ...
	  (len > (size_t)register_size(current_gdbarch, reg)))
	add_register(collect, (reg + 1));
      break;
    case LOC_REF_ARG:
      printf_filtered("Sorry, do NOT know how to do LOC_REF_ARG yet.\n");
      ...
      if (info_verbose)
	{
	  printf_filtered("LOC_REGPARM_ADDR %s: Collect %lu bytes at offset ",
			  DEPRECATED_SYMBOL_NAME(sym), (unsigned long)len);
	  printf_vma(offset);
	       ...
	       char ***stepping_actions)
{
  ...
  struct expression *exp = (struct expression *)NULL;
  struct action_line *action;
  int i;
  ...
  if (collect->next_aexpr_elt >= collect->aexpr_listsize)
	...
	((struct agent_expr **)
         xrealloc(collect->aexpr_list,
                  (2UL * collect->aexpr_listsize * sizeof(struct agent_expr *))));
	...
	if ((target_frameno = (int) strtol (++reply, &reply, 16)) == -1)
	  {
		...
		/* The following will not recurse, since it is
		   special-cased.  */
...
static void
scope_info(const char *args, int from_tty)
{
  struct symtabs_and_lines sals;
  struct symbol *sym;
...
_initialize_tracepoint(void)
{
  ...
  set_internalvar(lookup_internalvar("trace_frame"),
		  value_from_longest(builtin_type_int, (LONGEST)-1L));

  if (tracepoint_list.list == NULL)
    {
      tracepoint_list.listsize = 128;
      tracepoint_list.list =
        ((struct memrange *)
         xmalloc(tracepoint_list.listsize * sizeof(struct memrange)));
    }
  if (tracepoint_list.aexpr_list == NULL)
    {
      tracepoint_list.aexpr_listsize = 128;
      tracepoint_list.aexpr_list =
        ((struct agent_expr **)
         xmalloc(tracepoint_list.aexpr_listsize * sizeof(struct agent_expr *)));
    }
  ...
  if (stepping_list.list == NULL)
    {
      ...
      stepping_list.list =
        ((struct memrange *)
         xmalloc(stepping_list.listsize * sizeof(struct memrange)));
    }

  if (stepping_list.aexpr_list == NULL)
    {
      stepping_list.aexpr_listsize = 128;
      stepping_list.aexpr_list =
        ((struct agent_expr **)
         xmalloc(stepping_list.aexpr_listsize * sizeof(struct agent_expr *)));
    }
