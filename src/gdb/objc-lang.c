...
static void find_methods(struct symtab *, char, const char *, const char *,
                         const char *, struct symbol **, unsigned int *,
                         unsigned int *);
...
/* Should we always run all threads with only minimal checks for the
 * current thread when running po and printing?  */
int po_and_print_run_all_threads = 1;

/* If we cannot safely run po with the other threads frozen, should we run
 * it with the other threads free to execute?  */
...
static unsigned int objc_class_method_limit = 10000U;
...
static CORE_ADDR  lookup_implementation_in_cache(CORE_ADDR, CORE_ADDR);
static void add_implementation_to_cache(CORE_ADDR, CORE_ADDR, CORE_ADDR);

/* APPLE LOCAL: This tree is used for two purposes.  First, we put every
 * valid class we see in the tree using add_class_to_cache.
 * Use class_valid_p to see if we have seen your class already.
 *
 * It also keeps the map of class -> real class.  We find the "real class"
 * by getting the object's "isa" pointer then calling that classes "class"
 * method for this object.
 *
 * Note that you do NOT need to do both "add_class_to_cache" and
 * "add_real_class_to_cache".  If you have both class and real class, then
 * the latter function will add both.
 *
 * We need to do this because the KVO system overwrites the "isa" pointer
 * with the synthesized class that does the KVO notification, but that is
 * not the class that people want to see...
 *
 * We are assuming here that the synthesized class will always return the
 * same "real" class.  */
static struct rb_tree_node *real_class_tree = (struct rb_tree_node *)NULL;


int objc_setup_safe_print(struct cleanup **);

static int class_valid_p(CORE_ADDR);
static struct rb_tree_node *add_class_to_cache(CORE_ADDR);
static CORE_ADDR  lookup_real_class_in_cache(CORE_ADDR);
static void add_real_class_to_cache(CORE_ADDR, CORE_ADDR);

/* APPLE LOCAL: This tree keeps the map of class -> class type.  The tree
 * is probably overkill in this case, but it is easy to use, and we
 * will NOT have all that many of them: */
...
/* '-Wmissing-prototypes' or '-Wmissing-declarations': */
extern void _initialize_objc_lang(void);
...
lookup_objc_class(const char *classname)
{
  ...
  if (! target_has_execution)
    {
      /* Cannot call into inferior to lookup class: */
      return 0;
    }
  ...
  /* Pass in "ALL_THREADS" since we have locked the scheduler: */
...
lookup_child_selector_nocache(const char *selname)
{
  ...
  if (! target_has_execution)
    {
      /* Cannot call into inferior to lookup selector: */
      return 0;
    }
...
value_nsstring(char *ptr, int len)
{
  ...
  struct symbol *sym;
  struct type *type;
  static struct type *func_type = (struct type *)NULL;
  struct type *ret_type;

  if (!target_has_execution)
    return 0;	  /* Cannot call into inferior to create NSString.  */
  ...
  if (lookup_minimal_symbol("_NSNewStringFromCString", 0, 0))
    ...
    {
      function = find_function_in_inferior("istr", ret_type);
      nsstringValue = call_function_by_hand(function, 1, &stringValue[2]);
    }
...
objc_demangle(const char *mangled, int options)
{
  ...
  if (mangled[0] == '_' &&
      ...
      mangled[2] == '_')
    {
      cp = demangled = (char *)xmalloc(strlen(mangled) + 2UL);
	...
	*cp++ = ' ';		/* replace two '_' with one ' '    */
	strcpy(cp, (mangled + (cp - demangled) + 2UL));
      } else {
...
static struct type *
objc_create_fundamental_type(struct objfile *objfile, int c_typeid)
{
  ...
  switch (c_typeid)
	...
	type = init_type(TYPE_CODE_INT,
			 ...
			 0, "<?type?>", objfile);
        warning(_("internal error: no C/C++ fundamental type %d"),
                c_typeid);
	break;
      case FT_VOID:
...
struct objc_trampoline_record
{
  ...
  uint64_t flags; /* Flags as given above.  The size of the flags field in
		   * the actual trampoline in 32 bits if desc_size == 8,
                   * and 64 otherwise.  Might as well just use a 64 bit
                   * value to store it here.  */
};
...
objc_read_trampoline_region(CORE_ADDR addr)
{
  ...
  if (wordsize == 4)
    ...
    internal_error(__FILE__, __LINE__,
                   "Unrecognized word size: %d for trampoline, skipping.",
                   wordsize);

  /* Now that we know how big it is going to be, we can make the region: */
  region =
    ((struct objc_trampoline_region *)
  ...
  for (i = 0; i < region->num_records; i++)
    {
	 ...
	 is the first field of the record.  We just store the trampoline
      ...
      region->records[i].flags = read_memory_unsigned_integer(addr,
                                                              entry_size);
...
/* Programatically determine whether the libobjc this program is using is
   ...
   information).  */
void
objc_init_runtime_version(void)
{
  ...
  /* No __OBJC section, assume we are looking at a version 2 objc
   * runtime.  We could check explicitly for __DATA,__objc_imageinfo
   * but Jim wants us to just assume it is v2 at this point.  */
...
objc_init_trampoline_observer(void)
{
  ...
  /* I do NOT want to keep looking for this symbol forever & ever, so
   * I shalll assume it is in libobjc and if I find that library but not
   * the right symbol, I say it is NOT going to be found.  Note, to do this
   * I have to have a fixed name for the objc library.  This has NOT
   * changed ever (it is still libobjc.A.dylib).  But if this ever changes,
   * then that is going to cause some trouble...  */
  objc_objfile = find_libobjc_objfile();
  if (objc_objfile == NULL)
    return;

  /* Raise the load state.  Somebody might have set it to none or extern,
   * and we would NOT be able to see these private symbols: */
  objfile_set_load_state(objc_objfile, OBJF_SYM_ALL, 1);

  trampoline = lookup_minimal_symbol("gdb_objc_trampolines", NULL,
                                     objc_objfile);
  observer = lookup_minimal_symbol("gdb_objc_trampolines_changed", NULL,
                                   objc_objfile);
  ...
  TRY_CATCH(e, RETURN_MASK_ALL)
    {
      region_addr =
        read_memory_unsigned_integer(objc_trampolines.gdb_objc_trampoline_addr,
                                     wordsize);
    }
  ...
  while (region_addr != 0)
	...
	(struct objc_trampoline_region *volatile)NULL;
	 ...
	 and the objc library is NOT slid yet.  */
      TRY_CATCH(e, RETURN_MASK_ALL)
	{
...
/* This is our update listener function.  It just reads & adds another
 * region record: */
int
...
static void
objc_clear_trampoline_data(void)
{
  /* Free all the regions we have accumulated: */
  struct objc_trampoline_region *region;
...
add_msglist(struct stoken *str, int addcolon)
{
  ...
  len = (plen + strlen(msglist_sel) + 2UL);
  s = (char *)xmalloc(len);
...
end_msglist(void)
{
  ...
  if (!selid)
    error(_("Cannot find selector \"%s\""), p);
...
compare_selectors(const void *a, const void *b)
{
  ...
  return specialcmp((aname + 1UL), (bname + 1UL));
}
...
selectors_info(const char *regexp, int from_tty)
{
  ...
  if (regexp == NULL)
      ...
      else
	{
	  ...
	  if (myregexp[strlen(myregexp) - 1UL] == '$') /* end selector */
	    myregexp[strlen(myregexp) - 1UL] = ']'; /* end method name */
	  else
	    strcat(myregexp, ".*]");
	}
    }
  ...
  if (regexp != NULL)
    {
      val = (char *)re_comp((const char *)myregexp);
      if (val != 0)
...
classes_info(const char *regexp, int from_tty)
{
  ...
  if (regexp == NULL)
    ...
    {
      strcpy(myregexp, regexp);
      if (myregexp[strlen(myregexp) - 1UL] == '$')
	/* In the method name, the end of the class name is marked by ' '.  */
	myregexp[strlen(myregexp) - 1UL] = ' ';
      else
	strcat(myregexp, ".* ");
    }
...
char *
parse_method(char *method, char *type, char **class_str,
...
find_methods(struct symtab *symtab, char type,
	     const char *class_str, const char *category,
	     ...
	     unsigned int *nsym, unsigned int *ndebug)
{
  ...
  unsigned int csym = 0U;
  unsigned int cdebug = 0U;

  static char *tmp = NULL;
  static size_t tmplen = 0UL;
  ...
  ALL_MSYMBOLS(objfile, msymbol)
	...
	continue;
      ...
      while ((strlen(symname) + 1UL) >= tmplen)
	{
	  tmplen = ((tmplen == 0UL) ? 1024UL : (tmplen * 2UL));
	  tmp = (char *)xrealloc(tmp, tmplen);
	}
      strcpy(tmp, symname);

      name_end = parse_method(tmp, &ntype, &nclass, &ncategory,
                              &nselector);
	...
	continue;
      ...
      if ((class_str != NULL)
	  && ((nclass == NULL) || (strcmp(class_str, nclass) != 0)))
	continue;
          ...
          unsigned int *nsym, unsigned int *ndebug)
{
  ...
  unsigned int ncsym = 0U;
  unsigned int ncdebug = 0U;
  ...
  buf = (char *)alloca(strlen(method) + 1UL);
  strcpy(buf, method);
  tmp = parse_method(buf, &type, &class_str, &category, &selector);

  if (tmp == NULL) {
    struct symbol *sym = (struct symbol *)NULL;
    struct minimal_symbol *msym = (struct minimal_symbol *)NULL;
  ...
  if (syms != NULL)
    find_methods(symtab, type, class_str, category, selector,
  ...
  if (syms != NULL)
      ...
      while ((cdebug < csym) && (ncdebug > 0))
	{
	  ...
	  unsigned int j = (csym + ncdebug - 1U);
 ...
 * on the current thread to see if it safe to run code on this thread,
 * allowing the other threads to run: */
int
objc_check_safe_to_run_all_threads(void)
{
  /* Okay, let us see if we can let all the other threads run,
   * and if that will allow the PO to succeed: */
      ...
      if (debug_handcall_setup)
        {
          ...
          fprintf_unfiltered(gdb_stdout,
                             "Setting debugger mode returned: %d output: \"%s\"\n",
                             retval, data);
        }
      ...
      if (e.reason != (int)NO_ERROR)
        {
          if (debug_handcall_setup || info_verbose)
            fprintf_unfiltered(gdb_stdout,
                               "Got uncaught error setting debugger mode, returning NULL cleanup.\n");
      ...
      else if (retval == objc_debugger_mode_fail_objc_api_unavailable)
        {
          if (debug_handcall_setup || info_verbose)
            fprintf_unfiltered(gdb_stdout,
                               "Cannot find objc non-blocking mode, falling back to old print behavior.\n");
          do_cleanups(lock_cleanup);
          safe_p = 0;
        }
      else
          ...
          if (retval == objc_debugger_mode_success)
          ...
          else
            {
              ...
              if (scheduler_lock_on_p())
                {
                  /* The caller has scheduler locking turned on.
                   * We should NOT override that here: */
                  if (debug_handcall_setup || info_verbose)
                    fprintf_unfiltered(gdb_stdout,
                                       "Would have to allow all threads to run to avoid the "
              ...
              else if (objc_check_safe_to_run_all_threads())
                {
                  ...
                  if (debug_handcall_setup || info_verbose)
                    fprintf_unfiltered(gdb_stdout,
                                       "Allowing all threads to run to avoid the possible deadlock.\n");
              ...
              else
                {
                  ...
                  if (debug_handcall_setup || info_verbose)
                    fprintf_unfiltered(gdb_stdout,
                                       "It is unsafe to run code on the current thread, returning NULL cleanup.\n");
...
The 'print-object' command requires an argument (an Objective-C object)");
  ...
  if (po_and_print_run_all_threads)
    {
      /* Set the debugger mode so that we do NOT do any checking: */
  ...
  object =
    expr->language_defn->la_exp_desc->evaluate_exp(builtin_type_void_data_ptr,
                                                   expr, &pc, EVAL_NORMAL);
  ...
  if (obj.isa == 0)
    {
      ...
      if (object_addr == 0x0)
	error("Cannot print the description of a NIL object.");
      else
  ...
  if (c != 0)
    {
      do { /* Read and print characters up to EOS: */
	QUIT;
	printf_filtered("%c", c);
	read_memory((string_addr + i++), &c, 1);
      } while (c != 0);
  ...
  for (i = 0; i < nmethcalls; i++)
    {
      ...
      if (find_pc_partial_function_no_inlined(SYMBOL_VALUE_ADDRESS(func),
                                              NULL, NULL,
					      &methcalls[i].end) == 0)
        {
...
tell_objc_msgsend_cacher_objfile_changed(struct objfile *obj)
{
  ...
  /* I am also hijacking this to clear out the state of the objc trampoline
   * stuff if the changed objfile is libobjc.A.dylib.  This should only
   * happen if you are working on libobjc, but still...  */
...
/* The Objective-C 1.0 runtime uses simple internal data structures that
 * gdb can inspect directly; Objective-C 2.0 uses different internal data
 * structures, most of which gdb cannot inspect itself but must use
 * inferior function calls instead: */
int
new_objc_runtime_internals(void)
{
  ...
  if (validate_function == NULL)
    {
      if (lookup_minimal_symbol("gdb_class_getClass", 0, 0))
        validate_function =
          create_cached_function("gdb_class_getClass",
                                 builtin_type_voidptrfuncptr);
      else
	{
	  if (!already_warned)
	    {
	      already_warned = 1;
	      warning("Failed to find class validation function, calling "
...
static CORE_ADDR
new_objc_runtime_find_impl(CORE_ADDR objc_class, CORE_ADDR sel, int stret)
{
  static struct cached_value *function = NULL;
  ...
  if (stret == 1 && function_stret == NULL)
    {
      if (lookup_minimal_symbol("class_getMethodImplementation_stret", 0, 0))
        function_stret =
          create_cached_function("class_getMethodImplementation_stret",
                                 builtin_type_voidptrfuncptr);
      else
        return 0;
    }
  ...
  if (objc_retval == objc_debugger_mode_success)
    {
      ...
      classval =
        value_from_pointer(lookup_pointer_type(builtin_type_void_data_ptr),
                           objc_class);
      selval =
        value_from_pointer(lookup_pointer_type(builtin_type_void_data_ptr),
                           sel);

      /* Have to release these values since we intend to use them across
       * more than one call into the target: */
      release_value(classval);
      release_value(selval);

      value_cleanup = make_cleanup((make_cleanup_ftype *)value_free,
                                   classval);
      ...
      if (debug_objc)
        fprintf_unfiltered(gdb_stdlog,
                           "new_objc_runtime_find_impl: Getting class from 0x%s.\n",
                           paddr_nz(objc_class));
      ...
      if (retval == 0)
        {
          if (debug_objc)
            fprintf_unfiltered(gdb_stdlog,
                               "    new_objc_runtime_find_impl: Failed getting class from 0x%s.\n",
                               paddr_nz(objc_class));
	  goto cleanups;
        }
      ...
      if (debug_objc)
        fprintf_unfiltered(gdb_stdlog,
                           ...
                           paddr_nz(retval), paddr_nz(objc_class));

      ret_value =
        call_function_by_hand(lookup_cached_function(stret
                                                     ? function_stret
                                                     : function),
                              2, infargs);
      ...
      if (debug_objc)
        fprintf_unfiltered(gdb_stdlog,
                           "    new_objc_runtime_find_impl: Found implementation %s.\n",
                           paddr_nz(retval));
      ...
      if (find_pc_partial_function_no_inlined(retval, &imp_name, NULL, NULL))
	{
      ...
      add_implementation_to_cache(objc_class, sel, retval);
    }
  else
    {
      if ((objc_retval == objc_debugger_mode_fail_malloc_lock_held)
        ...
        {
          /* Something is holding a malloc/objc runtime lock, so we cannot
           * call into the usual method lookup functions.  The objc runtime
           * might already know this class/selector - see if it can tell us
           * the method address by looking in its existing cache tables. */
          ...
          if (function_cache_getImp == NULL)
            {
              if (lookup_minimal_symbol("_cache_getImp", 0, 0))
                function_cache_getImp =
                  create_cached_function("_cache_getImp",
                                         builtin_type_voidptrfuncptr);
              else
                {
                  retval = 0;
          ...
          ret_value =
            call_function_by_hand(lookup_cached_function(function_cache_getImp),
                                  2, infargs);
          ...
          if (debug_objc)
            fprintf_unfiltered(gdb_stdlog,
                               "new_objc_runtime_find_impl: Falling back on __cache_getImp - "
                               "found implementation %s.\n",
                               paddr_nz(retval));

          if (retval != 0)
            {
              if (find_pc_partial_function_no_inlined(retval, &imp_name,
                                                      NULL, NULL))
                {
                  if (strcmp(imp_name, "_objc_msgForward") == 0
                      || strcmp(imp_name, "_objc_msgForward_stret") == 0)
...
read_objc_method(CORE_ADDR addr, struct objc_method *method)
{
  ...
  method->imp = read_memory_unsigned_integer((addr + (addrsize * 2)),
                                             addrsize);
}
...
read_objc_object(CORE_ADDR addr, struct objc_object *object)
{
  ...
  output_cleanup = make_cleanup_ui_out_suppress_output(uiout);
  ...
  if (object_get_class_function == NULL)
    {
      if (lookup_minimal_symbol("gdb_object_getClass", 0, 0))
        {
          object_get_class_function =
            create_cached_function("gdb_object_getClass",
                                   builtin_type_voidptrfuncptr);
        }
      else
        {
          if (info_verbose && !already_warned)
            {
              already_warned = 1;
              warning("Failed to find gdb_object_getClass, we may not properly"
  ...
  if (object_get_class_function == NULL)
	...
	{
	  /* If we are on a system that does NOT have
  ...
  else
    {
      ...
      TRY_CATCH(e, RETURN_MASK_ALL)
        {
          ret_value =
            call_function_by_hand(lookup_cached_function(object_get_class_function),
                                  1, &object_ptr_val);
        }
...
static void
read_objc_class(CORE_ADDR addr, struct objc_class *inf_objc_class)
{
  int addrsize = TARGET_ADDRESS_BYTES;
  inf_objc_class->isa =
    read_memory_unsigned_integer((addr + (addrsize * 0)), addrsize);
  inf_objc_class->super_class =
    read_memory_unsigned_integer((addr + (addrsize * 1)), addrsize);
  inf_objc_class->name =
  ...
  inf_objc_class->methods =
    read_memory_unsigned_integer((addr + (addrsize * 7)), addrsize);
  inf_objc_class->cache =
    read_memory_unsigned_integer((addr + (addrsize * 8)), addrsize);
  inf_objc_class->protocols =
    read_memory_unsigned_integer((addr + (addrsize * 9)), addrsize);
}
...
static CORE_ADDR
lookup_implementation_in_cache(CORE_ADDR objc_class, CORE_ADDR sel)
{
  struct rb_tree_node *found;

  found = rb_tree_find_node_all_keys(implementation_tree, objc_class, -1,
                                     sel);
  if (found == NULL)
    return 0;
  else
...
find_implementation_from_class(CORE_ADDR objc_class, CORE_ADDR sel)
{
  ...
  if (debug_objc)
    fprintf_unfiltered(gdb_stdlog,
                       "Looking up implementation for class: 0x%s selector: 0x%s.\n",
                       paddr_nz(objc_class), paddr_nz(sel));

   implementation = lookup_implementation_in_cache(objc_class, sel);
   if (implementation != 0)
     {
       if (debug_objc)
         fprintf_unfiltered(gdb_stdlog,
                            "Found implementation 0x%s in cache.\n",
                            paddr_nz(implementation));
       return implementation;
     }

  if (new_objc_runtime_internals())
    return (new_objc_runtime_find_impl(objc_class, sel, 0));
     ...
     back is the same as the one we already had.  */
  {
    ...
    TRY_CATCH(e, RETURN_MASK_ALL)
      {
	read_objc_class(objc_class, &class_str);
	...
	return 0;
    ...
    else
      {
        ...
        TRY_CATCH(e, RETURN_MASK_ALL)
          {
            read_objc_class(objc_class, &verify_str);
          }
        if ((e.reason == (int)NO_ERROR)
            && (verify_str.name != class_str.name))
          {
	    if (info_verbose)
	      printf_unfiltered("Class address for name: \"%s\": 0x%s didn't match input address: 0x%s.\n",
                                class_name, paddr_nz(class_addr),
                                paddr_nz(objc_class));
	    return 0;
          }
      }
...
#define CLS_NO_METHOD_ARRAY 0x4000
      ...
      for (;;)
	{
	  ...
	  if (class_str.methods == 0x0)
	    ...
	    {
	      mlist =
                read_memory_unsigned_integer((class_str.methods
                                              + (addrsize * mlistnum)),
                                             addrsize);
	  ...
	  for (i = 0; i < nmethods; i++)
	    {
	      ...
	      if ((unsigned int)(++total_methods) >= objc_class_method_limit)
		{
		...
		{
		  add_implementation_to_cache(objc_class, sel,
                                              meth_str.imp);
		  return meth_str.imp;
		}
	    }
...
#ifdef OBJECT_IS_ADDR_OF_TWO_WORD_STRUCT
  object = read_memory_unsigned_integer((object + addrsize), addrsize);
  object = read_memory_unsigned_integer((object + addrsize), addrsize);
#endif /* OBJECT_IS_ADDR_OF_TWO_WORD_STRUCT */
...
int
should_lookup_objc_class(void)
{
  return lookup_objc_class_p;
}

static int
new_objc_runtime_get_classname(CORE_ADDR objc_class,
...
static char *
lookup_classname_in_cache(CORE_ADDR objc_class)
{
  struct rb_tree_node *found;

  found = rb_tree_find_node_all_keys(classname_tree, objc_class, -1, -1);
  if (found == NULL)
    return NULL;
  else
...
static void
add_classname_to_cache(CORE_ADDR objc_class, char *classname)
{
  struct rb_tree_node *new_node;
  new_node = (struct rb_tree_node *)xmalloc(sizeof(struct rb_tree_node));
...
struct type *
objc_target_type_from_object(CORE_ADDR object_addr, struct block *block,
			     int addrsize, char **class_name_ptr)
{
  ...
  if (name_ptr == NULL)
    {
      name_ptr = class_name;
      if (new_objc_runtime_internals())
	retval = new_objc_runtime_get_classname(isa_addr, class_name,
                                                sizeof(class_name));
      else
	{
	  size_t i;
	  ...
	  name_addr =
            read_memory_unsigned_integer((isa_addr + (addrsize * 2)),
                                         addrsize);
	  ...
	  if (!(isalpha(class_name[0]) || class_name[0] == '_'))
	    ...
	    {
	      for (i = 1UL; i < (sizeof(class_name) - 1UL); i++)
		{
		  if (class_name[i] == '\0')
		    break;
...
int
objc_runtime_lock_taken_p(void)
{
  static struct cached_value *function = NULL;
  if (function == NULL)
    {
  ...
  if (function != NULL)
    {
      /* initialize as NULL to quiet the compiler: */
      struct value *retval = (struct value *)NULL;
      struct gdb_exception e;
      TRY_CATCH(e, RETURN_MASK_ALL)
	{
	  retval = call_function_by_hand(lookup_cached_function(function),
...
objc_fixup_ivar_offset(const struct type *type, int ivar)
{
  ...
  if (is_bitfield_p)
	...
	cur_bitfield_offset = -cur_bitfield_offset;
      ...
      /* Find its offset relative to the first bitfield: */
  ...
  if (class_name == NULL)
	...
	TYPE_FIELD_BITPOS_ASSIGN(type, ivar) = (0 - old_offset);
      return TYPE_FIELD_BITPOS_ASSIGN(type, ivar);
    }
  ...
  len = (prefix_len + strlen(class_name) + strlen(field_name) + 1UL + 1UL);
  symbol_name = (char *)xmalloc(len);
  name_cleanup = make_cleanup(xfree, symbol_name);

  snprintf(symbol_name, len, "%s%s.%s", IVAR_OFFSET_PREFIX, class_name,
           field_name);
  ...
  if (!safe_read_memory_unsigned_integer(ivar_addr, 4, &ivar_offset))
    {
      warning("Failed to read ivar offset at %s for ivar symbol \"%s\".\n",
  ...
  if (info_verbose)
	...
	orig_value = -TYPE_FIELD_BITPOS_ASSIGN(type, ivar);
      ...
      if (ivar_offset != (ULONGEST)orig_value)
	{
	  fprintf_unfiltered(gdb_stdlog, "Changing ivar offset for ivar: %s"
			     " of class: %s from: %ld to %lu.\n",
			     field_name, class_name, orig_value,
...
do_end_debugger_mode(void *arg)
{
  ...
  struct cleanup *scheduler_cleanup;
  struct gdb_exception e;
  struct breakpoint *hidden_step_resume_bp;
  ...
  /* Make sure we do NOT hit any user breakpoints while turning off
     the debugger mode.  */
  ...
  TRY_CATCH(e, RETURN_MASK_ALL)
    {
      call_function_by_hand(lookup_cached_function(end_function), 0, NULL);
    }
  ...
  if (e.reason != (int)NO_ERROR)
    {
      fprintf_unfiltered(gdb_stdout,
                         "Error resetting ObjC debugger mode: %s",
                         e.message);
      throw_exception(e);
    }
}
...
static int
init_debugger_mode_fail_notification(void)
{
...
/* This sets the ObjC runtime into "debugger" mode if available
   ...
   * If we succeed in setting the debug mode, then we set debug_mode_flag_p
     to debug_mode_okay, & then on subsequent calls we return
     objc_debugger_mode_success.
   * If we successfully call the debugger mode function, but we get back
     "no can do" we set *CLEANUP to a no-op and return
...
make_cleanup_set_restore_debugger_mode(struct cleanup **cleanup, int level)
{
  ...
  if (level == 0)
	...
	do_hand_call_cleanups(ALL_CLEANUPS);
      ...
      /* Set the reason to success so that we shall return that on
       * subsequent calls: */
  ...
  if ((e.reason != (int)NO_ERROR) || !success)
    {
      do_cleanups(scheduler_cleanup);
  ...
  TRY_CATCH(e, RETURN_MASK_ALL)
    {
      tmp_value =
        call_function_by_hand(lookup_cached_function(start_function),
                              1, &tmp_value);
    }
  ...
  if (debug_handcall_setup)
    fprintf_filtered(gdb_stdout,
                     "Tried to start debugger mode, return value: %d.\n",
                     success);
...
static int
class_valid_p(CORE_ADDR objc_class)
{
  struct rb_tree_node *found;

  found = rb_tree_find_node(real_class_tree, objc_class, -1);
  return (found != NULL);
}

static CORE_ADDR
lookup_real_class_in_cache(CORE_ADDR objc_class)
{
  struct rb_tree_node *found;
...
add_class_to_cache(CORE_ADDR objc_class)
{
  struct rb_tree_node *new_node;

  new_node = rb_tree_find_node(real_class_tree, objc_class, -1);
  if (new_node != NULL)
    return new_node;
...
static void
add_real_class_to_cache(CORE_ADDR objc_class, CORE_ADDR real_class)
{
  struct rb_tree_node *new_node;
  ...
  new_node = add_class_to_cache(objc_class);
