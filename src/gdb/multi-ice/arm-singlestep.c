/* arm-singlestep.c: Single-stepping code for the Acorn Risc Machine (ARM)
...
#if !defined(DEFS_H) && !defined(__GDB_MULTI_ICE_SERVER_H__)
/* These from server.h. I am not including that because this file
 * is still hacked bleeding out of gdb, and I do NOT want to deal with
...
#endif /* !DEFS_H && !__GDB_MULTI_ICE_SERVER_H__ */

/* The routines in this file are taken from arm-tdep.c. I did NOT want to
...
#ifndef IS_THUMB_ADDR
# define IS_THUMB_ADDR(addr) ((addr) & 1)
#endif /* !IS_THUMB_ADDR */
#ifndef MAKE_THUMB_ADDR
# define MAKE_THUMB_ADDR(addr) ((addr) | 1)
#endif /* !MAKE_THUMB_ADDR */
#ifndef UNMAKE_THUMB_ADDR
 ...
 * macro is bogus. However, we do NOT have the minimal symbols. So,
                                     ...
                                     unsigned long status_reg)
{
  ...
  if (bit(inst, 4)) {
      ...
      shift = (((rs == 15) ? (pc_val + 8) : read_aregister(rs)) & 0xFF);
  } else {
...
static CORE_ADDR thumb_get_next_pc(CORE_ADDR pc, unsigned short *is_thumb)
{
  ...
  *is_thumb = 1;
  ...
  if ((inst1 & 0xff00) == 0xbd00)	/* pop {rlist, pc} */
    {
      ...
      /* Fetch the saved PC from the stack. It is/was stored above
       * all of the other registers.  */
#if defined(REGISTER_SIZE)
      offset = (bitcount(bits(inst1, 0, 7)) * REGISTER_SIZE);
#else
      offset = (bitcount(bits(inst1, 0, 7)) * 1);
#endif /* REGISTER_SIZE */
      sp = read_aregister(SP_REGNUM);
    ...
    {
      unsigned long status = read_aregister (PS_REGNUM);
  ...
  else if ((inst1 & 0xff80) == 0x4700) /* Branch & Exchange IS */
    {
      unsigned short pc_register;

      pc_register = (inst1 & 0x0038) >> 3;
      nextpc = read_aregister (pc_register);
      *is_thumb = (nextpc & 0x1);
      nextpc <<= 1;
    }
  ...
  if (condition_true (bits (this_instr, 28, 31), status))
      ...
      else
	{
	  switch (bits (this_instr, 24, 27))
		...
		operand1 = (rn == 15) ? pc_val + 8 : read_aregister (rn);
	      ...
	      if (bit (this_instr, 20))
		{
		  ...
		  if (bits (this_instr, 12, 15) == 15)
		      ...
		      if (bit (this_instr, 22))
			{
		      ...
		      /* byte write to PC */
		      rn = bits (this_instr, 16, 19);
		      base = (rn == 15) ? pc_val + 8 : read_aregister (rn);
	      ...
	      if (bit (this_instr, 20))
		{
		  ...
		  if (bit (this_instr, 15))
			...
			unsigned long rn_val =
			  read_aregister(bits(this_instr, 16, 19));
			nextpc =
			  (CORE_ADDR)read_memory_integer((CORE_ADDR)(rn_val + offset), 4);
		      }
		      nextpc = ADDR_BITS_REMOVE (nextpc);
		      if (nextpc == pc) {
			  output_error("Infinite loop detected\n");
