...
#ifndef GDB_MULTI_ICE_LOW_H
#define GDB_MULTI_ICE_LOW_H 1
...
enum target_signal low_query_last_signal(void);

int low_update_registers(void);
int low_write_registers(void);

int low_read_memory_raw(CORE_ADDR start, void *buffer,
                        unsigned int *nbytes);
int low_read_memory(char *buffer, CORE_ADDR start_addr,
                    unsigned int nbytes);
int low_write_memory(char *data, CORE_ADDR start_addr,
                     unsigned int nbytes);

int low_set_breakpoint(CORE_ADDR bp_addr, int size);
int low_delete_breakpoint(CORE_ADDR bp_addr, int size);

int low_set_watchpoint(CORE_ADDR watch_addr, int size,
                       enum watch_type type);
int low_delete_watchpoint(CORE_ADDR watch_addr, int size,
                          enum watch_type type);

int low_resume(enum resume_mode mode, int signo);
int low_test(char *buffer);

int low_thread_op(char *buffer, char **result);

/* from rdi150-low.c: */
int low_stop(void);
int no_thread_op(char *input_buffer);

#endif /* !GDB_MULTI_ICE_LOW_H */
