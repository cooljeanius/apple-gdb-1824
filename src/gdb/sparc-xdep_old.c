...
/* We do NOT store all registers immediately when requested, since they
     ...
     int regno;
{
  ...
  /* Global and Out regs are fetched directly, as well as the control
     registers.  If we are getting one of them in our local regs,
     and the stack pointer has not yet been fetched,
     we have to do that first, since they are/were found in memory relative
  ...
  if (regno < O7_REGNUM  /* including -1 */
	...
	register_valid[i] = 1;
      ...
      /* If we do NOT set these valid, read_register_bytes() rereads
     ...
     int regno;
{
  ...
  /* See if we are forcing the stores to happen now, or deferring. */
  if (regno == -2)
    {
      ...
      if (wanna_store == STACK_REGS)
	{
	  /* Fall through and just store one stack reg.  If we deferred
	     it, we would have to store them all, or remember more info.  */
	}
      else
	{
...
#define gregs ((struct regs *)core_reg_sect)
    ...
    *(int *)&registers[REGISTER_BYTE (Y_REGNUM)] = gregs->r_y;
    ...
    /* My best guess at where to get the locals and input
       ...
       will NOT work, but it is worth the try. */
    {
      int sp;
			  ...
			  16 * REGISTER_RAW_SIZE (L0_REGNUM)))
	{
	  ...
	  fprintf (stderr,
		   "Could NOT read input and local registers from core file\n");
	}
    }
  } else if (which == 2) {
