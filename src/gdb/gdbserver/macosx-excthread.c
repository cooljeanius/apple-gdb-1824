...
/* We use this to make sure the exception thread has finished writing all
   ...
   It is actually a little more complex than this, because the main thread
   is going to enter select first time round to wait till the exception thread
   wakes up. Then it is going to call select with a 0 timeout to poll for other
...
macosx_save_exception_ports(task_t task, struct macosx_exception_info *info)
{
  ...
  kret = task_get_exception_ports(task, EXC_MASK_ALL, info->masks,
...
/* This is the storage for the mach messages the exception thread
   ...
   but I do NOT imagine that we will ever really need to do that.  */
...
/* */
static void
excthread_debug_re_endline(int level)
{
  if (excthread_debugflag >= level)
    {
...
/* */
static void
excthread_debug_exception(int level, mach_msg_header_t *msg)
{
  if (excthread_debugflag < level)
    return;
...
/* */
kern_return_t
catch_exception_raise_state(mach_port_t port, exception_type_t exception_type,
...
/* */
kern_return_t
catch_exception_raise_state_identity(mach_port_t port, mach_port_t thread_port,
...
/* */
kern_return_t
catch_exception_raise(mach_port_t port, mach_port_t thread_port,
		      mach_port_t task_port, exception_type_t exception_type,
		      exception_data_t exception_data,
		      mach_msg_type_number_t data_count)
{
#if 0
...
macosx_exception_thread_create(macosx_exception_thread_status *s)
{
  ...
  kret = task_set_exception_ports(s->task, EXC_MASK_ALL,
				  s->inferior_exception_port,
...
macosx_exception_thread(void *arg)
{
  ...
  for (;;)
      ...
      while (1)
	{
	  ...
	  kret =
	    mach_msg(&msg_data[next_msg_ctr].msgin.hdr, receive_options, 0,
		     ...
		     s->inferior_exception_port, 0, MACH_PORT_NULL);
      ...
      for (counter = 0; counter < next_msg_ctr; counter++)
	{
	  ...
	  if (kret != KERN_SUCCESS)
	    {
	      excthread_debug_re(0,
				 "error sending exception reply: %s (0x%lx)\n",
				 MACH_ERROR_STRING(kret), (unsigned long)kret);
	      abort();
	    }
	}
