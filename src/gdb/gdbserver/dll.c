...
/* Find a DLL with the same name and/or base address. A NULL name in the key
...
void loaded_dll(const char *name, CORE_ADDR base_addr)
{
  ...
  /* casting by itself is NOT enough, need to choose a member... not sure which
   * one, so choose arbitrarily between the two members that are already 'long'
   * (going with the first one): */
  new_dll->entry.id = (unsigned long)minus_one_ptid.lwp;
#else
