...
if test x"${ac_cv_c_tclh}" != x ; then
  ...
  if test x"${ac_cv_c_tclh}" = x"installed" ; then
    ...
    # this hack is cuz the TCLHDIR will NOT print if there is a "-I" in it.
...
#
# Ok, let us find the tcl library
...
if test x"${no_tcl}" = x ; then
  ...
  no_tcl=true
  AC_ARG_WITH([tcllib],[AS_HELP_STRING([--with-tcllib],[directory where the tcl library is])],
         [with_tcllib=${withval}])
  AC_MSG_CHECKING([for Tcl library])
  AC_CACHE_VAL([ac_cv_c_tcllib],[
  # First check to see if --with-tcllib was specified.
  # This requires checking for both the installed & uninstalled name-styles
  # since we have no idea if it is installed or not.
  if test x"${with_tcllib}" != x ; then
    ...
    # if Makefile exists we assume it is configured and libtcl will be
    # built 1st.
      ...
      AC_MSG_ERROR([${with_tcllib} directory does NOT contain libraries])
    fi
  fi
  ...
  if test x"${ac_cv_c_tcllib}" = x ; then	
    ...
    Tcl_AppInit()
    { exit(0); }]])],[ac_cv_c_tcllib="-l$installedtcllibroot"],[ac_cv_c_tcllib=""],[ac_cv_c_tclib="-l$installedtcllibroot"])
    LIBS="${orig_libs}"
   fi
  ])
  if test x"${ac_cv_c_tcllib}" = x ; then
    TCLLIB="# no Tcl library found"
    AC_MSG_WARN([Cannot find Tcl library])
  else
...
# and again w/Tcl. The search order is the best place to look 1st, then in
...
AC_ARG_WITH([tkinclude],[AS_HELP_STRING([--with-tkinclude],[directory where the tk private headers are])],
...
# & search for the worst fit, overwriting it w/better fits as we find them
...
if test x"${ac_cv_c_tkh}" != x ; then
  ...
  if test x"${ac_cv_c_tkh}" = x"installed" ; then
    ...
    # this hack is cuz the TKHDIR will NOT print if there is a "-I" in it.
...
if test x"${no_tk}" = x ; then
  ...
  AC_ARG_WITH([tklib],[AS_HELP_STRING([--with-tklib],[directory where the tk library is])],
              [with_tklib=${withval}])
  ...
  if test x"${ac_cv_c_tklib}" = x ; then
       ...
       AC_RUN_IFELSE([AC_LANG_SOURCE([[
       Tcl_AppInit()
       { exit(0); }]])],[ac_cv_c_tklib="-l$installedtklibroot"],[ac_cv_c_tklib=""],[ac_cv_c_tklib="-l$installedtklibroot"])
       LIBS="${orig_libs}"
   fi
  ])
  if test x"${ac_cv_c_tklib}" = x ; then
    TKLIB="# no Tk library found"
    AC_MSG_WARN([Cannot find Tk library])
  else
    TKLIB=$ac_cv_c_tklib
...
AC_DEFUN([CY_AC_PATH_TK],[
  AC_REQUIRE([CY_AC_PATH_TKH])
  AC_REQUIRE([CY_AC_PATH_TKLIB])
])

AC_DEFUN([CY_AC_PATH_TCL],[
  AC_REQUIRE([CY_AC_PATH_TCLH])
  AC_REQUIRE([CY_AC_PATH_TCLLIB])
])
