...
#  warning "irix5-nat.c expects <sys/procfs.h> to be included."
...
fill_gregset(gregset_t *gregsetp, int regno)
{
  ...
  if ((regno == -1) || (regno == PC_REGNUM))
    {
      regcache_raw_read_signed(current_regcache,
			       mips_regnum(current_gdbarch)->pc, &regval);
      *(regp + CTX_EPC) = regval;
    }
  ...
  if ((regno == -1) || (regno == mips_regnum(current_gdbarch)->cause))
    {
      regcache_raw_read_signed(current_regcache,
			       mips_regnum(current_gdbarch)->cause, &regval);
  ...
  if ((regno == -1)
    ...
    {
      regcache_raw_read_signed(current_regcache,
			       mips_regnum(current_gdbarch)->hi, &regval);
  ...
  if ((regno == -1) || (regno == mips_regnum (current_gdbarch)->lo))
    {
      regcache_raw_read_signed(current_regcache,
			       mips_regnum(current_gdbarch)->lo, &regval);
...
supply_fpregset(fpregset_t *fpregsetp)
{
  ...
  /* We cannot supply the FSR register directly to the regcache,
  ...
  /* FIXME: how can we supply FCRIR?  SGI does NOT tell us. */
  regcache_raw_supply(current_regcache,
