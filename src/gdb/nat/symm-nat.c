/* symm-nat.c: Sequent Symmetry host interface, for GDB when running under Unix.
...
#if 0	 /* disabled because it does NOT go through the target vector.  */
		  ...
		  struct target_ops *target)
{
  ...
  register int count =
    (int)((((memaddr + len) - addr) + sizeof(PTRACE_XFER_TYPE) - 1)
...
extern void _initialize_symm_nat(void); /* -Wmissing-prototypes */
void
