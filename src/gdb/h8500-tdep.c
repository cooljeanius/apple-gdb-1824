/* h8500-tdep.c: Target-dependent code for Hitachi H8/500, for GDB.
...
NEXT_PROLOGUE_INSN(CORE_ADDR addr, CORE_ADDR lim, char *pword1)
{
  if (addr < lim + 8)
    {
      read_memory(addr, (gdb_byte *)pword1, 1);
      read_memory(addr, (gdb_byte *)(pword1 + 1U), 1);
      return 1;
    }
  return 0;
}
...
/* */
static void
big_command(const char *arg, int from_tty)
{
...
/* */
static void
medium_command(const char *arg, int from_tty)
{
  h8500_set_pointer_size(32);
  code_size = 4;
  data_size = 2;
...
/* */
static void
compact_command(const char *arg, int from_tty)
{
  h8500_set_pointer_size(32);
  code_size = 2;
  data_size = 4;
...
/* */
static void
small_command(const char *arg, int from_tty)
{
...
/* */
static void
set_memory(const char *args, int from_tty)
{
...
{
  const char *page_regnum;
  char *regnum;
  char expression[100];
  unsigned int new_regval;
  struct type *type;
  ...
  if (((newval_type_code != TYPE_CODE_INT)
    ...
    error(_("Illegal type (%s) for assignment to $%s\n"),
	  TYPE_NAME(VALUE_TYPE(newval)), var->name);

#ifdef VALUE_CONTENTS_RAW
  new_regval = *(long *)VALUE_CONTENTS_RAW(newval);
#else
  new_regval = 0U;
#endif /* VALUE_CONTENTS_RAW */
  ...
  snprintf(expression, sizeof(expression), "$%s=%d", regnum,
	   (new_regval & 0xffff));
  parse_and_eval(expression);
}
...
extern void _initialize_h8500_tdep(void); /* -Wmissing-prototypes */
void
