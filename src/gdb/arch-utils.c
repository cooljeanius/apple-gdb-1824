...
gdbarch_info_fill(struct gdbarch *gdbarch, struct gdbarch_info *info)
{
  /* APPLE LOCAL: I moved the "info->abfd" test before the gdbarch
     ...
     architecture, you will always just get that, so you cannot ask the
     question "does the architecture of THIS BFD match the currently
     selected gdbarch".  That is a useful question to have
     answered.  */
  if ((info->bfd_arch_info == NULL)
      && (info->abfd != NULL)
    ...
    {
      if (gdbarch_debug > 1)
	fprintf_unfiltered(gdb_stdlog, "bfd_get_arch_info");
