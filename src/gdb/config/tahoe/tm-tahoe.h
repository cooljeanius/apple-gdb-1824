...
extern CORE_ADDR tahoe_skip_prologue  (CORE_ADDR);
...
/* Wrong for cross-debugging. I do NOT know the real values.  */
...
/* On BSD, sigtramp is in the u area. Cannot check the exact
   addresses because for cross-debugging we do NOT have target include
...
extern int tahoe_frame_num_args  (struct frame_info * fi);
