...
/* This seems to be right for the 90x comp.vuw.ac.nz.
   ...
   maximum control stack depth.  If so, I do NOT know where the
   control-stack depth is configured, so I cannot #include it here. */
...
/* The following definitions does NOT seem to work.
...
/* Put here the code to store, into a struct frame_saved_regs,
   ...
   compiler and gcc currently ignore them, so it should not be an issue. */
...
/*first_insn = read_memory_integer (get_pc_function_start ((fi_p)->pc),4); */ \
  ...
  if (0x08 == ((first_insn >> 20) &0x0ff)) {                                \
    /* NB: because WINDOW_REGISTER_P(cfp) is false, a saved cfp             \
       ...
       I do NOT understand why we do NOT have to do that for pc, too.  */    \
                                                                    \
