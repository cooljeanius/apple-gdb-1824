...
/* Given the exposed pipeline, there is NOT any one correct value.
   However, this value must be 4.  GDB cannot handle any other value (other than
   zero).  See for example infrun.c:
   "prev_pc != stop_pc - DECR_PC_AFTER_BREAK"  */
/* FIXME */
#define DECR_PC_AFTER_BREAK 8

/* We do NOT have a reliable single step facility.
   ??? We do have a cycle single step facility, but that will NOT work.  */
