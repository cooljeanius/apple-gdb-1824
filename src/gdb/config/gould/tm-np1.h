 ...
 * Do NOT know what 0xa4 is; it is mentioned in stab.h
 * but only in the sdb symbol list.
 */
#define IGNORE_SYMBOL(type) (type == N_ENTRY || type == 0xa4)

/* We do NOT want the extra gnu symbols on the machine;
 ...
 * terminology, small structs do NOT use the struct return convention).
 */
extern use_struct_convention_fn gould_use_struct_convention;
#define USE_STRUCT_CONVENTION(gcc_p, type) gould_use_struct_convention (gcc_p, type)

...
#define FRAME_CHAIN_VALID(chain, thisframe) gould_frame_chain_valid (chain, thisframe)
...
#define FRAME_NUM_ARGS(fi)          (findarg(fi))
