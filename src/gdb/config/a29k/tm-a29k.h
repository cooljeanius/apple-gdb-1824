...
/* Parameters for an EB29K (a board which plugs into a PC and is
   ...
   use as we would use a remote stub (see remote-eb.c).
...
/* Immediately after a function call, return the saved pc.
   Cannot go through the frames for this because on some machines
...
/* The a29k user's guide documents well what the stacks look like.
   But what is NOT so clear there is how this interracts with the
   symbols, or with GDB.
   ...
   functions with fixed size memory frames (i.e. those who do NOT use
...
/* LOC_ARG, LOC_LOCAL - For GCC, these are relative to saved_msp.
   ...
   macros, it does NOT really matter exactly how we
   ...
   CALL_DUMMY_LOCATION == ON_STACK (but we do NOT do so, so do NOT worry about it).
   ...
   (FIXME: the following ASCII art's whitespace might be messed up...)
...
/* Because INIT_FRAME_PC gets passed fromleaf, that is where we init
...
/* On the a29k, the nominal address of a frame is the address on the
   ...
   However, that does NOT work for us, so when creating the innermost
...
/* If rsize is zero, we must be at end of stack (or otherwise hosed).
   If we do NOT check rsize, we loop forever if we see rsize == 0.  */
...
/* The dummy frame looks like this (see also the general frame picture
   ...
   (FIXME: whitespace looks messed up)
					register stack
   ...
   arg_slop: This area is so that when the call dummy adds 16 words to
      the msp, it will NOT end up larger than mfp_dummy (it is needed in the
...
#define	BS(const)	(((const) & 0xff) << 24) |	\
			...
			(((const) & 0xff0000) >> 8) |	\
			(((const) & 0xff000000) >> 24)
#endif /* TARGET_BYTE_ORDER == HOST_BYTE_ORDER */
...
/* a29k architecture has separate data & instruction memories -- wired to
   different pins on the chip -- and cannot execute the data memory.
   Also, there should be space after text_end;
   we will NOT get a SIGSEGV or scribble on data space.  */
...
/* How to translate register numbers in the .stab's into gdb's internal register
   numbers.  We do NOT translate them, but we warn if an invalid register
