...
void frame_to_registers(struct StackFrame *frame, char *regs)
{
  /* why not just write "128" instead of "(4 * 32)"? Same thing with "8"
   * instead "(4 * 2)", and "4" instead of "(4 * 1)"... */
  mem2hex(&frame->ExceptionState.CsavedRegs, &regs[(GP0_REGNUM * 4 * 2)],
	  (4 * 32), 0);

  mem2hex(&frame->ExceptionState.CSavedFPRegs, &regs[(FP0_REGNUM * 4 * 2)],
	  (4 * 32), 0);

  mem2hex(&frame->ExceptionPC, &regs[(PC_REGNUM * 4 * 2)], (4 * 1), 0);

  mem2hex(&frame->ExceptionState.u.SpecialRegistersEnumerated.CsavedSRR1,
	  &regs[(CR_REGNUM * 4 * 2)], (4 * 1), 0);
  mem2hex(&frame->ExceptionState.u.SpecialRegistersEnumerated.CsavedLR,
	  &regs[(LR_REGNUM * 4 * 2)], (4 * 1), 0);
  mem2hex(&frame->ExceptionState.u.SpecialRegistersEnumerated.CsavedCTR,
	  &regs[(CTR_REGNUM * 4 * 2)], (4 * 1), 0);
  mem2hex(&frame->ExceptionState.u.SpecialRegistersEnumerated.CsavedXER,
	  &regs[(XER_REGNUM * 4 * 2)], (4 * 1), 0);
  mem2hex(&frame->ExceptionState.u.SpecialRegistersEnumerated.CsavedMQ,
	  &regs[(MQ_REGNUM * 4 * 2)], (4 * 1), 0);
}

/* Put the registers back into the frame information: */
void registers_to_frame(char *regs, struct StackFrame *frame)
{
  hex2mem(&regs[(GP0_REGNUM * 4 * 2)], &frame->ExceptionState.CsavedRegs,
	  (4 * 32), 0);

  hex2mem(&regs[(FP0_REGNUM * 4 * 2)], &frame->ExceptionState.CSavedFPRegs,
	  (4 * 32), 0);

  hex2mem(&regs[(PC_REGNUM * 4 * 2)], &frame->ExceptionPC, (4 * 1), 0);

  hex2mem(&regs[(CR_REGNUM * 4 * 2)],
	  &frame->ExceptionState.u.SpecialRegistersEnumerated.CsavedSRR1,
	  (4 * 1), 0);
  hex2mem(&regs[(LR_REGNUM * 4 * 2)],
	  &frame->ExceptionState.u.SpecialRegistersEnumerated.CsavedLR,
	  (4 * 1), 0);
  hex2mem(&regs[(CTR_REGNUM * 4 * 2)],
	  &frame->ExceptionState.u.SpecialRegistersEnumerated.CsavedCTR,
	  (4 * 1), 0);
  hex2mem(&regs[(XER_REGNUM * 4 * 2)],
	  &frame->ExceptionState.u.SpecialRegistersEnumerated.CsavedXER,
	  (4 * 1), 0);
  hex2mem(&regs[(MQ_REGNUM * 4 * 2)],
	  &frame->ExceptionState.u.SpecialRegistersEnumerated.CsavedMQ,
	  (4 * 1), 0);
}
...
int mem_write(char *dst, char *src, int len)
{
  while (len-- && !mem_err) {
      set_char(dst++, *src++);
  }
...
void set_step_traps(struct StackFrame *frame)
{
  union inst inst;
  LONG *target;
  int opcode;
...
/* Remove step breakpoints. Returns non-0 if pc was at a step breakpoint,
 * 0 otherwise. This routine works even if there were no step breakpoints set */
int clear_step_traps(struct StackFrame *frame)
{
  int retcode;
  LONG *pc = (LONG *)frame->ExceptionPC;
...
void do_status(char *ptr, struct StackFrame *frame)
{
  int sigval;
