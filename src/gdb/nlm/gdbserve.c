...
/* stuff that might be missing in case none of the headers exist: */
#if !defined(LONG) && !defined(HAVE_DECL_LONG)
/* just a guess: */
# define LONG long
#endif /* !LONG && !HAVE_DECL_LONG */
...
#if !defined(BREAKPOINT_SIZE) && !defined(HAVE_DECL_BREAKPOINT_SIZE)
/* another arbitrarily made-up default: */
# define BREAKPOINT_SIZE 0
#endif /* !BREAKPOINT_SIZE && !HAVE_DECL_BREAKPOINT_SIZE */
...
/* Write a character to serial port. Returns 0 on failure, non-0 on success: */
static int putDebugChar(unsigned char c)
{
  int err;
  LONG put;
...
/* Scan for sequence $<data>#<checksum>. Return 0 on fail, non-0 on success: */
...
/* Send the packet in buffer. Returns 0 on failure, non-zero on success: */
...
/* These are separate functions so that they are so short and sweet that
 * the compiler will NOT save any registers (if there is a fault to mem_fault,
 * they will NOT get restored, so there better not be any saved). */
...
/* This function takes the 386 exception vector and attempts to translate
 * this number into a unix-compatible signal value: */
 ...
 * It is called whenever an exception occurs in the module being debugged: */
static LONG handle_exception(struct StackFrame *frame)
{
  int addr, length;
  char *ptr;
  static struct DBG_LoadDefinitionStructure *ldinfo = 0;
  static unsigned char first_insn[BREAKPOINT_SIZE]; /* The 1st instruction in the program.  */

#if 0
  /* According to some documentation from Novell, the bell sometimes
   * may be ringing at this point. This can be stopped on Netware 4 systems
   * by calling the undocumented StopBell() function: */
  StopBell();
#endif /* 0 */

  if (remote_debug) {
      ConsolePrintf("vector=%d: %s, pc=%08x, thread=%08x\r\n",
		    frame->ExceptionNumber, frame->ExceptionDescription,
		    frame->ExceptionPC, GetThreadID());
  }

  switch (frame->ExceptionNumber) {
    case START_NLM_EVENT:
      /* If the NLM just started, we record the module load info & thread ID,
       * and set a breakpoint at the first instruction in the program. */

      ldinfo = ((struct DBG_LoadDefinitionStructure *)
		frame->ExceptionErrorCode);
      memcpy(first_insn, ldinfo->LDInitializationProcedure, BREAKPOINT_SIZE);
    ...
    case KEYBOARD_BREAK_EVENT:
      /* Pass events on to the next debugger, in case it will handle them. */
      return RETURN_TO_NEXT_DEBUGGER;

    case 3:			/* Breakpoint */
      /* After we have reached the initial breakpoint, reset it: */
    ...
    default:
      /* At the moment, we do NOT care about most of the unusual NetWare
       * exceptions: */
    ...
    case 14:			/* Page fault */
      /* If we get a GP fault, a& mem_may_fault is set, & the instruction
       * pointer is near set_char or get_char, then we caused the fault
       * ourselves accessing an illegal memory location: */
      if (mem_may_fault
	  ...
	  /* Keep going. This will act as though it returned from set_char
	   * or get_char. The calling routine will check mem_err & do right. */
	  return RETURN_TO_PROGRAM;
      }
   ...
   * the range of the module we are debugging, but that does NOT help
	...
	case 'q':		/* Query message */
	  if (strcmp(&remcomInBuffer[1], "Offsets") == 0) {
	      sprintf(remcomOutBuffer, "Text=%x;Data=%x;Bss=%x",
		      ldinfo->LDCodeImageOffset, ldinfo->LDDataImageOffset,
		      (ldinfo->LDDataImageOffset + ldinfo->LDDataImageLength));
	  } else {
	      sprintf(remcomOutBuffer, "E04, Unknown query %s",
		      &remcomInBuffer[1]);
	  }
	  break;
      } /* end switch */
 ...
 * the named NLM module, and then goes to sleep. The serial I/O port is named
 * as a board number and a port number. It would be more DOS-like to provide
 * a menu of available serial ports, but I do NOT want to have to figure out
 * how to do that. */
int main(int argc, char **argv)
{
  ...
  if (argc == 0) {
      fprintf(stderr, "Usage: load %s [options] program [arguments]\n",
	      progname);
      exit(1);
  }
 ...
 Flow:%s\n",
  ...
  s.DDSResourceTag = ((struct ResourceTagStructure *)
		      AllocateResourceTag(GetNLMHandle(), (BYTE *)"gdbserver",
					  DebuggerSignature));
  ...
  /* Get the command line we were invoked with, and advance it past our name
   * and the board and port arguments: */
  ...
  /* If we are woken up, print an optional error message, deregister ourselves,
   * and then exit: */
