...
struct mi_cmd mi_cmds[] =
{
  ...
  { "break-list", { "info break", 0 }, NULL, NULL },
  ...
  { "exec-metrowerks-step", { NULL, 0 }, mi_cmd_exec_metrowerks_step, NULL },
  ...
  { "target-detach", { "detach", 0 }, 0, NULL },
  { "target-disconnect", { "disconnect", 0 }, 0, NULL },
...
lookup_table(const char *command)
{
  ...
  for (chp = command; *chp; chp++)
    {
      /* some what arbitrary: */
      uindex = (((uindex << 6) + (unsigned int)*chp) % MI_TABLE_SIZE);
    }
  /* look it up */
  while (1)
    {
      struct mi_cmd **entry = &mi_table[uindex];
      if ((*entry) == 0)
	{
	  /* not found, so return pointer to next free: */
	  stats.miss++;
	  return entry;
	}
...
build_table(struct mi_cmd *commands)
{
  ...
  size_t sizeof_table = (sizeof(struct mi_cmd **) * MI_TABLE_SIZE);

  mi_table = (struct mi_cmd **)xmalloc(sizeof_table);
