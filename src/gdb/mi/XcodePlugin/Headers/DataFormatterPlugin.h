...
/*
 Xcode supports customizing the information displayed in the debugger data
 view. Developers can enter simple format strings which describe what to
 display in the variable value and summary columns on a per-type basis. These
 format strings are described below. Additionally, developers can write bundles
 which are loaded into the debugged process. This allows developers to write
 their own "print for debugger" functions which can be invoked via the format
 strings.
 ...
 - $PARENT(type) - replaced with an expression for the parent of the variable
   which is of the given type. This is only useful for a very limited
   circumstance, where values of a particular type are always contained in a
   structure of known type, and information from the containing structure is
   required to interpret the value. For example, an "event" structure might
   contain an "eventType" code, and an "eventResult" value, where the
   eventResult values are overloaded for different eventTypes. This construct
   provides a mechanism for the eventResult formatter to obtain the containing
   event to access the eventType.
 ...
 well-known directories for .plist files and bundles which contain custom
 format strings. These are (in highest to lowest precedence):
 ...
 If there are multiple format strings defined for the same type, the one with
 the highest precedence is used. At present there is no mechanism for selecting
 from among multiple possible formatters for a particular variable.
 ...
 & Carbon data types installed in ${DEVELOPER_DIR}/Library/Xcode/CustomDataViews.
 ...
 can be entered directly into the GUI by double clicking on the value or summary
 columns. This will store the required entries into the user's customized plist
 (~/Library/Application Support/Developer/Shared/Xcode/CustomDataViews). The
 appropriate entries can be copied to a .plist file with a different name in any
 of the above locations and it will be loaded the next time Xcode is restarted.

 However, for some types there is no combination of simple expressions which can
 construct a suitable description. For these types there is support for writing
 custom code to construct the descriptions. Developers can create a bundle which
 contains functions they write themselves to construct display strings. The
 bundle must also contain a file resource named CustomDataViews.plist, which is
 the plist containing format strings. Whenever a formatter string from the
 bundle's .plist is used, Xcode ensures that the bundle code is loaded into the
 process being debugged. The expressions in the format strings can contain calls
 to functions in the bundle. The functions you write should be as lightweight as
 possible because they will be evaluated after every step in the debugger.
 Additionally they should not modify any global state in the process. And they
 must be carefully written to avoid causing problems when presented with
 uninitialized variables. gdb can typically handle the case where a function
 called from an expression causes a crash, but it is good to be as prudent as
 possible.
 ...
 loaded the loading code looks up this symbol by name and fills in a value. This
 allows the plugin to use utility functions which are already present in the
 debugging environment without requiring that the linker be informed of them.
 These functions are described below. (Note that bundle initializer functions
 are called before this value is filled in.)
 */
...
/*
 ...
 buffer needs to be valid. Therefore, the Xcode debugging environment provides a
 special memory allocator. This allows Xcode to manage the memory allocated by a
 plugin.
 The value for the identifier parameter is supplied by the expression parser in
 Project Builder using "$ID". This value should be passed to the allocator. Only
 one memory allocation is allowed per identifier. If a buffer for the identifier
 already exists, the existing buffer will be reused if it is large enough.
 Otherwise the old buffer is freed and a new, larger one is allocated.
 The size parameter specifies the minimum size in bytes that the returned buffer
...
/*
 ...
 _pbxgdb_allocator function, then constructs a character string in the allocated
 buffer using a printf() style format string and vararg list.
 The value for the identifier parameter is supplied by the expression parser in
 Project Builder using "$ID". This value is passed to the allocator.
...
/*
 ...
 functions in a plugin bundle. Functions may be added to this list iin future
