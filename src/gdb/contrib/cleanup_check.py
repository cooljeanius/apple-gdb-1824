# (is the shebang missing on purpose?)
#   cleanup_check.py
...
def is_constructor(decl):
    ...
    return is_cleanup_type(decl.type.type) and (
        not decl.name or str(decl.name) != "make_final_cleanup"
    )
...
class CleanupChecker:
    ...
    def compute_master(self, bb, bb_from, master_cleanup):
        ...
        for stmt in bb.gimple:
            ...
            if isinstance(stmt, gcc.GimpleCall) and stmt.fndecl:
                ...
                if isinstance(stmt.lhs, gcc.VarDecl) and isinstance(
                    stmt.rhs[0], gcc.VarDecl
                ):
    ...
    def check_cleanups(self):
        ...
        self.is_special_constructor = (
            not self.is_constructor
            and str(self.fun.decl.name).find("with_cleanup") > -1
        )
        # Yuck.
