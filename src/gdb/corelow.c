...
int ATTRIBUTE_CONST
default_check_format(bfd *abfd ATTRIBUTE_UNUSED)
{
  return (0);
}
...
core_open(const char *filename, int from_tty)
{
  ...
  core_bfd = temp_bfd;
  old_chain = make_cleanup(core_close_cleanup, (void *)0);
  ...
  if (build_section_table(core_bfd, &core_ops.to_sections,
    ...
    error(_("\"%s\": Cannot find sections: %s"),
...
static void
get_core_register_section(const char *name, int which,
			  const char *human_name, int required)
{
  ...
  if (! section)
    {
      if (required)
	warning(_("Failed to find %s registers in core file."), human_name);
      return;
    }
  ...
  if (! bfd_get_section_contents(core_bfd, section, contents,
				 (file_ptr)0L, size))
    {
      warning(_("Failed to read %s registers from `%s' section in core file."),
	      human_name, name);
      return;
    }
  ...
  if (core_gdbarch && gdbarch_regset_from_core_section_p(core_gdbarch))
	...
	{
	  if (required)
	    warning(_("Failed to recognize %s registers in core file."),
		    human_name);
	  return;
	}
...
/* We just get all the registers, so we do NOT use regno: */
static void
  ...
  if (!(core_gdbarch && gdbarch_regset_from_core_section_p(core_gdbarch))
      && ((core_vec == NULL) || (core_vec->core_read_registers == NULL)))
    {
      fprintf_filtered(gdb_stderr,
		       "Cannot fetch registers from this type of core file\n");
      return;
    }
  ...
  switch (object)
      ...
      if (readbuf)
	{
	  ...
	  if (offset >= size)
	    return 0;
	  size -= offset;
	  if (size > (bfd_size_type)len)
	    size = len;
	  if ((size > 0UL)
	      && !bfd_get_section_contents(core_bfd, section, readbuf,
					   (file_ptr)offset, size))
	    {
	      warning(_("Failed to read NT_AUXV note in core file."));
	      return -1;
	    }
      ...
      if (readbuf)
	{
	  ...
	  if ((size > 0UL)
	      && !bfd_get_section_contents(core_bfd, section, readbuf,
					   (file_ptr)offset, size))
	    {
	      warning(_("Failed to read StackGhost cookie in core file."));
	      return -1;
	    }
