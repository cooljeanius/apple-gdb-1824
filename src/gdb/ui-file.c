/* ui-file.c: UI_FILE - a generic STDIO like output stream.
...
ui_file_new(void)
{
  struct ui_file *file = (struct ui_file *)xmalloc(sizeof(struct ui_file));
...
null_file_write(struct ui_file *file, const char *buf, long sizeof_buf)
{
  if (file->to_fputs == null_file_fputs)
    ...
    {
      /* The fputs method is NOT null, so slowly pass the write request
         onto that.  FYI, this is NOT as bad as it may look - the
...
null_file_fputs(const char *buf, struct ui_file *file)
{
  if (file->to_write == null_file_write)
    ...
    {
      /* The write method was implemented, so use that: */
...
do_ui_file_xstrdup(void *context, const char *buffer, long length)
{
  ...
  if (acc->buffer == NULL)
    acc->buffer = (char *)xmalloc(length + 1UL);
  else
               ...
               long length_buffer)
{
  ...
  if (stream->buffer == NULL)
      ...
      if (new_length >= stream->sizeof_buffer)
	{
      ...
      memcpy((stream->buffer + stream->length_buffer), buffer,
             length_buffer);
...
stdio_file_new(FILE *file, int close_p)
{
  ...
  struct stdio_file *stdio;
  stdio = (struct stdio_file *)xmalloc(sizeof(struct stdio_file));
	     ...
	     struct ui_file *two, int close_two)
{
  struct ui_file *ui_file = ui_file_new();
  struct tee_file *tee;
  tee = (struct tee_file *)xmalloc(sizeof(struct tee_file));
