...
/* Descriptor for I/O to remote machine. Initialize it to -1 so that
   eb_open knows that we do NOT have a file open when the program
   starts.  */
int eb_desc = -1;
...
eb_create_inferior (char *execfile, char *args, char **env)
{
  ...
  if (args && *args)
    error ("Cannot pass arguments to remote EBMON process");
...
eb_read_inferior_memory (CORE_ADDR memaddr, char *myaddr, int len)
{
  ...
  /* However, something like
     ...
     does NOT need to work.  Detect it and give up if there is an attempt
     to do that.  */
