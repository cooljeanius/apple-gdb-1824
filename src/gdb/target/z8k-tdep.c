/* z8k-tdep.c: Target-machine dependent code for Zilog Z8000, for GDB.
...
static CORE_ADDR examine_frame(CORE_ADDR, CORE_ADDR *, CORE_ADDR);
...
NEXT_PROLOGUE_INSN(CORE_ADDR addr, CORE_ADDR lim, short *pword1)
{
  ...
  if (addr < (lim + 8))
    {
      read_memory(addr, (gdb_byte *)buf, 2);
      *pword1 = extract_signed_integer((const gdb_byte *)buf, 2);
...
#if defined(HAVE_MEMSET) && defined(FRAME_FP) && defined(IS_PUSHL) && \
    defined(IS_PUSHW) && defined(PC_REGNUM) && defined(FP_REGNUM)
/* Put here the code to store, into a struct frame_saved_regs,
   the addresses of the saved registers of frame described by FRAME_INFO.
   This includes special registers such as pc and fp saved in special
   ways in the stack frame.  sp is even more special:
...
#endif /* HAVE_MEMSET && FRAME_FP && IS_PUSHL && IS_PUSHW && PC_REGNUM && FP_REGNUM */

/* FIXME: arbitrary; found elsewhere in an unrelated file: */
#ifndef PTR_SIZE
# define PTR_SIZE 4
#endif /* !PTR_SIZE */
...
{
  return ADDR_BITS_REMOVE(read_memory_integer(read_register(SP_REGNUM),
					      PTR_SIZE));
}
...
/* */
static void
segmented_command(const char *args ATTRIBUTE_UNUSED,
...
/* */
static void
unsegmented_command(const char *args ATTRIBUTE_UNUSED,
		    int from_tty ATTRIBUTE_UNUSED)
{
...
/* */
static void
set_memory(const char *args ATTRIBUTE_UNUSED, int from_tty ATTRIBUTE_UNUSED)
{
  printf_unfiltered("\"set memory\" must be followed by the name of a memory subcommand.\n");
  help_list(setmemorylist, "set memory ", (enum command_class)-1, gdb_stdout);
}

/* Usual gdb initialization hook: */
extern void _initialize_z8ktdep(void); /* -Wmissing-prototypes */
void
