...
/* Prototypes */
extern int gould_frame_chain_valid(CORE_ADDR, struct frame_info *);
extern int gould_use_struct_convention(int, struct type *);
extern int gould_print_insn(CORE_ADDR, FILE *);
extern int findarg(struct frame_info *);
extern CORE_ADDR findframe(struct frame_info *);
extern CORE_ADDR framechain(register struct frame_info *);

/* Functions */
int
gould_frame_chain_valid(CORE_ADDR chain, struct frame_info *fi ATTRIBUTE_UNUSED)
{
...
int
gould_use_struct_convention(int gcc_p, struct type *type)
{
...
int
gould_print_insn(CORE_ADDR memaddr, FILE *stream)
{
 ...
 */
CORE_ADDR
findframe(struct frame_info *thisframe)
{
 ...
 */
CORE_ADDR
framechain(register struct frame_info *frame)
{
    ...
    if (!frame->frame) {
#if defined(SP_REGNUM) && defined(HAVE_READ_REGISTER)
	prevsp = read_register(SP_REGNUM);
#else
	prevsp = 0;
#endif /* SP_REGNUM && HAVE_READ_REGISTER */
    } else
	prevsp = frame->frame;
