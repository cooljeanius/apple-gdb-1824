/* i386-obsd-tdep.c: Target-dependent code for OpenBSD/i386.
...
i386obsd_sigtramp_p (struct frame_info *next_frame)
{
  ...
  /* If the function has a valid symbol name, then it is NOT a trampoline: */
  find_pc_partial_function(pc, &name, NULL, NULL);
  if (name != NULL)
    return 0;

  /* If the function lives in a valid section (even without a starting point),
   * then it is NOT a trampoline: */
...
i386obsd_supply_uthread(struct regcache *regcache, int regnum, CORE_ADDR addr)
{
  ...
  for (i = 0; i < (int)ARRAY_SIZE(i386obsd_uthread_reg_offset); i++)
    {
  ...
  for (i = 0; i < (int)ARRAY_SIZE(i386obsd_uthread_reg_offset); i++)
    {
	  ...
	  && ((regnum == -1) || (regnum == i)))
	{
	  /* Fetch stack pointer from thread structure (if we did NOT calculate
	   * it already): */
	  if (sp == 0)
	    sp = read_memory_unsigned_integer(sp_addr, 4);
...
i386obsd_init_abi(struct gdbarch_info info, struct gdbarch *gdbarch)
{
  ...
  /* OpenBSD has a `struct sigcontext' that is different from the original
   * 4.3 BSD: */
