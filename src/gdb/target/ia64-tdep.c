...
#define SIGCONTEXT_REGISTER_ADDRESS \
  (new_gdbarch_tdep(current_gdbarch)->sigcontext_register_address)
...
static int
floatformat_valid(const struct floatformat *fmt ATTRIBUTE_UNUSED,
...
static long long
extract_bit_field(gdb_byte *bundle, int from, int len)
{
  long long result = 0LL;
  int to = from + len;
  int from_byte = from / 8;
  int to_byte = to / 8;
  unsigned char *b = bundle;
  unsigned char c;
  int lshift;
  int i;
...
fetch_instruction (CORE_ADDR addr, instruction_type *it, long long *instr)
{
  ...
  if (slotnum > 2)
    {
      warning(_("Cannot fetch instructions for slot numbers greater than 2.\n"
  ...
  *instr = slotN_contents(bundle, slotnum);
  the_template = extract_bit_field(bundle, 0, 5);
  *it = template_encoding_table[(int)the_template][slotnum];
...
ia64_memory_insert_breakpoint (CORE_ADDR addr, bfd_byte *contents_cache)
{
  ...
  if (slotnum > 2)
    error(_("Cannot insert breakpoint for slot numbers greater than 2."));
  ...
  the_template = extract_bit_field (bundle, 0, 5);
  if (slotnum == 1 && template_encoding_table[the_template][1] == L)
    {
      slotnum = 2;
    }
                           ...
                           int regnum, gdb_byte *buf)
{
  if (regnum >= V32_REGNUM && regnum <= V127_REGNUM)
      ...
      if (gr_addr != 0)
	{
      ...
      store_unsigned_integer(buf, register_size(current_gdbarch, regnum),
			     natN_val);
    }
			   ...
			   int regnum, const gdb_byte *buf)
{
  if ((regnum >= V32_REGNUM) && (regnum <= V127_REGNUM))
  ...
  else if ((IA64_NAT0_REGNUM <= regnum) && (regnum <= IA64_NAT31_REGNUM))
    {
      ...
      regcache_cooked_read_unsigned(regcache, IA64_UNAT_REGNUM, &unat);
      unatN_val = extract_unsigned_integer(buf, register_size(current_gdbarch,
							      regnum)); 
	...
	gr_addr = rse_address_add(bsp, (regnum - V32_REGNUM));
      ...
      natN_val = extract_unsigned_integer(buf, register_size(current_gdbarch,
							     regnum)); 
...
static CORE_ADDR
ia64_refine_prologue_limit(CORE_ADDR pc, CORE_ADDR lim_pc, int *trust_limit)
{
...
examine_prologue (CORE_ADDR pc, CORE_ADDR lim_pc, struct frame_info *next_frame, struct ia64_frame_cache *cache)
{
  ...
  lim_pc = ia64_refine_prologue_limit(pc, lim_pc, &trust_limit);
			  ...
			  int *realnump, gdb_byte *valuep)
{
  struct ia64_frame_cache *cache =
    ia64_frame_cache (next_frame, this_cache);
  gdb_byte dummy_valp[MAX_REGISTER_SIZE];
  gdb_byte buf[8];
  ...
  *realnump = -1;
  ...
  if (regnum == SP_REGNUM)
    ...
    {
      gdb_byte cfm_valuep[MAX_REGISTER_SIZE];
      enum opt_state cfm_optim;
      int cfm_realnum;
    ...
    {
      gdb_byte pr_valuep[MAX_REGISTER_SIZE];
      enum opt_state pr_optim;
      int pr_realnum;
      enum lval_type pr_lval;
      CORE_ADDR pr_addr;
      ULONGEST prN_val;
      ia64_frame_prev_register(next_frame, this_cache, IA64_PR_REGNUM,
			       &pr_optim, &pr_lval, &pr_addr, &pr_realnum,
			       pr_valuep);
      if (VP16_REGNUM <= regnum && regnum <= VP63_REGNUM)
	{
	  /* Fetch predicate register rename base from current frame
      ...
      prN_val = extract_bit_field(pr_valuep, (regnum - VP0_REGNUM), 1);
      store_unsigned_integer(valuep, register_size(current_gdbarch, regnum),
			     prN_val);
    }
  else if (IA64_NAT0_REGNUM <= regnum && regnum <= IA64_NAT31_REGNUM)
    {
      gdb_byte unat_valuep[MAX_REGISTER_SIZE];
      enum opt_state unat_optim;
      int unat_realnum;
      ...
      unatN_val = extract_bit_field(unat_valuep, (regnum - IA64_NAT0_REGNUM),
				    1);
      store_unsigned_integer (valuep, register_size (current_gdbarch, regnum), 
                              unatN_val);
    }
      ...
      if (addr != 0)
	{
	  ...
	  *addrp = addr;
	  read_memory (addr, valuep, register_size (current_gdbarch, regnum));
        ...
        {
	  gdb_byte r_valuep[MAX_REGISTER_SIZE];
	  enum opt_state r_optim;
	  int r_realnum;
...
ia64_gdb2uw_regnum (int regnum)
{
  if (regnum == sp_regnum)
    ...
    return UNW_IA64_BSP;
  else if ((unsigned int) (regnum - IA64_GR0_REGNUM) < 128)
    return UNW_IA64_GR + (regnum - IA64_GR0_REGNUM);
  else if ((unsigned int) (regnum - V32_REGNUM) < 95)
    return UNW_IA64_GR + 32 + (regnum - V32_REGNUM);
  else if ((unsigned int) (regnum - IA64_FR0_REGNUM) < 128)
    return UNW_IA64_FR + (regnum - IA64_FR0_REGNUM);
  else if ((unsigned int) (regnum - IA64_PR0_REGNUM) < 64)
    return -1;
  else if ((unsigned int) (regnum - IA64_BR0_REGNUM) < 8)
  ...
  else if (regnum == lr_regnum)
    return -1;
  else if (regnum == fp_regnum)
    return -1;
  else
    return -1;
}
...
CORE_ADDR ATTRIBUTE_NORETURN
ia64_extract_struct_value_address(struct regcache *regcache ATTRIBUTE_UNUSED)
{
		      ...
		      int struct_return, CORE_ADDR struct_addr)
{
  ...
  int floatreg;
  CORE_ADDR bsp, cfm, pfs, new_bsp, funcdescaddr, global_pointer;
  ...
  for (argno = 0; argno < nargs; argno++)
	  ...
	  && (TYPE_CODE(TYPE_TARGET_TYPE(type)) == TYPE_CODE_FUNC))
	{
	  ...
	  store_unsigned_integer((gdb_byte *)val_buf, 8,
				 find_func_descr(extract_unsigned_integer(value_contents(arg), 8),
						 &funcdescaddr));
	  if (slotnum < rseslots)
	    write_memory(rse_address_add(bsp, slotnum),
			 (const gdb_byte *)val_buf, 8);
	  else
	    write_memory((sp + 16 + (8 * (slotnum - rseslots))),
			 (const gdb_byte *)val_buf, 8);
	  slotnum++;
	  continue;
	}
	  ...
	  while ((len > 0) && (floatreg < IA64_FR16_REGNUM))
	    {
	      ...
	      convert_typed_floating((value_contents(arg) + argoffset),
				     float_elt_type, to, builtin_type_ia64_ext);
	      regcache_cooked_write(regcache, floatreg, (const gdb_byte *)to);
	      floatreg++;
...
ia64_unwind_dummy_id(struct gdbarch *gdbarch, struct frame_info *next_frame)
{
  ...
  frame_unwind_register(next_frame, sp_regnum, (gdb_byte *)buf);
  sp = extract_unsigned_integer((const gdb_byte *)buf, 8);

  frame_unwind_register(next_frame, IA64_BSP_REGNUM, (gdb_byte *)buf);
  bsp = extract_unsigned_integer((const gdb_byte *)buf, 8);
...
ia64_unwind_pc(struct gdbarch *gdbarch, struct frame_info *next_frame)
{
  ...
  frame_unwind_register(next_frame, IA64_IP_REGNUM, (gdb_byte *)buf);
  ip = extract_unsigned_integer((const gdb_byte *)buf, 8);
  frame_unwind_register(next_frame, IA64_PSR_REGNUM, (gdb_byte *)buf);
  psr = extract_unsigned_integer((const gdb_byte *)buf, 8);
  ...
  if (TYPE_CODE(type) == TYPE_CODE_FLT)
    {
      ...
      regcache_cooked_write(regcache, IA64_FR8_REGNUM, (const gdb_byte *)to);
