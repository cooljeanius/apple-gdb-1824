...
/* *INDENT-OFF* */
const char *mcore_register_names[] =
...
mcore_breakpoint_from_pc(CORE_ADDR *bp_addr, int *bp_size)
{
  ...
  {0x00, 0x00};
  *bp_size = 2;
  return (unsigned char *)breakpoint;
}
...
mcore_analyze_prologue (struct frame_info *fi, CORE_ADDR pc, int skip_prologue)
{
  ...
  status = find_pc_partial_function(pc, (const char **)&name, &func_addr,
				    &func_end);
  ...
  for (addr = func_addr; addr < stop; addr += 2)
	       ...
	       || IS_BGENI1 (insn) || IS_BMASKI1 (insn))
	{
	  ...
	  if (IS_LRW1 (insn))
	    {
	      adjust =
		(int)read_memory_integer(((addr + 2 + ((insn & 0xff) << 2))
					  & 0xfffffffc), 4);
	    }
...
#ifndef read_memory_nobpt
# define read_memory_nobpt deprecated_read_memory_nobpt
#endif /* !read_memory_nobpt */
...
{
  char buf[4];
  int status = read_memory_nobpt(pc, (gdb_byte *)buf, 2);
  if (status != 0)
    return 0;

  return (int)extract_unsigned_integer((const gdb_byte *)buf, 2);
}

extern int print_insn_mcore(bfd_vma, disassemble_info *);

extern void _initialize_mcore_tdep(void); /* -Wmissing-prototypes */
void
