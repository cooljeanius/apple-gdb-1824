...
/* This is like remote.c but uses the Universal Debug Interface (UDI) to
   ...
   protocol; for hardware that does NOT run TCP, an interface adapter
 ...
 * udi_open knows that we do NOT have a file open when the program
 * starts.  */
...
udi_create_inferior (char *execfile, char *args, char **env)
{
  ...
  if (execfile[0] == '\0')
     ...
     * (sounds pretty bogus to me...). In fact it does NOT work (with
...
#endif /* 0 */
  /* But if we are going to want to run it again, we better remove the
   * breakpoints...  */
  remove_breakpoints ();
...
udi_open (char *name, int from_tty)
{
  ...
  /* Print out some stuff, letting the user now what is/was going on */
	    ...
	    int quitting)
{
  ...
  /* We should never get here if there is NOT something valid in
   * udi_session_id.  */
...
udi_detach (char *args, int from_tty)
{
  ...
  /* Do NOT try to UDIDisconnect it again in udi_close, which is called from
   * pop_target.  */
  udi_session_id = -1;
  inferior_ptid = null_ptid;
  ...
  while (1)
	...
	case UDIStdoutReady:
	  if (UDIGetStdout (sbuf, (UDISizeT) SBUF_MAX, &CountDone))
	     ...
	     * guess). It does NOT seem to happen with the simulator.  */
...
translate_addr (CORE_ADDR addr)
{
  ...
  /* FIXME: does NOT work for user virtual addresses */
  if (addr >= UVADDR)
    {
...
udi_remove_breakpoint (CORE_ADDR addr, char *contents_cache)
{
  ...
  if (cnt >= BKPT_TABLE_SIZE)
    error ("Cannot find breakpoint in table");
...
#if 0
/*
 * UDIStop does not really work as advertised. It causes the TIP to close its
 ...
 * FIXME: Cannot distinguish I_ROM/D_ROM.
...
/* Target_ops vector. Not static because there does not seem to be
   ...
   The RS/6000 does NOT like "extern" followed by "static"; SunOS
   /bin/cc does NOT like "static" twice.  */
