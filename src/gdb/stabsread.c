...
#if defined(HAVE_ALLOCA_H) && !defined(_ALLOCA_H_)
# include <alloca.h>
#endif /* HAVE_ALLOCA_H && !_ALLOCA_H_ */

#include <ctype.h>

#if defined(HAVE_STRING_H) && !defined(_STRING_H_)
# include <string.h>
#endif /* HAVE_STRING_H && !_STRING_H_ */
...
#define STABS_CONTINUE(pp,objfile)				\
  do {							\
    if ((*(pp) != NULL) && (**(pp) == '\\' || (**(pp) == '?' && (*(pp))[1] == '\0'))) \
      *(pp) = (char *)next_symbol_text(objfile);	\
  } while (0)

		  ...
		  struct objfile *objfile)
{
  ...
  if (stabs)
	...
	{
	  name = stabs->stab[ii];
	  pp = (const char *)strchr(name, ':');
	  while (pp[1] == ':')
	    {
	      pp += 2;
	      pp = (const char *)strchr(pp, ':');
	    }
...
void
process_symbol_types_only(const char *string, const char *prefix,
			  int desc, int type, struct objfile *objfile)
{
  const char *p = (const char *)find_name_end(string);
  int deftype;
...
define_symbol(CORE_ADDR valu, const char *string, const char *prefix,
              int desc, int type, struct objfile *objfile)
{
  struct symbol *sym;
  const char *p = (const char *)find_name_end(string);
...
{
  complaint(&symfile_complaints,
	    _("failed to parse type; debugger out of date?"));
  while (1)
    {
	  ...
	  *pp = p + 1;
	  ...
	  switch (*(*pp)++)
	    ...
	    case '?':
	      ...
	      {
		size_t slen = strlen(main_fn_name);
		   ...
		   struct objfile *objfile)
{
  ...
  while ((**pp != ';') && (**pp != '\0'))
    {
      ...
      newf = (struct nextfield *)xmalloc(sizeof(struct nextfield));
      make_cleanup(xfree, newf);
      memset(newf, 0, sizeof(struct nextfield));
      newf->next = fip->list;
      fip->list = newf;
  ...
  if ((p[0] == ':') && (p[1] == ':'))
    {
      /* The list of fields for (the deleted) Chill: the last entry (at
		 ...
		 struct objfile *objfile)
{
  ...
  for (i = 0; i < TYPE_N_BASECLASSES(type); i++)
    {
      newfield = (struct nextfield *)xmalloc(sizeof(struct nextfield));
      make_cleanup(xfree, newfield);
      memset(newfield, 0, sizeof(struct nextfield));
      newfield->next = fip->list;
      fip->list = newfield;
	...
	default:
      ...
      newfield->visibility = *(*pp)++;
      switch (newfield->visibility)
	{
	case VISIBILITY_PRIVATE:
	case VISIBILITY_PROTECTED:
	...
	default:
	     ...
	     public.  */
	  {
	    complaint(&symfile_complaints,
		      ...
		      newfield->visibility);
	    newfield->visibility = VISIBILITY_PUBLIC;
	  }
	}
	...
	FIELD_BITPOS(newfield->field) = read_huge_number(pp, ',',
                                                         &nbits, 0);
	if (nbits != 0)
	  return 0;
      }
      ...
      newfield->field.type = read_type(pp, objfile);
      newfield->field.name = type_name_no_tag(newfield->field.type);
...
static long
read_huge_number(const char **pp, int end, int *bits, int twos_complement_bits)
{
  const char *p = *pp;
  int sign = 1;
  int sign_bit;
  ...
  *pp = p;
  ...
  /* It is *BITS which has the interesting information.  */
  return 0;
}
...
common_block_end (struct objfile *objfile)
{
  ...
  int i;
  struct symbol *sym;
  struct pending *newly_pending = 0;
  struct pending *next;
  int j;
  ...
  for (next = local_symbols;
	...
	add_symbol_to_list(next->symbol[j], &newly_pending);
    }
  ...
  if (common_block != NULL)
    for (j = common_block_i; j < common_block->nsyms; j++)
      add_symbol_to_list(common_block->symbol[j], &newly_pending);

  SYMBOL_TYPE(sym) = (struct type *)newly_pending;
...
find_name_end(const char *name)
{
  ...
  /* It is tempting to use strchr to look for the leftmost lbrac, but that
   * would mean scanning the whole stab string, which can be quite long.
   * Since we only care whether there is a left square bracket BEFORE the first
   * colon, restrict the search to that.  */
