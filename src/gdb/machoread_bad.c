...
/* This file is from a newer version of gdb than what I started with
 * for my base files, so ifdef out errors */
#if 0
		   ...
		   long number_of_symbols, asymbol **symbol_table)
{
  ...
  for (i = 0; i < number_of_symbols; i++)
	...
	{
	  /* Skip names that do NOT exist (should NOT happen), or names
  ...
  for (ix = 0; VEC_iterate (oso_el, vec, ix, oso); ix++)
	...
	if (oso->symbols[i] != NULL)
	  {
		...
		if (msym == NULL)
		  {
		    warning (_("cannot find symbol '%s' in minsymtab"),
			     oso->symbols[i]->name);
      ...
      if (pfx_len > 0)
	{
	  ...
	  if (!bfd_check_format (member_bfd, bfd_object))
	    {
	      warning (_("`%s': cannot read symbols: %s."), oso->name,
      ...
      else
	{
	  ...
	  if (!abfd)
	    {
	      warning (_("`%s': cannot open to read symbols: %s."), oso->name,
		       bfd_errmsg (bfd_get_error ()));
	      continue;
	    }
...
macho_check_dsym (struct objfile *objfile)
{
  ...
  if (sect == NULL)
    {
      warning (_("cannot find UUID in %s"), objfile->name);
      return NULL;
    }
  if (!bfd_get_section_contents (objfile->obfd, sect, main_uuid,
				 0, sizeof (main_uuid)))
    {
      warning (_("cannot read UUID in %s"), objfile->name);
      return NULL;
    }
  ...
  if (dsym_bfd == NULL)
    {
      warning (_("cannot open dsym file %s"), dsym_filename);
      xfree (dsym_filename);
      return NULL;
    }
  ...
  if (sect == NULL)
    {
      warning (_("cannot find UUID in %s"), dsym_filename);
      bfd_close (dsym_bfd);
      xfree (dsym_filename);
      return NULL;
    }
  if (!bfd_get_section_contents (dsym_bfd, sect, dsym_uuid,
				 0, sizeof (dsym_uuid)))
    {
      warning (_("cannot read UUID in %s"), dsym_filename);
      bfd_close (dsym_bfd);
      xfree (dsym_filename);
      return NULL;
    }
  if (memcmp (dsym_uuid, main_uuid, sizeof (main_uuid)))
    {
      warning (_("dsym file UUID does not match the one in %s"), objfile->name);
      bfd_close (dsym_bfd);
      xfree (dsym_filename);
      return NULL;
    }
  ...
  if (bfd_get_file_flags (abfd) & (EXEC_P | DYNAMIC))
    {
      ...
      if (storage_needed < 0)
	error (_("Cannot read symbols from %s: %s"),
      ...
      if (dsym_bfd != NULL)
	{
	  ...
	  /* Remove oso. They will NOT be used.  */
	  ...
	  /* Do NOT try to read dwarf2 from main file or shared libraries.  */
	  return;
	}
    }
                       ...
                       struct section_addr_info *addrs)
{
  ...
  /* This code is run when we first add the objfile with
     ...
     N.B. if an objfile slides after we have already created it, then it
...
/* if-ing this function out will lead to missing symbols for init.c, but the
 * alternative is having it error out now, so have it error out then instead
 * of now. */
#if 0
void
