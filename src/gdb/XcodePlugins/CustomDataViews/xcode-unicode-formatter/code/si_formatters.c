...
char *
formatCoreString(long *input, int identifier)
{
  if (!input)
  ...
  CFStringRef string = CFStringCreateWithBytes(NULL, (UInt8 *) actual_buffer,
					       length * 2,
					       kCFStringEncodingUTF16LE,
					       false);
  char *result = ConvertStringToEncoding(string, kCFStringEncodingUTF8,
					 identifier);
  CFRelease(string);
