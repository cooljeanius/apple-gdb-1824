...
char *normal_pid_to_str PARAMS ((int pid));
...
static char *
debug_to_pid_to_str PARAMS ((int));
...
#define de_fault(field, value) \
  ...
  de_fault (to_pid_to_str,		(char * (*) PARAMS((int))) normal_pid_to_str);
...
/* Given a name, return its signal.  */
  ...
  target_signal_from_name(char *name)
  {
    ...
    for (sig = TARGET_SIGNAL_HUP; signals[sig].name != NULL;
...
/* Convert host signal to our signals.  */
  ...
  target_signal_from_host(int hostsig)
  {
    ...
    if (hostsig >= REALTIME_LO && hostsig < REALTIME_HI)
      return (enum target_signal)(hostsig - 33
...
# endif
	...
	if (oursig >= TARGET_SIGNAL_REALTIME_33
	  ...
	  {
	    int retsig = (int) oursig - (int) TARGET_SIGNAL_REALTIME_33
			 + REALTIME_LO;
...
static char *
debug_to_pid_to_str(int pid)
{
  char *retval;

  retval = debug_target.to_pid_to_str (pid);

  fprintf_unfiltered (stderr, "target_thread_alive (%d) = %s\n", pid, retval);
