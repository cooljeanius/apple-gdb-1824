/* value.c: Low level packing and unpacking of values for GDB, the GNU Debugger.
...
struct value
{
  ...
  /* If a value represents a C++ object, then the `type' field gives
     ...
     the pointer and fetched the complete object.  (I do NOT really see
     the point.  Why not just determine the run-time type when you
     indirect, and avoid the special case?  The contents do NOT matter
     until you indirect anyway.)

     If we are/were not doing anything fancy, `enclosing_type' is equal to
  ...
  /* If zero, contents of this value are in the contents field.  If
     ...
     the chain does NOT need to be watched as part of watching the
...
/* */
enum lval_type * ATTRIBUTE_CONST
...
/* */
struct internalvar ** ATTRIBUTE_CONST
...
/* Return a copy of the value ARG.
   ...
   but it is/was a different block of storage.  */
...
record_latest_value (struct value *val)
{
  ...
  /* We do NOT want this value to have anything to do with the inferior anymore.
...
value_as_address (struct value *val)
{
  ...
  /* There are several targets (IA-64, PowerPC, and others) which
     do NOT represent pointers to functions as simply the address of
     ...
     On such targets, it is/was difficult for GDB to convert an arbitrary
  ...
  /* JimB writes:
     ...
     and needs to be justified carefully.  In other words, GDB does NOT
     really have the freedom to do these conversions in clever and
     useful ways.

     AndrewC pointed out that users are NOT complaining about how GDB
     casts integers to pointers; they are complaining that they cannot
     ...
     anyone else.  The standard does NOT specify how integers get
     converted to pointers; usually, the ABI does NOT either, but
...
/* Unpack raw data (copied from debugee, target byte order) at VALADDR
   ...
   and can convert between fixed and floating point.  We do NOT assume
  ...
  if (code == TYPE_CODE_FLT)
    {
	 ...
	 would NOT help since the VAX floating-point code is also badly
...
/* Unpack raw data (copied from debugee, target byte order) at VALADDR
   ...
   We do NOT assume any alignment for the raw data.  Return value is in
   host byte order.
...
/* Get the value of the FIELDN'th field (which must be static) of
   TYPE.  Return NULL if the field does NOT exist or has been
   optimized out. */
...
value_change_enclosing_type (struct value *val, struct type *new_encl_type)
{
  if (TYPE_LENGTH (new_encl_type) <= TYPE_LENGTH (value_enclosing_type (val)))
  ...
  else
    {
	 ...
	 chain as the original copy, so its clean-up behavior is the same.
...
#if defined(VALUE_OFFSET) && 0
         VALUE_OFFSET(*arg1p) += offset;
#endif /* VALUE_OFFSET && 0 */
    }
