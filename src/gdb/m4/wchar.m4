...
wchar_t w;]],
      ...
      [gl_cv_header_wchar_h_standalone=no])])dnl
  ## the default(?):
  ...
  ## make sure var is set based on that:
  if test "x${ac_cv_header_wchar_h}" = "xyes"; then
    HAVE_WCHAR_H=1
  else
    HAVE_WCHAR_H=0
  fi
  AC_SUBST([HAVE_WCHAR_H])dnl
  gl_CHECK_NEXT_HEADERS([wchar.h])dnl
])dnl
