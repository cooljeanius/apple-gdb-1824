...
dnl#  including without limitation the rights to use, copy, modify,
dnl#  merge, publish, distribute, sublicense, and/or sell copies of the
dnl#  Software, and to permit persons to whom the Software is furnished to
dnl#  do so, subject to the following conditions:
dnl#
...
dnl# version is at least the given one (assumes that we do use this
dnl# compiler)
dnl#
...
[
    AC_LANG_PUSH([$2])
    AC_CACHE_CHECK([whether we are using $1 $2 compiler v$5 or later],
...
[
    ...
    dnl# different architectures (x86, amd64 and Itanium) so it is easier
    dnl# to just always test for it
    AC_BAKEFILE_PROG_INTEL$1

    dnl# If we use an Intel compiler, we also need to know its version
    if test "$INTEL$1" = "yes"; then
        AC_BAKEFILE_PROG_INTEL$1_8
        AC_BAKEFILE_PROG_INTEL$1_10
    fi

    dnl# if we are using gcc, we cannot be using any of the incompatible
    dnl# compilers
    if test "x$G$1" != "xyes"; then
        ...
        case `uname -s` in
            ...
            Linux*)
                dnl# Sun CC is now available under Linux too, test for
                dnl# it unless we already found that we were using a
                dnl# different compiler
                if test "$INTEL$1" != "yes"; then
