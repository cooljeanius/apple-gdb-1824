...
dnl# various options that select ANSI C on some system or another.
dnl# It considers the compiler to be in ANSI C mode if it handles function
...
dnl# @code{am_cv_prog_cc_stdc} is set to @samp{no}.  If you wrote your
dnl# source code in ANSI C, you can make an un-ANSIfied copy of it by using
dnl# the program @code{ansi2knr}, which comes with Ghostscript.
dnl# @end defmac
...
dnl# Force this before AC_PROG_CPP. Some CPPs, e.g. on HPUX, require
