/* m2-exp.y: YACC grammar for Modula-2 expressions, for GDB.
 * Copyright 1986, 1989-1996, 1999-2000
...
#if 0  /* FIXME! */
int
overflow(long a, long b)
{
   return ((MAX_OF_TYPE(builtin_type_m2_int) - b) < a);
}

int
uoverflow(unsigned long a, unsigned long b)
{
  ...
  while (len-- > 0)
      ...
      else
	{
	 ...
	 unsigned_p = 1;		/* Try something unsigned */
      /* Do NOT do the range check if n==i and i==0, since that special
...
static char *
make_qualname(char *mod, char *ident)
{
   char *newstr = (char *)malloc(strlen(mod) + strlen(ident) + 2UL);
