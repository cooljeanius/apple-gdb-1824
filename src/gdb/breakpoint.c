/* breakpoint.c: Everything about breakpoints, for GDB.
...
static void map_breakpoint_numbers(const char *, void (*)(struct breakpoint *));
...
/* APPLE LOCAL radar 6366048 search both minsyms & syms for bps.  */
static int break_command_1(const char *, int, int, struct breakpoint *, int);
...
/* APPLE LOCAL radar 6366048 search both minsyms & syms for bps.  */
static int break_command_2(const char *, int, int, struct breakpoint *,
...
static void condition_command_1(struct breakpoint *b, const char *condition,
				int from_tty);
...
static void watch_command_1(const char *, int, int, int);
...
static void solib_load_unload_1(const char *hookname, int tempflag,
...
#if !defined(__cplusplus) && defined(ALLOW_DUPLICATE_DECLS)
static int exception_catchpoint_catch_enabled;
static int exception_catchpoint_throw_enabled;
#endif /* !__cplusplus && ALLOW_DUPLICATE_DECLS */

/* APPLE LOCAL - for future-break & -break-insert -f.  */
static void restore_saved_pending_break_support(void *val);

/* APPLE LOCAL: Cannot let breakpoint_sals_to_pc error when resolving
...
#endif /* !SOLIB_LOADED_LIBRARY_PATHNAME */
...
#endif /* !SOLIB_UNLOADED_LIBRARY_PATHNAME */
...
#endif /* !SOLIB_CREATE_CATCH_LOAD_HOOK */
...
#endif /* !SOLIB_CREATE_CATCH_UNLOAD_HOOK */
...
get_number_trailer(const char **pp, int trailer)
{
  ...
  if (p == NULL)
  ...
  else if (*p == '$')
    {
      ...
      while (isalnum(*p) || *p == '_')
	p++;
      varname = (char *)alloca((p - start) + 1UL);
...
/* APPLE LOCAL begin refactor condition_command */
static void
condition_command_1(struct breakpoint *b, const char *condition, int from_tty)
{
  if (b->cond)
    {
      xfree(b->cond);
  ...
  if (condition == 0 || *condition == 0)
      ...
      if (!b->pending)
	{
	  parse_successful = gdb_parse_exp_1(&condition,
					     ...
					     0, &(b->cond));
	  if (!parse_successful)
		error(_("Error parsing breakpoint condition.  "
...
/* As mentioned above, this is a helper for MI: */
void
...
/* As mentioned above, this is another helper for MI: */
void
			     ...
			     unsigned int len)
{
  ...
  ALL_BP_LOCATIONS(b)
  {
      ...
      if (bp_addr > memaddr)
	{
	  ...
	  status =
            deprecated_read_memory_nobpt(memaddr, myaddr,
      ...
      if ((bp_addr + bp_size) < (memaddr + len))
	{
	  ...
	  status =
            deprecated_read_memory_nobpt((bp_addr + bp_size),
                                         ((myaddr + bp_addr + bp_size)
                                          - memaddr),
                                         (unsigned int)((memaddr + len)
                                                        - (bp_addr
                                                           + bp_size)));
	  if (status != 0)
	    return status;
      ...
      if (val)
	{
	  ...
	  if (
...
#endif /* DISABLE_UNSETTABLE_BREAK */
	      )
	    {
	      ...
	      if (!*disabled_breaks)
		{
	      ...
	      *disabled_breaks = 1;
	      fprintf_unfiltered(tmp_error_stream,
				 /* APPLE LOCAL breakpoint notices on 1 line */
				 " %d", bpt->owner->number);
	    }
	  else
	    {
...
insert_breakpoints(void)
{
  ...
  ALL_BP_LOCATIONS_SAFE (b, temp)
	...
	continue;
      ...
      /* APPLE LOCAL: The watchpoint code will set this to 0 if there is/was an
       * error: */
      retval = 1;
	   ...
	   || b->owner->type == bp_watchpoint) && !b->owner->val)
	{
	  ...
	  valptr = evaluate_expression(b->owner->exp);
	  release_value(valptr);
	  /* APPLE LOCAL: Don't throw an error here if we cannot read
	     memory at the breakpoint site.  If we do this when starting
	     up the target, we just stall.  */
	  if (value_lazy(valptr))
	    {
	      retval = gdb_value_fetch_lazy(valptr);
	      if (retval == 0)
      ...
      if (retval)
	{
	  i_val = insert_bp_location(b, tmp_error_stream,
...
breakpoint_here_p(CORE_ADDR pc)
{
  ...
  return ((enum breakpoint_here)
          (any_breakpoint_here ? ordinary_breakpoint_here : 0));
}
  ...
  for (; bs; bs = bs->next)
    {
	  ...
	  || val == PRINT_NOTHING)
	return (enum print_stop_action)val;
    }
...
bpstat_stop_status(CORE_ADDR bp_addr, ptid_t ptid, int stopped_by_watchpoint)
{
  ...
  ALL_BREAKPOINTS_SAFE(b, temp)
  {
	...
	switch (e)
	  ...
	  default:
	    /* Cannot happen.  */
	    warning(_("Unhandled case %d; cannot happen."), e);
	    /* FALLTHROUGH */
	  case 0:
	...
	/* APPLE LOCAL: We allow a breakpoint condition to be set if it failed
	 * to be parsed at the time of the condition command, since there are
	 * legit reasons why that might happen.  So now we should work from
	 * cond_string.  Note, also it is important to catch errors here, since
	 * if we error out we will miss inserting the real instruction in place
	 * of the breakpoint trap: */
	if (b->cond_string)
	  {
...
/* "Cannot happen."  Might want to print an error message.
...
print_one_breakpoint(struct breakpoint *b, CORE_ADDR *last_addr)
{
  ...
  /* 2 */
  annotate_field(1);
  if (((size_t)b->type > (sizeof(bptypes) / sizeof(bptypes[0])))
      || (b->type != bptypes[(int)b->type].type))
    internal_error(__FILE__, __LINE__,
...
do_captured_breakpoint_query(struct ui_out *uiout, void *data)
{
  ...
  struct breakpoint *b;
  CORE_ADDR dummy_addr = 0UL;

  args = (struct captured_breakpoint_query_args *)data;
...
gdb_breakpoint_query(struct ui_out *uiout, int bnum, char **error_message)
{
  ...
  /* For the moment we don't trust print_one_breakpoint() to not throw
     an error. */
  return ((enum gdb_rc)
          catch_exceptions_with_msg(uiout, do_captured_breakpoint_query,
                                    &args, error_message,
                                    RETURN_MASK_ALL));
}
...
{
  struct lang_and_radix *p = (struct lang_and_radix *)old;
  set_language(p->lang);
  input_radix = p->radix;
}
...
#if defined(__GNUC__) && defined(__APPLE__) && !defined(__STRICT_ANSI__)
  __asm__("");
#endif /* __GNUC__ && __APPLE__ && !__STRICT_ANSI__ */
		    ...
		    char *cond_string, enum bptype bp_kind)
{
  ...
  if (sals.nelts == 0)
    {
      warning(_("Unable to set a breakpoint on dynamic linker callback.\n"
		"Suggest linking with /opt/langtools/lib/end.o (if present).\n"
  ...
  if (dll_pathname == NULL)
    ...
    {
      b->dll_pathname = (char *)xmalloc(strlen(dll_pathname) + 1UL);
...
/* */
static void
create_fork_vfork_event_catchpoint(int tempflag, const char *cond_string,
				   enum bptype bp_kind)
{
  struct symtab_and_line sal;
  struct breakpoint *b;
...
/* */
void
create_fork_event_catchpoint(int tempflag, const char *cond_string)
{
...
/* */
void
create_vfork_event_catchpoint(int tempflag, const char *cond_string)
{
...
/* */
void
create_exec_event_catchpoint(int tempflag, const char *cond_string)
{
  struct symtab_and_line sal;
  struct breakpoint *b;
  int thread = -1;		/* All threads. */
...
/* APPLE LOCAL: Creates the internal breakpoint for the objc
   failure mode.  */
struct breakpoint *
create_objc_hook_breakpoint(const char *hookname)
{
		   ...
		   struct breakpoint_list **new_breakpoints)
{
  ...
  /* Now set all the breakpoints: */
  {
	...
	if (pending_bp)
	  {
	    ...
	    if (pending_bp->cond_string)
	      {
		/* APPLE LOCAL: I am turning the errors from parsing the
		 * expression, and the "junk at end of expression" case into
		 * warnings.  That will mean that we will allow the breakpoint
		 * and just ignore the condition.  If we error when resolving a
		 * pending breakpoint we are NOT in a good state to figure out
		 * what happened above us, and for instance sometimes we think
		 * the breakpoint resolution failed, and shlib_disable the
		 * breakpoint, which is undesirable.  */
		int parse_succeeded;
		arg = pending_bp->cond_string;
		b->cond_string = savestring(arg, strlen(arg));
		parse_succeeded =
		  gdb_parse_exp_1(&arg, block_for_pc(b->loc->address), 0,
				  &b->cond);
	...
	/* APPLE LOCAL: Tell listeners that a pending breakpoint has been
	 * resolved: */
	if (pending_bp != NULL)
    ...
    if (type != bp_inlined_breakpoint)
      {
        ...
        new_sals =
	  check_for_additional_inlined_breakpoint_locations(sals, addr_string,
							    cond, cond_string,
							    &new_addr_string,
							    &new_cond,
							    &new_cond_string);
      ...
      if (requested_shlib != NULL)
	{
	  ...
	  if (restrict_cleanup == (void *) -1)
		...
		throw_error(GENERIC_ERROR,
                    _("Failed to locate shared library \"%s\" for breakpoint."),
			    requested_shlib);
	    }
...
#endif /* DEPRECATED_PC_REQUIRES_RUN_BEFORE_USE */
    }
}

/* APPLE LOCAL: introducing a safe_breakpoint_sals_to_pc: first a struct: */
...
/* */
static int
safe_breakpoint_sals_to_pc(struct symtabs_and_lines *sals, const char *address)
{
  ...
  if (!catch_errors((catch_errors_ftype *)wrap_breakpoint_sals_to_pc,
...
do_captured_parse_breakpoint(struct ui_out *ui, void *data)
{
  ...
  args = (struct captured_parse_breakpoint_args *)data;
...
/* APPLE LOCAL end radar 6366048 search both minsyms & syms for bps.  */
{
  ...
  /* APPLE LOCAL: Kind of hacky, but look for requested_shlib and strip it off
   * if found: */
  if (!arg)
...
/* APPLE LOCAL begin radar 6366048 search both minsyms & syms for bps.  */
static int
break_command_2(const char *arg, int flag, int from_tty,
                struct breakpoint *pending_bp,
...
/* APPLE LOCAL end radar 6366048 search both minsyms & syms for bps.  */
{
  ...
  cond = (struct expression **)xcalloc(sals.nelts,
                                       sizeof(struct expression *));
  make_cleanup(xfree, cond);

  /* Allocate space for all the cond strings: */
  cond_string = (char **)xcalloc(sals.nelts, sizeof(char **));
...
do_captured_breakpoint(struct ui_out *uiout, void *data)
{
  ...
  args = (struct captured_breakpoint_args *)data;
                   ...
                   char **error_message)
{
  ...
  return ((enum gdb_rc)
          catch_exceptions_with_msg(uiout, do_captured_breakpoint, &args,
...
void
/* APPLE LOCAL radar 6366048 search both minsyms & syms for bps.  */
rbr_break_command(const char *arg, int from_tty, int use_msymbol_p)
{
...
stopat_command(const char *arg, int from_tty)
{
  ...
  if (arg == (const char *)NULL || *arg == '*')	/* no line number */
    badInput = 1;
  else
    {
      const char *argptr = arg;
...
/* APPLE LOCAL: BY_LOCATION - if set, just watch the memory location
   ...
   up to it.  Also, do NOT tie the watch to the current scope.  */
static void
watch_command_1(const char *arg, int accessflag, int by_location, int from_tty)
{
  struct breakpoint *b;
  struct symtab_and_line sal;
  ...
  if (!target_has_execution)
    {
      warning(_("cannot do that without a running program; try \"break main\"), \"run\" first"));
      return;
    }
#endif /* HPUXHPPA */
...
#endif /* !TARGET_REGION_OK_FOR_HW_WATCHPOINT */
		      ...
		      && TYPE_CODE(vtype) != TYPE_CODE_ARRAY))
		{
		  ...
		  if (!TARGET_REGION_OK_FOR_HW_WATCHPOINT(vaddr, len))
		    ...
		    found_memory_cnt++;
                  ...
                  /* for when TARGET_REGION_OK_FOR_HW_WATCHPOINT() skips
                   * on using vaddr: */
                  if (vaddr == INVALID_ADDRESS) {
                    warning(_("Invalid value address; maybe it was skipped?"));
                  }
		}
	    }
	}
...
/* */
void
watch_command_wrapper(const char *arg, int by_location, int from_tty)
{
...
/* */
int
detect_location_arg(const char **arg)
{
  int by_location = 0;
...
/* */
void
rwatch_command_wrapper(const char *arg, int by_location, int from_tty)
{
...
/* */
void
awatch_command_wrapper(const char *arg, int by_location, int from_tty)
{
...
{
  /* FIXME: 1024 might be too small; use a relevant macro instead: */
...
static void
catch_fork_command_1(catch_fork_kind fork_kind, const char *arg, int tempflag,
		     int from_tty)
{
...
/* */
static void
catch_exec_command_1(const char *arg, int tempflag, int from_tty)
{
  const char *cond_string = NULL;
  ...
  if (cond_string == NULL)
    {
      ...
      if (dll_pathname == NULL) {
        warning(_("Null pathname to dll."));
      }
      ep_skip_leading_whitespace(&arg);
      cond_string = ep_parse_optional_if_clause(&arg);
...
static enum print_stop_action print_exception_catchpoint(struct breakpoint *);
static void print_one_exception_catchpoint(struct breakpoint *, CORE_ADDR *);
static void print_mention_exception_catchpoint(struct breakpoint *);
...
print_exception_catchpoint(struct breakpoint *b)
{
  switch (b->type)
    {
    case bp_gnu_v3_catch_catch:
      printf_filtered("\nCatchpoint %d (exception caught).", b->number);
      ATTRIBUTE_FALLTHROUGH; /* XXX really fallthrough? */
    case bp_catch_catch:
    ...
    case bp_gnu_v3_catch_throw:
      printf_filtered("\nCatchpoint %d (exception thrown).", b->number);
      ATTRIBUTE_FALLTHROUGH; /* XXX really fallthrough? */
    case bp_catch_throw:
      if (current_exception_event &&
          (CURRENT_EXCEPTION_KIND == EX_EVENT_THROW))
        {
          ...
          /* Do NOT bother to print location frame info: */
...
/* APPLE LOCAL: The gnu_v3_exception handling code in gdb could NOT
			     ...
			     int delete_it, struct objfile *objfile)
{
  ...
  sals = target_find_exception_catchpoints(ex_event, objfile);
  if (sals && (sals != (struct symtabs_and_lines *)-1))
      ...
      if (delete_it)
	{
	  ...
	  for (i = 0; i < sals->nelts; i++)
	    if (sals->sals[i].pc != (CORE_ADDR)-1L)
...
current_exception_should_stop(void)
{
  ...
  /* If we cannot figure out the object type, then always stop: */
  if (!obj_type) {
    return 1;
  }
  ...
  if (exception_type_regexp)
    {
      ...
      /* Xcode sets the regexp to just "$" when it means match anything.
         That does NOT work with the way re_exec works, since that demands
         that the match have content.  So just shortcut the two cases of
         empty string and just "$"... */

      if ((*exception_type_regexp == '\0')
          || (strcmp(exception_type_regexp, "$") == 0)) {
        return 1;
      }

      old_syntax = re_set_syntax(REG_EXTENDED);
...
catch_command_1(const char *arg, int tempflag, int from_tty)
{
  ...
  const char *arg1_end;
  ptrdiff_t arg1_length;

  if (arg1_start == NULL)
    {
      /* Old behavior was to use pre-v-4.16 syntax: */
...
delete_breakpoint (struct breakpoint *bpt)
{
  ...
  if (bpt->loc->inserted
	    ...
	    && (b->enable_state != bp_call_disabled))
	{
	  ...
	  /* If there was an error in the insert, print a message, then stop
	   * execution: */
	  if (val != 0)
	    {
	      struct ui_file *tmp_error_stream = mem_fileopen();
	      ...
	      if (b->type == bp_hardware_breakpoint)
		{
		...
		else
		  {
		    ...
		    fprintf_filtered(tmp_error_stream,
				     "Error accessing memory address ");
		    deprecated_print_address_numeric(b->loc->address, 1,
						     tmp_error_stream);
		    fprintf_filtered(tmp_error_stream, ": %s.\n",
				     safe_strerror(val));
		  }
  ...
  if (arg == 0)
    {
      ...
      ALL_BREAKPOINTS(b)
      {
	if ((b->type != bp_call_dummy) && (b->type != bp_shlib_event)
	    && (b->type != bp_thread_event) && (b->type != bp_overlay_event)
	    && (b->number >= 0))
	  breaks_to_delete = 1;
      }
...
breakpoint_re_set_one(void *bint)
{
  ...
  switch (b->type)
    ...
    case bp_catch_unload:
	 ...
	 to reset it.  I need this because I am reusing the bp_shilb_event
	 for other purposes since it is such a PITA to add new breakpoint
	 types...  */
    case bp_shlib_event:
      if (b->addr_string == NULL)
	break;
      ...
      if (b->requested_shlib != NULL)
	{
	  ...
	  if ((b->type != bp_gnu_v3_catch_throw)
	      && (b->type != bp_gnu_v3_catch_catch))
	    {
	      if (objfile_name_set_load_state(b->requested_shlib, OBJF_SYM_ALL,
					      0) == -1)
		{
		  return 0;
		}
      ...
      else if (b->bp_objfile_name != NULL)
	{
	  ...
	  if ((b->type != bp_gnu_v3_catch_throw)
	      && (b->type != bp_gnu_v3_catch_catch))
	    {
	      if (objfile_name_set_load_state(b->bp_objfile_name, OBJF_SYM_ALL,
					      0) == -1)
		{
		  return 0;
		}
...
/* APPLE LOCAL: Call breakpoint_update to sync breakpoints with the new
 * symbol environment: */
void
breakpoint_update(void)
{
...
static void
map_breakpoint_numbers(const char *args, void (*function)(struct breakpoint *))
{
  const char *p = args;
  const char *p1;
  int num;
...
/* */
static void
set_breakpoint_cmd(const char *args ATTRIBUTE_UNUSED,
...
/* */
static void
show_breakpoint_cmd(const char *args ATTRIBUTE_UNUSED,
		    int from_tty ATTRIBUTE_UNUSED)
{
  return;
}
...
/* Generate a break, watch, or catch command defined by B to the STREAM.
   ...
   function's caller.  */
static void
write_one_breakpoint(struct breakpoint *b, struct ui_file *stream,
		     struct ui_out *uiout)
{
  ...
  switch (b->type)
	...
	const char *tempflag = ((b->disposition == disp_del) ? "t" : "");
        ...
        fprintf_unfiltered(stream, "%s%s%sbreak", futureflag, tempflag,
			   hardwareflag);
	...
	if (b->addr_string)
	  ...
	  fprintf_unfiltered(stream, " %s",
			     hex_string_custom((unsigned long)b->loc->address,
					       8));
      }
      break;
...
/* */
static void
save_breakpoints_command(const char *arg, int from_tty)
{
  struct cleanup *cleanups;
          ...
          if (time(&t) != -1)
            {
              ...
              if (l)
                {
                  char *orig_locale;
                  orig_locale = strcpy((char *)xmalloc(strlen(l) + 1UL), l);
                  setlocale(LC_ALL, "");
                  if (strftime(buf, sizeof(buf), "%a %b %e %H:%M:%S %Z %Y",
			       localtime(&t)))
...
/* */
void
future_break_command(const char *arg, int from_tty)
{
 ...
 * We need this so that if you hit the solib event breakpoint and actually
...
/* This one sets the breakpoint as unset, but does NOT mark it as pending.
 * That way breakpoint_re_set_all will re-set it: */
void
 ...
 * as pending again, so that it will get reset when the shared library
 * reappears: */
void
