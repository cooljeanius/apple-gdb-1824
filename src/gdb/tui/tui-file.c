/* tui/tui-file.c: UI_FILE - a generic STDIO like output stream.
...
tui_file_new(void)
{
  struct tui_stream *tui = (struct tui_stream *)xmalloc(sizeof(struct tui_stream));
...
tui_sfileopen(int n)
{
  ...
  if (n > 0)
    {
      tmpstream->ts_strbuf = (char *)xmalloc((n + 1) * sizeof(char));
...
static void
tui_file_put(struct ui_file *file, ui_file_put_method_ftype *write,
	     void *dest)
{
  struct tui_stream *stream = (struct tui_stream *)ui_file_data(file);
  if (stream->ts_magic != &tui_file_magic)
...
tui_file_adjust_strbuf(int n, struct ui_file *file)
{
  ...
  if (stream->ts_strbuf)
      ...
      if (n > (stream->ts_buflen - non_null_chars - 1))
	{
	  ...
	  stream->ts_strbuf = (char *)xrealloc(stream->ts_strbuf,
                                               stream->ts_buflen);
	}
    }
  else
    /* No buffer yet, so allocate one of the desired size: */
    stream->ts_strbuf = (char *)xmalloc((n + 1) * sizeof(char));
}
