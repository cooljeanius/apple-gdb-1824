...
AC_DEFUN([GCC_CHECK_TLS],[
  ...
  AC_CACHE_CHECK([whether the target supports thread-local storage],
	...
	dnl# Also check if the libc supports TLS, i.e. whether a variable
	dnl# with __thread linkage has a different address across threads.
	dnl# 1st, find the thread_CFLAGS necessary to link a program that
	...
	if test "X$thread_CFLAGS" != Xfailed; then
      ...
      [gcc_cv_have_tls=no],
      [dnl# The cross-compiling case. Assume libc supports TLS if the
