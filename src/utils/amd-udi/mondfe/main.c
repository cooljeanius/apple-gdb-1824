...
# include "config.h"
#else
# warning main.c expects "config.h" to be included.
#endif /* HAVE_CONFIG_H */
#include <stdio.h>
#include <signal.h>
#include <unistd.h>
...
#   else
#    warning main.c expects a header that provides malloc() to be included.
...
int main(argc, argv)
   ...
   */
   if (signal (SIGINT, Def_CtrlC_Hdlr) == SIG_ERR) {
     fprintf(stderr, "Could NOT install default Ctrl-C handler.\n"); 
     if (io_config.echo_mode == (INT32) TRUE)
        fprintf(io_config.echo_file, "Could NOT install default Ctrl-C handler.\n"); 
   }
