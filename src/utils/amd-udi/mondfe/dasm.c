...
# include "config.h"
#else
# warning dasm.c expects "config.h" to be included.
#endif /* HAVE_CONFIG_H */
#include <stdio.h>
#include <string.h>
#ifdef MSDOS
...
#   else
#    warning dasm.c expects a header that provides malloc() to be included.
   ...
   if ((retval = Mini_read_req(memory_space,
	...
	return(FAILURE);
	  ...
	  if (host_config.target_endian == LITTLE) {
		   ...
		   } /* where to place this closing brace was ambiguous */
		...
		if (io_config.echo_mode == (INT32) TRUE) {
          ...
          fprintf(stderr, "\n");
		}  /* where to place this closing brace for the above "if" was ambiguous */
       }  /* end for loop */
   };
