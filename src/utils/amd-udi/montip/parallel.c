...
# include "config.h"
#else
# warning parallel.c expects "config.h" to be included.
#endif /* HAVE_CONFIG_H */
#ifdef MSDOS
# include <bios.h> /* for _PRINTER_INIT and _PRINTER_INIT */
# include <conio.h>
#else
# include <unistd.h>
#endif /* MSDOS */
...
{
  unsigned status;
#if defined(_PRINTER_INIT) || defined(MSDOS)
...
INT32 		length;
{
 ...
 {
#if defined(_PRINTER_WRITE) || defined(MSDOS)
   status = _bios_printer(_PRINTER_WRITE, portID, (unsigned) *buffer);
#endif /* _PRINTER_WRITE */
#ifdef DEBUG
   printf("status 0x%.4x \n", status);
#endif /* DEBUG */
   buffer++;
