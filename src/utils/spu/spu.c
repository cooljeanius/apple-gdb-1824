...
#  warning spu.c expects "config.h" to be included.
...
#include <ctype.h>
#include <time.h>
#include <math.h> /* for abs(), and maybe signbit() later... */
...
const char *typenames[] = { "?", "void", "int", "short", "char" };
...
struct decl_table {
  int size; /* might make more sense as 'size_t'... */
  int nextentry;
...
const char *random_computer_word(void);

void display_usage(void);

int random_type(int libonly);

const char *name_from_type(int n);

void add_decl_to_table(enum decl_types type, void *desc,
                       struct decl_table *table);

int hash_string(const char *str);

struct hash_entry *get_hash_entry(void);

const char *add_to_hash_table(const char *buf, struct hash_table *table);

const char *get_from_hash_table(const char *buf, struct hash_table *table);
...
void gen_random_global_name(const char *root, char *namebuf);
...
const char *gen_new_macro_name(void);

void create_enums(void);

void create_enum(struct enum_desc *enumdesc);

const char *gen_random_enumerator_name(void);
...
int compare_entries(const void *x1, const void *x2);
...
const char *extensions[] = { "c", "c", "cc", "m" };

/* Names for each language: */
const char *lang_names[] = {
  "K&R C", "standard C", "standard C++", "Objective-C"
};
...
const char *file_base_name = "file";
...
main(int argc, char **argv)
{
  ...
  order.nextentry = 0;
  order.entries =
    (struct decl_entry *)xmalloc((size_t)order.size * sizeof(struct decl_entry));
  lib_order.size =
    (num_lib_macros + num_lib_enums + num_lib_structs + num_lib_classes
     + num_lib_functions);
  lib_order.nextentry = 0;
  lib_order.entries =
    (struct decl_entry *)xmalloc((size_t)lib_order.size * sizeof(struct decl_entry));
  create_macros();
  create_enums();
  create_structs();
  create_functions();
...
create_macros(void)
{
  ...
  macros =
    (struct macro_desc *)xmalloc((size_t)num_macros * sizeof(struct macro_desc));
  ...
  /* It is important to create library macros second, so that their ids
  ...
  lib_macros =
    (struct macro_desc *)xmalloc((size_t)num_lib_macros * sizeof(struct macro_desc));
...
create_macro(struct macro_desc *macrodesc)
{
  ...
  if (numargs > 0)
    {
      macrodesc->args = (char **)xmalloc((size_t)numargs * sizeof(char *));
      for (j = 0; j < numargs; ++j)
	{
...
create_enums(void)
{
  ...
  enums =
    (struct enum_desc *)xmalloc((size_t)num_enums * sizeof(struct enum_desc));
  ...
  lib_enums =
    (struct enum_desc *)xmalloc((size_t)num_lib_enums * sizeof(struct enum_desc));
...
create_enum(struct enum_desc *enumdesc)
{
  ...
  enumdesc->enumerators =
    (struct enumerator_desc *)xmalloc((size_t)num * sizeof(struct enumerator_desc));
  for (j = 0; j < num; ++j)
    {
...
create_structs(void)
{
  ...
  lib_structs =
    (struct struct_desc *)xmalloc((size_t)num_lib_structs * sizeof(struct struct_desc));
  for (i = 0; i < num_lib_structs; ++i)
    {
  ...
  structs =
    (struct struct_desc *)xmalloc((size_t)num_structs * sizeof(struct struct_desc));
  for (i = 0; i < num_structs; ++i)
    {
...
create_struct(struct struct_desc *structdesc, int lib)
{
  ...
  structdesc->fields =
    (struct field_desc *)xmalloc((size_t)numf * sizeof(struct field_desc));
  for (j = 0; j < numf; ++j)
    {
...
create_classes(void)
{
  ...
  lib_classes =
    (struct class_desc *)xmalloc((size_t)num_lib_classes * sizeof(struct class_desc));
  ...
  classes =
    (struct class_desc *)xmalloc((size_t)num_classes * sizeof(struct class_desc));
...
create_class(struct class_desc *classdesc, int lib)
{
  ...
  classdesc->methods =
    (struct function_desc *)xmalloc((size_t)numm * sizeof(struct function_desc));
  for (j = 0; j < numm; ++j)
    {
      create_function(&(classdesc->methods[j]), lib);
      classdesc->methods[j].classdesc = classdesc;
    }
...
create_functions(void)
{
  ...
  functions =
    (struct function_desc *)xmalloc((size_t)num_functions * sizeof(struct function_desc));
  ...
  lib_functions =
    ((struct function_desc *)
     xmalloc((size_t)num_lib_functions * sizeof(struct function_desc)));
...
create_function(struct function_desc *fndesc, int lib)
{
  ...
  if (numargs > 0)
    {
      fndesc->args =
	(struct arg_desc *)xmalloc((size_t)numargs * sizeof(struct arg_desc));
      for (j = 0; j < numargs; ++j)
	{
...
write_header_file(int n)
{
  ...
  qsort(order.entries, (size_t)order.size, sizeof(struct decl_entry),
	compare_entries);
...
write_lib_header_file(void)
{
  ...
  qsort(lib_order.entries, (size_t)lib_order.size,
        sizeof(struct decl_entry), compare_entries);
  ...
  if (fp)
    {
      ...
      const char *compiler_string = "gcc";
#else
      const char *compiler_string = "cc";
#endif /* __GNUC__ && __GNUC_MINOR__ && !__clang__ */
#ifdef ALLOW_UNUSED_VARIABLES
      /* FIXME: might want to use this to silence compiler warnings in the
       * generated Makefile (such as '-Wint-to-pointer-cast'), but it would
       * be better to just have the generated program avoid the warnings in
       * the first place... */
      const char *compiler_flags_string = "";
	...
	fprintf(fp, " %s%d.o", file_base_name, i);
      ...
      fprintf(fp, "\t$(CC) -g -o %s.out", file_base_name);
      for (i = 0; i < num_files; ++i)
	fprintf(fp, " %s%d.o", file_base_name, i);
      fprintf(fp, " %slib.o", file_base_name);
      fprintf(fp, "\n\n");
...
const char *
gen_unique_global_name(const char *root, int upcase)
{
  int i, j;
  const char *str;
  char namebuf[100];
  ...
  for (i = 0; i < 10000; ++i)
      ...
      if (upcase)
	{
	  for (j = 0; namebuf[j] != '\0'; ++j)
	    namebuf[j] = (char)toupper(namebuf[j]);
	}
  ...
  if (str == NULL)
    {
      fprintf(stderr, "Cannot get a unique name!\n");
      exit(1);
    }
  return str;
}
...
void
gen_random_global_name(const char *root, char *namebuf)
{
  char smallbuf[100];
  int n, i;
  size_t len;
...
#ifdef STUDY_RANDOM_NAME_DISTRIBUTION
  printf("Try %s\n", namebuf);
#endif /* STUDY_RANDOM_NAME_DISTRIBUTION */
}
...
{
  int hash;
  struct hash_entry *ent, *lastent = (struct hash_entry *)NULL;
...
const char *
get_from_hash_table(const char *buf, struct hash_table *table)
{
  int hash;
  struct hash_entry *ent, *lastent = (struct hash_entry *)NULL;
...
unsigned long initial_randstate = 0UL;

unsigned long randstate = 0UL;
...
init_xrandom(int in_seed)
{
  ...
  if (in_seed > 0)
      ...
      if (randstate > 0UL)
	{
      ...
      randstate = (unsigned long)in_seed;
    }
  else
    {
      time(&tm);
      randstate = (unsigned long)tm;
    }
  /* Whatever its source, put the randstate into known range (0 - 199999): */
  randstate = (unsigned long)abs((int)randstate);
  randstate %= 200000UL;
...
xrandom(int m)
{
  ...
  randstate = ((9301UL * randstate + 49297UL) % 233280UL);
  rslt = abs((int)((unsigned long)(m * (int)randstate) / 233280UL));
  /* enable to study random number distribution: */
#ifdef STUDY_RANDOM_NUMBER_DISTRIBUTION
  printf("# %lu -> %d\n", randstate, rslt);
  /* (warning: I hope you have a large scrollback buffer if you enable
   *  this part!) */
#endif /* STUDY_RANDOM_NUMBER_DISTRIBUTION */
  return rslt;
}
