...
struct AFPAlternateAddress {
	UInt8 fAddressCount;
	UInt8 fAddressList[1]; /* actually var length packed set of AFPTagData */
};
typedef struct AFPAlternateAddress		AFPAlternateAddress;

#endif /* UNIVERSAL_INTERFACES_VERSION < 0x0301 */
...
**	since some file systems do NOT bother to set this field).
**
...
/*
** For those times where you need to use more than 1 kind of File Manager parameter
** block but do NOT feel like wasting stack space, here is a parameter block you can reuse.
*/
...
/*	¦ Allocate a temporary copy or search buffer.
	...
	buffReqSize		input:	Size you would like the buffer to be.
...
/*	¦ Get a volume's file system ID.
	...
	that handles requests to a particular volume. Here is a partial list
	of file system ID numbers (only Apple-specific file systems are listed):
		FSID	File System
		-----   -----------------------------------------------------
...
/*	¦ Set the default volume before making Standard I/O requests.
	...
	By using these functions as a wrapper, you will NOT need to open a working
	directory (because SetDefault and RestoreDefault use HSetVol) and you
	will NOT have to worry about the effects of using HSetVol (documented in
...
/*	¦ Get a file system object's location.
	...
	and name. While we are at it, determine if the object is a file or directory.
	If GetObjectLocation returns fnfErr, then the location information
	returned is valid, but it describes an object that does NOT exist.
	...
	realName	output:	The name of the specified object (the case of the
						...
						system is not case sensitive, it should NOT matter).
...
/*	¦ Delete the contents of a directory.
	...
	Result Codes
		...
		fBsyErr				-47		File busy, directory not empty, or working dir control block open
		paramErr			-50		No default volume
		dirNFErr			-120	Directory not found or incomplete pathname
		afpAccessDenied		-5000	User does not have the correct access
		afpObjectTypeErr	-5025	Directory not found or incomplete pathname
...
pascal	OSErr	FSpChangeCreatorType(const FSSpec *spec,
									 ...
									 OSType fileType);
/*	¦ Change the creator or file type of a file.
	The FSpChangeCreatorType func changes the creator or file type of a file.
...
/*	¦ Copy all File Manager attributes from the source to the destination.
	...
	srcName		input:	Pointer to source object name, or nil when
						srcDirID specifies a directory that is the object.
	dstVRefNum	input:	Destination volume specification.
	dstDirID	input:	Destination directory ID.
	dstName		input:	Pointer to destination object name, or nil when
						dstDirID specifies a directory that is the object.
...
/*	¦ Open the data fork of a file using deny mode permissions.
	...
	OpenDF first, and then Open if OpenDF is NOT available. By using
...
pascal	OSErr	CopyFork(short srcRefNum,
						 ...
						 void *copyBufferPtr,
						 long copyBufferSize);
/*	¦ Copy all data from the src fork to the dest fork of open file forks.
...
/*	¦ Copy the AFP directory access privileges.
	...
	Result Codes
		...
		fnfErr				-43		Directory not found
		vLckdErr			-46		Volume is locked or read-only
		paramErr			-50		Volume does NOT support this function
		afpAccessDenied		-5000	User does not have the correct access
									to the directory
...
/*	¦ Move a file or directory and optionally rename it.
	...
	volume. This routine works even if the volume does NOT support MoveRename.
	...
	copyName	input:	Points to the new name if the object is to be
						renamed or nil if the object is NOT to be renamed.
	...
	Result Codes
		...
		badMovErr			-122	Attempted to move directory into
									offspring
		wrgVolTypErr		-123	Not an HFS volume (it is/was a MFS volume)
		notAFileErr			-1302	The pathname is nil, the pathname
									is empty, or the pathname cannot refer
									to a filename
