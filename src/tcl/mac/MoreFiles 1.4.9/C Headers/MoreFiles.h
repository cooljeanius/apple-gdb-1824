...
/*	¶ Determine the characteristics of a volume.
	...
	of a volume. A result of paramErr usually just means the volume does NOT
...
/*	¶ Map a user or group ID to a user or group name.
	...
	volName		input:	A pointer to the name of a mounted volume
						or nil.
	vRefNum		input:	Volume specification.
	objType		input:	The mapping function code: 1 if you are mapping a
						user ID to a user name or 2 if you are mapping a
...
/*	¶ Map a user or group name to a user or group ID.
	...
	objType		input:	The mapping function code: 3 if you are mapping a
						user name to a user ID or 4 if you are mapping a
...
/*	¶ Duplicate a file on a file server and optionally to rename it.
	...
	copyName	input:	Points to the new file name if the file is to be
						renamed or nil if the file is NOT to be renamed.
