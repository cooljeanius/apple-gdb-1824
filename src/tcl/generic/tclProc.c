 ...
 *
 * RCS: @(#) tclProc.c,v 1.5 2003/01/21 19:40:06 hunt Exp
 */
    ...
    Proc **procPtrPtr;          /* returns:  pointer to proc data */
{
	...
	if (precompiled) {
		...
		goto procError;
            ...
            localPtr->nameLength = (int)nameLength;
            localPtr->frameIndex = i;
		    ...
		    Tcl_NewStringObj(fieldValues[1], (int)valueLength);
 	...
 	if (result != TCL_OK) {
		...
		numChars = strlen(procName);
 		...
 		sprintf(buf, "\n    (compiling %s \"%.*s%s\", line %d)",
 			description, (int)numChars, procName, ellipsis,
 			interp->errorLine);
