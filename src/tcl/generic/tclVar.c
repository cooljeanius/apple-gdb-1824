    ...
    Tcl_Obj *CONST objv[];	/* Argument objects. */
{
	...
	if (numRequired > listRepPtr->maxElemCount) {
		...
		ckalloc((size_t)newMax * sizeof(Tcl_Obj *));
    ...
    Tcl_Obj *objPtr;
{
    ...
    id = (int)strtoul(string+2, &end, 10);
    if ((end == (string+2)) || (*end != '-')) {
	goto syntax;
    }
    /*
				 ...
				 * name. */
{
    ...
    offset = (size_t)(((char*)handleObj->internalRep.twoPtrValue.ptr2) -
                      ((char*)NULL));
    /*
    ...
    Tcl_Obj *objPtr;
{
    ...
    objPtr->bytes = ckalloc(nameLen + 1U);
    memcpy(objPtr->bytes, localPtr->name, nameLen + 1U);
    objPtr->length = (int)nameLen;
    return;

    emptyName:
    objPtr->bytes = ckalloc(1U);
    *(objPtr->bytes) = '\0';
    objPtr->length = 0;
}
    ...
    Tcl_Obj *objPtr;
{
    ...
    part1 = Tcl_GetStringFromObj(arrayPtr, (int *)&len1);
    len2 = strlen(part2);

    totalLen = (len1 + len2 + 2UL);
    p = ckalloc((unsigned int)totalLen + 1UL);
    objPtr->bytes = p;
    objPtr->length = (int)totalLen;
