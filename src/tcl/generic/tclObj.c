    ...
    register Tcl_Obj *objPtr;	/* Double obj with string rep to update. */
{
    ...
    objPtr->bytes = (char *)ckalloc(len + 1UL);
...
{
#ifdef TCL_WIDE_INT_IS_LONG
    return Tcl_NewLongObj((long)wideValue);
#else
    register Tcl_Obj *objPtr;
...
{
#ifdef TCL_WIDE_INT_IS_LONG
    Tcl_SetLongObj(objPtr, (long)wideValue);
#else
    register Tcl_ObjType *oldTypePtr = objPtr->typePtr;
    ...
    register Tcl_WideInt *wideIntPtr; /* Place to store resulting long. */
{
     ...
     */
    return Tcl_GetLongFromObj(interp, objPtr, (long *)wideIntPtr);
#else
    register int result;
