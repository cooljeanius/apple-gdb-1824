...
AC_DEFUN([SC_PATH_TKCONFIG],[
    ...
    AC_ARG_WITH([tk],[AS_HELP_STRING([--with-tk=DIR],[use Tk 8.4 binaries from DIR])],
...
AC_DEFUN([SC_LOAD_TCLCONFIG],[
    ...
    #
    # If TCL_BIN_DIR is the build directory (not the install directory),
...
AC_DEFUN([SC_ENABLE_THREADS],[
	...
	AC_MSG_RESULT([yes])
	TCL_THREADS=1
	AC_DEFINE([TCL_THREADS],[1],[Define to 1 for threads])
	# USE_THREAD_ALLOC tells us to try the special thread-based
	# allocator that significantly reduces lock contention
	AC_DEFINE([USE_THREAD_ALLOC],[1],[Define to 1 to use a special thread-based allocator])
    else
	TCL_THREADS=0
...
#FIXME: Currently, LDFLAGS_DEFAULT is not used, it should work
# like CFLAGS_DEFAULT.
	...
	AC_DEFINE([TCL_MEM_DEBUG],[1],[Define to 1 to debug memory])
    fi
...
AC_DEFUN([SC_CONFIG_CFLAGS],[
    ...
    # set various compiler flags depending on whether we use gcc or cl
	...
	if test "$ac_cv_cygwin" = "yes"; then
	    ...
	    extra_ldflags="$extra_ldflags -mno-cygwin"
	else
	...
	if test "${SHARED_BUILD}" = "0" ; then
	    ...
	    # included so -mno-cygwin passed the correct libs to the
	    # linker.
