...
AC_DEFUN([SC_PATH_TCLCONFIG],[
	...
	AC_CACHE_VAL([ac_cv_c_tclconfig],[
		...
		for i in \
			...
			AC_MSG_NOTICE([file from which tclConfig.sh is generated was found in ${i}/unix, but not the actual script itself])
			AC_MSG_NOTICE([perhaps you still need to configure and/or build there?])
		    fi
		    if test -f "${i}/win/tclConfig.sh"; then
			ac_cv_c_tclconfig=`(cd ${i}/win; pwd)`
			break
		    elif test -f "${i}/win/tclConfig.sh.in"; then
			AC_MSG_NOTICE([file from which tclConfig.sh is generated was found in ${i}/win, but not the actual script itself])
			AC_MSG_NOTICE([perhaps you still need to configure and/or build there?])
		    fi
		done
	    fi
