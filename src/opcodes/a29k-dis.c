/* a29k-dis.c: Instruction printing code for the AMD 29000
...
extern int print_insn_big_a29k(bfd_vma, struct disassemble_info *);
extern int print_insn_little_a29k(bfd_vma, struct disassemble_info *);
...
{
  return ((opcode == 0xa8) || (opcode == 0xa9) || (opcode == 0xa0)
          || (opcode == 0xa1) || (opcode == 0xa4) || (opcode == 0xa5)
	  || (opcode == 0xb4) || (opcode == 0xb5) || (opcode == 0xc4)
          || (opcode == 0xc0) || (opcode == 0xac) || (opcode == 0xad)
	  || (opcode == 0xcc));
}

/* Now find the four bytes of INSN and put them in *INSN{0,8,16,24}: */
static void
find_bytes_big(char *insn, unsigned char *insn0, unsigned char *insn8,
...
static void
find_bytes_little(char *insn, unsigned char *insn0, unsigned char *insn8,
                  unsigned char *insn16, unsigned char *insn24)
{
  *insn24 = insn[3];
  *insn16 = insn[2];
      ...
      if (((unsigned long)insn24 << 24) == opcode->opcode)
	{
	  ...
	  for (s = opcode->args; *s != '\0'; ++s)
		...
		case 'P':
		   ...
		   * maybe that is OK (it is what the GDB m68k and EBMON
	  ...
	  /* Now we look for a const,consth pair of instructions, in which
           * case we try to print the symbolic address: */
	  if (insn24 == 2)  /* consth */
	    {
	      int errcode;
	      char prev_insn[4];
