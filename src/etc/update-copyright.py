...
class GenericFilter:
    ...
    def skip_file(self, dir, filename):
        ...
        if extension == ".in":
            ...
            if os.path.exists(base + ".def") and os.path.exists(base + ".tpl"):
                return True
...
class Copyright:
    ...
    def is_complete(self, match):
        ...
        return holder and (holder not in self.holder_prefixes or holder in self.holders)
                    ...
                    if not (line_filter and line_filter.match(line)):
                        ...
                        if match:
                            res = self.update_copyright(
                                dir, filename, filter, file, line, match
                            )
                        ...
                        # Check for copyright lines that might have slipped by.
                            ...
                            self.errors.report(
                                pathname, "unrecognised copyright: %s" % line.strip()
                            )
                    lines.append(line)
                    line = next_line
...
class CmdLine:
    def __init__(self, copyright=Copyright):
        ...
        self.add_option(
            "--quilt", '"quilt add" files before changing them', self.o_quilt
        )
        self.add_option(
            "--this-year", "Add the current year to every notice", self.o_this_year
        )
        ...
        if self.errors.ok():
            ...
            if len(self.chosen_dirs) == 0:
                ...
                for chosen_dir in self.chosen_dirs:
                    ...
                    if count == 0:
                        self.errors.report(
                            None, "unrecognised directory: " + chosen_dir
                        )
