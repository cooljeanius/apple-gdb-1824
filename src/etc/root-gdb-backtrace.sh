...
# It is used by TUnixSystem::StackTrace() in ROOT on Linux and MacOS X.
...
if [ "$(uname -s)" = "Darwin" ]; then
   ...
   if test $? -ne 0; then
      echo "${tempname}: cannot create temp file, exiting..." 1>&2
      exit 1
   fi
   ...
   # GDB does NOT allow "thread apply all bt" when the process is NOT
...
#     it is worth dumping the stack frames.
#   * The user does NOT call CINT directly, so whenever we reach
