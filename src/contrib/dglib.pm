...
# Options, if provided, should be a hashref w/0 or more of the following keys:
#	gccdir:
# 		Passing in the full path to the root of the gcc/testsuite directory
#		will help in the parsing, but if it is NOT provided, it will be guessed.
#	diagnostics:
...
#	diagnostics:
#		List of (type, value) for any errors detected.
...
#		        auxiliary data.
#		pseudotest: If 1, this test may not truly exist; see "pseudotest" below.
...
sub parseLogFile($;$) {
	...
	open(LOGFILE, $logfile) or die "Failed to open log file $logfile: $!\n";
	...
	# We do NOT want gccdir matching on an empty string.
	$gccdir ||= "this will never match, or my name is NOT Reginald St. Croix";
	...
	while(<LOGFILE>) {
		...
		if(/^gcc version/) {
			...
			# some tests do NOT bother printing out the full paths of their
			# files, and this gives us the directory information.
			...
			# Okay, if it is "Executing on host", it is a new
			# file.  If it is "Executing on unix", it is probably
			...
			# If we cannot find it, it is NOT really a new testfile,
			# but a derived file.
			...
			if($exectype eq "host" or !$currgroup) {
				...
				my $nogroup = 0;
				if($execwhat =~ / /) {
					# We probably have NOT parsed the file correctly.
					...
					# as a test.  We do NOT really want to treat this as a test.
					if($execwhat =~ m!/(gcc|c\+\+)$!) {
						undef $currtest;
						undef $currgroup;
						$nogroup = 1;
			...
			if($ret{incomplete} == 2) {
				# Ah, but we previously saw indication that we were killed via
				# a signal.
				$ret{incomplete} = 1;
			} else {
				$ret{incomplete} = 0;
			...
			# If the currtest already has a name, that means we have already
			# seen its results, so what we have now is a new test.
			# However, if we have NOT seen results for currtest yet, that means
			# currtest just has some diagnostics associated with it,
			# but no actual results, so just use that one.
			...
			# And some other tests have random information after a colon, for
			# example, emoticons such as :-( and :-)
			...
			# Sometimes there is a test which shows up as:
			#	foo (test for excess errors)
			#	foo (something else)
			#	foo: error executing dg-final
			# if it runs, but just:
			#	foo
			# if it does NOT.  When we see the top form, we create a
			...
			if(exists($testindex{$test}) and !$testindex{$test}->{pseudotest}) {
				...
				# Sometimes there is a test at the root of the group.
				# For instance, you will have:
				#	FAIL: foo/bar.c (test for excess errors)
				#	UNRESOLVED: foo/bar.c: failed to open "bar.s": no such file or directory
		...
		if(/^(ERROR|WARNING|NOTE): (.*)/) {
			...
			if($message eq "program timed out.") {
				...
				$message =~ /cannot read "(HOSTCC|libiconv)": no such variable/ or
