...
Element.Methods = {
    ...
    function c(f, g) {
      ...
      if (b || a) {
        if (e in Element._insertionTranslations.tags) {
          ...
          Element._getContentFromAnonymousElement(e, g.stripScripts()).each(
            function (h) {
              f.appendChild(h);
            },
          );
  ...
  insert: function (c, e) {
    ...
    for (var a in e) {
      ...
      d = Object.toHTML(d);
      b = (
        a == "before" || a == "after" ? c.parentNode : c
      ).tagName.toUpperCase();
  ...
  compileXPathMatcher: function () {
    ...
    while (h && g != h && /\S/.test(h)) {
      ...
      for (var f = 0; f < a; f++) {
        ...
        if ((b = h.match(j[f].re))) {
          this.matcher.push(
            Object.isFunction(c[d]) ? c[d](b) : new Template(c[d]).evaluate(b),
          );
          h = h.replace(b[0], "");
          break;
        }
      ...
      not: function (f) {
        ...
        while (j && a != j && /\S/.test(j)) {
          ...
          for (var g = 0; g < h; g++) {
            b = c[g].name;
            if ((f = j.match(c[g].re))) {
              l = Object.isFunction(k[b])
                ? k[b](f)
                : new Template(k[b]).evaluate(f);
